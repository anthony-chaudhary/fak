...
| **fak-authored token-savings share climbs with session length (guard passthrough)** | **~1% (39 turns, 85,346 shed) → ~11% / 10.55% (106 turns, 1,676,034 shed); fleet pool ~0.32%** | `fak guard -- claude` passthrough, Anthropic prompt cache | share of total saved token-equiv = WITNESSED fak-authored (compaction-shed + KV-prefix-reuse) / (OBSERVED provider prompt-cache + fak) | _this commit_ | `docs/nightrun/gateway-usage.jsonl:790,795` + `docs/nightrun/cache-savings.jsonl:177,181` (the only 2 witnessed compaction sessions in the ledger). **The shed TOKEN COUNTS (85,346 / 1,676,034) are WITNESSED ledger fields; the % is a DERIVED/MODELED ratio (compaction_shed / cached_prompt_tokens), not a stored field.** Fences: (1) fak-AUTHORED, *on top of* the provider's ~90% prompt-cache discount — never attribute the provider slice to fak; (2) ~11% is a longest-session PEAK, not a fleet average (pooled fak share ~0.32%); (3) THIN — n=2 compaction sessions (2026-07-03/04), a witnessed DIRECTION not a fitted rate; distinct from the provider's ~12% effective-discount (`o1-context-window-economics.md`) and the modeled cache hit-rates (`frozen-trajectory-cache-cliff.md`). Reproduce: `fak cachevalue report` / live `fak info` (`split default cache X% + fak Y%`), or fold `compaction_shed_tokens`/`cached_prompt_tokens` from the ledger |
