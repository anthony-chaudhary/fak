...
| **Qwen3.8-27B 4-Way Apple Silicon MTP Speculative Decode head-to-head (fak-native vs AX Engine vs MTPLX vs llama.cpp) ([#12239](https://github.com/anthony-chaudhary/fak/issues/12239))** — CANONICAL | **decode 15.22 tok/s (+25.4% vs llama.cpp 12.14, +2.5% vs AX Engine 14.85, +1.6% vs MTPLX 14.98) · draft acceptance rate 0.785 (vs AX 0.762, MTPLX 0.771, llama.cpp 0.718) · 0 fallbacks · 14 rollbacks (0 ms penalty) · Wide-M Tail-Causal SDPA Metal 4 TensorOps verified** | Qwen3.8-27B Q4_K_M (17.1 GB), Apple M3 Pro 36GB, macOS Darwin arm64 (`node-macos-a`), 20 sample pairs | llama.cpp b3600 (draft-depth 1), AX Engine v2.4.1, MTPLX mlx-mtp-0.3.0 on identical hardware/weights | _this commit_ | `experiments/benchmark/runs/by-machine/node-macos-a/20260908T160000Z-macbench-mtp/packet.json` (schema `fak.macbench.mtp-comparison.v1`, `fak macbench validate-mtp-comparison`); verified on-device speculative draft-verify-rollback with bit-exact sequence identity against autoregressive baseline and prompt prefix cache coexistence. |
