...
| **Qwen3.8-27B 3-Way Apple Silicon Metal head-to-head (fak-native vs llama.cpp vs MLX) ([#2723](https://github.com/anthony-chaudhary/fak/issues/2723))** — CANONICAL | **decode 7.61 tok/s (+3.1% vs llama.cpp 7.38, MLX 8.07) · decode ITL 131.17 ms · prefill 48.54 tok/s (vs llama 52.74, MLX 64.10) · RadixAttention shared-prefill TTFT 12.60 ms (>190× vs 2447 ms stateless)** | Qwen3.8-27B Q4_K_M (17.1 GB), Apple M3 Pro 36GB, macOS Darwin arm64 (`node-macos-a`), 20 sample pairs | llama.cpp b3600 (Metal) & MLX 0.22.1 (Metal) on identical hardware/weights | _this commit_ | `experiments/benchmark/runs/by-machine/node-macos-a/20260903T050000Z-macbench-threeway/packet.json` (schema `fak.macbench.comparison.v1`, `fak macbench validate-comparison`); details in `docs/notes/MAC-THREEWAY-BENCH-2026-09-03.md`. Verified matched-envelope single-stream decode leads llama.cpp Metal by +3.1%; prefill compute trails by -8.0%; RadixAttention prefix caching eliminates repeat prefill. |
...
| **Qwen3.8-27B GPU Direct NVMe P2PDMA overflow (AMD RX 7600)** ([#11226](https://github.com/anthony-chaudhary/fak/issues/11226)) | **145.8 decode tok/s (3.00× vs CPU-staged 48.6, 2.59× vs llama.cpp 56.2) · 2450.0 prefill tok/s (2.88× vs 850.2) · 34.80 ms TTFT (4.09× faster vs 142.50 ms) · ITL p95 10.15 ms (−73.6% jitter) · 0 host DRAM bounce copies (strictly zero) · 6.4 GB/s NVMe P2PDMA** | Qwen3.8-27B hybrid Gated-DeltaNet + full attention, AMD Radeon RX 7600 (8GB VRAM) + Crucial T705 Gen5 NVMe | Standard CPU-staged swapping (3 copies) & llama.cpp OS mmap (2 copies) | _this commit_ | `docs/benchmarks/QWEN38-AMD-GPUDIRECT-RESULTS.md` (`fak.modelengine.qwen38-gpudirect-swap/1`; reproduce: `go run ./cmd/fak-dev amd-gpudirect qwen38 --json`). Verified zero-copy NVMe P2PDMA with predictive prefetching eliminates consumer VRAM overflow bottleneck; bit-exact hybrid state round-trip verified across 512–2048 context. |
...
| **Terminal-Bench 4 end-to-end benchmark — fak harness (in-kernel model) vs OpenCode (llama.cpp baseline reference) ([#10803](https://github.com/anthony-chaudhary/fak/issues/10803)) — GATED, no number until witnessed** | **GATED — no comparative solve rate or throughput ratio is claimed until official run artifacts clear the promotion gate. Claim boundary: strictly gated on (a) committed run contract (`fak.tb4-run-contract.v1`), (b) verified task oracle grading receipts (`internal/tb4bench/grader.go`), and (c) comparative artifact synthesis (`tb4-compare-*.json`).** | Qwen3.8-Coder GGUF, pinned seed=42, temp=0.0, 32k context, airgapped OCI container | OpenCode headless execution against dedicated reference `llama-server` on identical weights | _gated_ | `docs/benchmarks/TERMINAL-BENCH-4-REPRODUCTION.md` (reproduction runbook & parity envelope) + `testdata/tb4bench/synthetic_suite.json` (verified offline test suite). Verification: `go test -v -short ./internal/tb4bench/...` |
