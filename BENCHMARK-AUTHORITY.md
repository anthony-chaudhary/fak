...
| **fak-authored token-savings share climbs with session length (guard passthrough)** | **per-session ~15% → ~75% as the session lengthens (widest witnessed span, 2026-07-06 ledger: longest compacting session 746,956 shed / 247,074 provider cache-read = 75.1%; mid ~52–60%; short ~15–18%). Earlier n=2 witness ~1% (39 turns, 85,346 shed) → ~11% (106 turns, 1,676,034 shed). Fleet pool ~0.32–5.2%.** | `fak guard -- claude` passthrough, Anthropic prompt cache | share of total saved token-equiv = WITNESSED fak-authored (compaction-shed + KV-prefix-reuse) / (OBSERVED provider prompt-cache + fak) | _this commit_ | `docs/nightrun/cache-savings.jsonl` — 2026-07-06 per-session rows paired by `generated_at` (fak `compaction_shed_tokens` vs anthropic `cache_read_tokens`): `21:58:33Z` 746,956/247,074 = **75.1%**, `21:13:41Z` 60.5%, `22:23:34Z` 57.6%, `21:28:21Z` 53.8%, `21:48:08Z` 52.7%, `21:48:55Z` 17.8%; earlier n=2 rows `:177,181` + `gateway-usage.jsonl:790,795`. **The shed TOKEN COUNTS are WITNESSED ledger fields; the % is a DERIVED/MODELED ratio (compaction_shed / (compaction_shed + provider cache_read)), not a stored field.** Fences: (1) fak-AUTHORED, *on top of* the provider's prompt-cache discount — never attribute the provider slice to fak; (2) the 75% is a LONGEST-session peak, not a fleet average (pooled fak share ~0.32–5.2%, `fak cachevalue report` owner-attribution table, 2026-W28 ~16.4%); (3) the span is a witnessed DIRECTION (share rises with horizon), not a fitted rate — sessions that never hit the compaction budget shed 0 and sit at ~0%; distinct from the provider's ~12% effective-discount (`o1-context-window-economics.md`) and the modeled cache hit-rates (`frozen-trajectory-cache-cliff.md`). Reproduce: `fak cachevalue report` (owner-attribution + fleet-benefit) / live `fak info` (`split default cache X% + fak Y%`), or fold `compaction_shed_tokens`/`cache_read_tokens` per `generated_at` from the ledger |
