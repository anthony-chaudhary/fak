...
func Registry() []Mechanism {
	return []Mechanism{
		...
		{ID: "context_elision", Label: "stale-read context elision", Producer: "internal/agent anthropic elision path", Status: Unavailable, Units: []string{"input_tokens"}, Reason: "live gateway does not yet publish elided input-token count"},
		{ID: "schema_tool_filtering", Label: "schema/tool filtering", Producer: "internal/gateway tool projection/filter path", Status: Unavailable, Units: []string{"input_tokens"}, Reason: "live gateway does not yet publish filtered schema-token count"},
		{ID: "cold_tool_defer", Label: "cold-tool defer routing", Producer: "internal/gateway.AdjudicationSummary.DeferColdCount", Status: Overlapping, Units: []string{"model_calls", "latency"}, SourceID: "unknown", ExclusivityGroup: "routing_defer_decision/v1", Reason: "decision count is visible in Cache ablation; token/call counterfactual overlaps provider execution and is not independently summable"},
		{ID: "model_routing", Label: "model routing/defer", Producer: "internal/gateway routing decision records", Status: Unavailable, Units: []string{"latency", "bytes"}, Reason: "route choice is visible, but no calibrated per-route counterfactual is emitted"},
