...
// sink must not build or marshal the event at all.
func TestGatewayOperationLogSkippedWithoutSink(t *testing.T) {
	var srv *Server
	srv.logGatewayOperation("adjudicate", "t", "x", WireVerdict{Kind: "ALLOW"}, nil, time.Microsecond) // nil receiver must not panic
	(&Server{}).logGatewayOperation("adjudicate", "t", "x", WireVerdict{Kind: "ALLOW"}, nil, time.Microsecond)
}
