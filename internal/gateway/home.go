...
<h2 class="section-title">Rich dashboards</h2><p class="section-note">Choose a named view below; on-demand · {{len .RichDashboards}} dashboards.</p>
...
const qKey=new URLSearchParams(window.location.search).get("key");
if(qKey){try{sessionStorage.setItem("fak_gateway_key",qKey)}catch(e){}}
const storedKey=qKey||(()=>{try{return sessionStorage.getItem("fak_gateway_key")}catch(e){return null}})();
if(storedKey){document.querySelectorAll("a.card, a.rich-dashboard").forEach(a=>{const href=a.getAttribute("href");if(href&&href.startsWith("/")&&!href.startsWith("/healthz")&&!href.includes("key=")){const sep=href.includes("?")?"&":"?";a.setAttribute("href",href+sep+"key="+encodeURIComponent(storedKey))}})}
const authHeaders=storedKey?{"Authorization":"Bearer "+storedKey}:{};
...
async function refresh(){try{const [health,metrics]=await Promise.all([fetch("/healthz",{headers:{Accept:"application/json"}}),fetch("/metrics",{headers:{Accept:"text/plain",...authHeaders}})]);if(!health.ok||!metrics.ok)throw new Error("health "+health.status+", metrics "+metrics.status);const healthBody=await health.json(),metricsBody=await metrics.text();byId("live-ready").textContent=(healthBody.ok===false||healthBody.ready===false)?"not ready":"ready";show("live-requests",metric(metricsBody,"fak_gateway_http_requests_total"));show("live-cache-hits",metric(metricsBody,"fak_gateway_inference_cached_prompt_hits_total"));show("live-inflight",metric(metricsBody,"fak_gateway_inflight_requests"));state.textContent="live";state.dataset.state="live";note.textContent="Updated "+new Date().toLocaleTimeString()+" / refreshes every 5 seconds."}catch(error){state.textContent="unavailable";state.dataset.state="unavailable";note.textContent="Live refresh failed; last good values are preserved. Open Health or Metrics below for details."}}
