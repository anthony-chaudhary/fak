...
//
// The rungs, in ascending order of the decisions they permit:
//
...
// presetLadder is the ascending-agency order. It is the ladder's contract:
...
// PresetNames returns the built-in preset names in ascending-agency order. The
// returned slice is a copy — a caller cannot reorder the ladder in place.
...
func PresetManifest(name string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("policy: unknown preset %q; valid rungs (ascending agency): %s",
