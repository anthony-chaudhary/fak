...
var exemptions = map[string]string{
	...
	"switch|internal/cachelayout|TestHybridCacheTypeRegistrySpine":                              "Test helper enumerates the subset of layer kinds it exercises; partial by design.",
	"switch|internal/deepseekv4moe|cachePolicyReplayArm":                                        "Bench replay arm exercises the cache policies under test; remaining policies are handled by the default arm.",
	"literal|internal/fleetaccounts|anthropicThinkingBudgets":                                   "Provider thinking-budget map lists only supported effort levels; ReasoningNone needs no budget.",
	"literal|internal/fleetaccounts|reasoningProfiles.ReasoningProviderOpenAIResponses.allowed": "Per-provider allowed list names supported effort levels; ReasoningNone is the unset default.",
	"literal|internal/fleetaccounts|reasoningProfiles.ReasoningProviderAnthropic.allowed":       "Per-provider allowed list names supported effort levels; ReasoningNone is the unset default.",
	"literal|internal/fleetaccounts|reasoningProfiles.ReasoningProviderGemini.allowed":          "Per-provider allowed list names supported effort levels; ReasoningNone and unsupported ReasoningXHigh are deliberately omitted.",
	"literal|internal/gateway|proofFeatures":                                                    "Proof-feature registry list names only features with dedicated proof entries; the remainder share the generic proof path.",
	"switch|internal/ggufload|AdmittedRoutedExpertEncoding":                                     "Admitted encoding switch handles routed-expert encodings only; other tensor types are rejected as unsupported for routed experts.",
	"switch|internal/ggufload|intArrayValue":                                                    "Test helper decodes the integer array value types it constructs; other value types are exercised by separate unit tests.",
	"literal|internal/modelroute|DefaultFreeTierRoster.Accounts":                                "Default free-tier roster names only accounts that have a free tier; paid and local kinds are intentionally unlisted.",
	"switch|internal/selfupdate|ClassifyOutcome":                                                "Outcome classifier maps settled update outcomes; self-version comparison states are classified by the version check, not the receipt classifier.",
	"literal|internal/model|widestFirst":                                                        "Test helper enumerates the precision levels it exercises; LevelKVUnknown is a non-selectable sentinel handled by the loader, not the widest-first search.",
}
