...
// TestQ4KReduceAsmMatchesScalar pins the NEON SDOT reduction kernel to the scalar reference on the
...
// int32 reductions match, the full asm-path dot equals the scalar-int8-path dot exactly. SDOT and
// the ones-vector sum are associative with no overflow on these ranges, so any lane order yields
// the same int32 — a mismatch here means a real unpack/sign/ordering bug in the asm. Skips on a
// part without FEAT_DotProd (where the dispatcher never calls the asm).
func TestQ4KReduceAsmMatchesScalar(t *testing.T) {
	if !detectDotProd() {
		t.Skip("FEAT_DotProd (asimddp) not available — q4k asm inactive")
	}
	...
	for o := 0; o < out; o++ {
		...
		q4kReduceRowAsm(&row[0], nblk, &qv.q[0], &isAsm[0], &ssAsm[0])
		q4kReduceRowScalar(row, nblk, qv.q, isSc, ssSc)
		for i := range isAsm {
			if isAsm[i] != isSc[i] {
				t.Fatalf("row %d IS[%d]: asm=%d scalar=%d (nibble-dot mismatch — unpack/SDOT bug)", o, i, isAsm[i], isSc[i])
			}
			if ssAsm[i] != ssSc[i] {
	...
	t.Logf("q4k SDOT reduce bit-identical to scalar across %d rows x %d sub-blocks (neonDot=%v)", out, nblk*8, neonDot)
}
