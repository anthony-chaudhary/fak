...
// q8w, and f32-manifest entries must report their exact byte/param counts and the correct
// decode-bytes-per-token (= q4k + q8 matmul bytes). This is the cheap observability witness
// for the loader's resident split, runnable in ms without any 27B load.
func TestResidentReportByteMath(t *testing.T) {
	...
	if r.TotalResidentBytes != 288+wantQ8Bytes+480 {
		t.Errorf("total=%d want %d", r.TotalResidentBytes, 288+wantQ8Bytes+480)
	}
	if r.DecodeBytesPerToken != 288+wantQ8Bytes {
		t.Errorf("decode bytes/token=%d want %d (q4k+q8 only)", r.DecodeBytesPerToken, 288+wantQ8Bytes)
	}
