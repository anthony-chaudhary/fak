...
// TestQ6KReduceAsmMatchesScalar pins the AVX2 Q6_K reduction kernel to the scalar reference on the
// integer reductions it owns (IS = Σ q6*qx, SS = Σ qx), bit-for-bit. The float combine is shared
// Go, so once the int32 reductions match, the full asm-path dot equals the scalar-int8-path dot
// exactly. The Q6_K 2-bit qh is reassembled per-position with shifts; a mismatch here means a real
// qh-bit / nibble / position bug in the asm. Skips on a CPU without AVX2.
func TestQ6KReduceAsmMatchesScalar(t *testing.T) {
	if !detectAVX2() {
		t.Skip("AVX2 not available — q6k asm inactive")
	}
	const out, in = 9, 768 // 3 super-blocks/row, odd rows
	nblk := in / qkK
	...
	for o := 0; o < out; o++ {
		row := qt.raw[o*rowBytes : (o+1)*rowBytes]
		q6kReduceRowAsmAVX2(&row[0], nblk, &qv.q[0], &isAsm[0], &ssAsm[0])
	...
	t.Logf("q6k AVX2 reduce bit-identical to scalar across %d rows x %d groups (tier=%d)", out, nblk*q6kGroupsPerBlock, qtier)
}
