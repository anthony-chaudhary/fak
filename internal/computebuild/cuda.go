...
func RunCUDA(ctx context.Context, cfg *CUDAConfig) error {
	...
	case "build":
		if _, err := BuildCUDALib(ctx, cfg.Toolchain, cfg.RepoRoot, cfg.Arch, cfg.EnableNCCL, cfg.Stdout); err != nil {
			return err
		}
		...
		if outBin != "" {
			...
			if cfg.Toolchain.IsWindows {
				artifact := filepath.Join(cfg.RepoRoot, outBin)
	...
	default:
		return fmt.Errorf("unknown subcommand: %s (use check|build|test|bench|binary)", cfg.Command)
	}
}
