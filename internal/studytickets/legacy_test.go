...
func auditTicket(id string, n int, deps ...string) AuditTicket {
	return AuditTicket{ClusterID: id, CandidateID: id, Disposition: Selected, Issue: n, Outcome: "bounded", Source: "vllm@rev", For: "operators", Problem: "gap", Today: "manual", BetterBecause: "witnessed", Witness: "test", Centrality: "Core", P1P4: "pass", Horizon: "now", CloseCondition: "green", NativeConstraint: "fak-native", Dependencies: deps}
}
func audit() Audit {
	return Audit{Schema: "fak.study-ticket-audit/1", Cutoff: "2026-08-26", SourceRevision: "vllm@abc", Checksum: "sha256:x", PriorityChecksum: "sha256:y", SourceCount: 4, CreatedCount: 1, ReusedCount: 1, RefreshObligations: []string{"refresh"}, Tickets: []AuditTicket{auditTicket("base", 100), func() AuditTicket { x := auditTicket("next", 101, "base"); x.Existing = true; return x }(), {ClusterID: "later", Disposition: Deferred}, {ClusterID: "old", Disposition: Landed}}}
}
...
func TestRejectsInvalidAudit(t *testing.T) {
	...
	for i, mut := range cases {
		a := audit()
		mut(&a)
		if !errors.Is(ValidateAudit(a), ErrInvalid) {
