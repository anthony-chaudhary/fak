...
func ProductOutcomes() []ProductOutcome {
	return []ProductOutcome{
		...
		{ID: "context-compression", Name: "Compress tool output before it reaches the model", Summary: "measure reversible context compression on built-in or captured tool output so fewer input tokens enter the model window", Tags: []string{"token savings", "save tokens", "context compression", "tool output compression", "headroom", "input tokens", "long context", "efficiency"}, Effect: "read", Command: []string{"fak", "headroom", "bench", "--json"}, Detail: "docs/CAPABILITIES.md#context-compression", Witness: "internal/headroom + docs/benchmarks/CONTEXT-COMPRESSION-LIVE-2026-08-09.md"},
