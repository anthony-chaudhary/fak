...
const (
	...
	SubagentStatePrefilling SubagentState = "prefilling"
...
type SubagentSchedulerConfig struct {
	...
	// Chunked Prefill and Interleaved Admission Parameters (#12328)
	ChunkSize int // default 256 tokens per prefill chunk
	TMax      int // default 512 tokens max per iteration (prefill budget = TMax - B_active_decodes)
}
...
type SubagentSession struct {
	...
	// Continuous batching and chunked prefill tracking (#12328)
	promptCursor  int
	prefillDone   bool
	PrefillChunks int
...
// PromptCursor returns the count of prompt tokens prefilled so far.
func (s *SubagentSession) PromptCursor() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.promptCursor
}

// PrefillDone reports whether the subagent's prompt prefill has finished.
func (s *SubagentSession) PrefillDone() bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.prefillDone
}

// IsPrefilling reports whether the session is currently prefilling its prompt.
func (s *SubagentSession) IsPrefilling() bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.LaneState == schedLanePrefilling
}

// IsDecodeReady reports whether the session has finished prefill and is ready for decode.
func (s *SubagentSession) IsDecodeReady() bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.LaneState == schedLaneDecode
}
...
type RaggedBatch struct {
	...
	PrefillTokens      int
	PrefillSessions    []*SubagentSession
	PrefillChunkSizes  []int
	TotalTokensInBatch int
	RaggedRequests     []compute.RaggedQKVBatchRequest
}
...
type SubagentSchedulerReceipt struct {
	...
	TotalPrefillChunks      int64
	TotalPrefillTokens      int64
	DecodeStallsCount       int64
	PrefillInterleavedSteps int64
}
...
type SubagentScheduler struct {
	...
	subagentPrefillPool []*SubagentSession
...
func NewSubagentScheduler(cfg SubagentSchedulerConfig) (*SubagentScheduler, error) {
	...
	return &SubagentScheduler{
		...
		subagentPrefillPool: make([]*SubagentSession, 0),
