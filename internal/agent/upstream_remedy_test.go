...
func TestClassifyUpstream(t *testing.T) {
	cases := []struct {
		...
		{"nebius exhausted-budget 402 (live body)", http.StatusPaymentRequired,
			`{"detail":"Payment Required: You have exhausted your budget. Please add funds to continue using the API."}`, RemedyFailoverAccount},
		{"openai insufficient_quota 402", http.StatusPaymentRequired,
			`{"error":{"type":"insufficient_quota","message":"You exceeded your current quota"}}`, RemedyFailoverAccount},
