...
func loadSelfQueryDevCatalog(root string) (*selfquery.DevCatalog, error) {
	...
	for _, verb := range cat.Verbs() {
		out.Verbs = append(out.Verbs, selfquery.DevVerb{Name: verb.Name, Synopsis: verb.Synopsis, Lane: verb.Lane, Binary: verb.Binary, Aliases: append([]string(nil), verb.Aliases...)})
	}
	return out, nil
}
