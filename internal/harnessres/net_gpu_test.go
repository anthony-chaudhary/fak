...
func TestNetAndGPUProvidersFold(t *testing.T) {
	...
	s.SetGPUUtilProvider(func() (pct float64, ok bool) { return 73, true })
	...
	if !snap.HaveGPUUtil || snap.GPUUtilPercent != 73 {
		t.Fatalf("gpu util = %g (have=%v), want 73%%", snap.GPUUtilPercent, snap.HaveGPUUtil)
	}
	...
	if !strings.Contains(rep, "gpu vram 6.0 GiB/24.0 GiB util 73%") {
		t.Errorf("Report missing gpu vram+util: %s", rep)
	}
	...
	if !strings.Contains(prom, "fak_harness_gpu_utilization_percent 73") {
		t.Errorf("Prometheus missing gpu utilization: %s", prom)
	}
	...
	if _, ok := m["gpu_utilization_percent"]; !ok {
		t.Errorf("ledger missing gpu_utilization_percent: %s", b)
	}
	...
	if strings.Contains(string(bare), "gpu_vram") || strings.Contains(string(bare), "gpu_utilization") || strings.Contains(string(bare), "net_rx") {
