...
func RequiredN(s PowerSpec) (int, error) {
	...
	za := normInvCDF(1 - s.Alpha/float64(s.Tails))
	zb := normInvCDF(s.Power)
...
func Simulate(s PowerSpec, n, trials int, tol float64) SimResult {
	...
	zc := normInvCDF(1 - s.Alpha/float64(s.Tails))
...
// the z-critical values that size a sample are the same on every platform (reproducible
// provenance, not a drifting table lookup). p<=0 and p>=1 saturate to -Inf/+Inf.
