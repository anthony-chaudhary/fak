...
function subscribeSessionEvents(){if(!window.EventSource)return;try{const es=new EventSource("/api/sessions/events");es.addEventListener("session_update",e=>{if(e.data){try{const p=JSON.parse(e.data);if(p.html){sessionCards.innerHTML=p.html;bindSessionCards();return}}catch(_){}}loadSessions()});es.addEventListener("session_cards",e=>{if(e.data){try{const p=JSON.parse(e.data);if(p.html){sessionCards.innerHTML=p.html;bindSessionCards();return}}catch(_){}}loadSessions()});es.addEventListener("approval_requested",e=>{if(e.data){try{const p=JSON.parse(e.data);sessionFeedback.textContent="Approval requested for "+(p.session_id||"session")}catch(_){}}loadSessions()});es.addEventListener("approval_resolved",e=>{if(e.data){try{const p=JSON.parse(e.data);sessionFeedback.textContent="Approval "+(p.resolution||"resolved")+" for "+(p.session_id||"session")}catch(_){}}loadSessions()});es.onmessage=()=>{loadSessions()}}catch(_){}}
