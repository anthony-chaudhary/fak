...
.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.session-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px}.session-card{min-width:0;border:1px solid var(--line);border-radius:16px;padding:14px;background:var(--panel);outline:none}.session-card:focus{box-shadow:0 0 0 3px var(--accent-soft)}.session-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.provider,.session-state{font-size:12px;border:1px solid var(--line);border-radius:999px;padding:3px 7px}.session-state{font-weight:700}.state-awaiting-approval,.state-awaiting-input{color:var(--warn)}.state-failed,.state-disconnected{color:var(--bad)}.state-working{color:var(--good)}.session-card dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:12px 0}.session-card dl div{min-width:0}.session-card dt{font-size:11px;color:var(--muted);text-transform:uppercase}.session-card dd{margin:2px 0;overflow-wrap:anywhere}.pending{padding:8px;border-left:3px solid var(--warn);background:var(--warn-soft)}.session-controls{display:flex;gap:6px;flex-wrap:wrap}.session-controls button[disabled]{opacity:.55;cursor:not-allowed}@media(max-width:560px){.session-grid{grid-template-columns:minmax(0,1fr)}.session-title{align-items:flex-start}.session-state{margin-left:0}.session-card dl{grid-template-columns:minmax(0,1fr)}.session-controls{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.session-controls button{width:100%;min-height:44px}}
</style></head><body><main><div class="topbar"><div class="brand"><span class="mark">f</span><p class="eyebrow">fak operator home</p></div><p class="mode"><span class="dot"></span><span id="status">loading local state…</span></p></div><section class="shell"><header class="top"><h1>Harness overview</h1><p class="sub">Agents, goals, and live operating surfaces in one place. Start a run after you know what is active.</p><div class="actions"><a id="gateway-link" class="button primary" aria-disabled="true">Web gateway</a><button id="refresh" class="button" type="button">Refresh status</button><button id="skin" class="skin" type="button">Theme</button></div></header><nav class="nav" aria-label="Harness pages"><a href="#overview">Overview</a><a href="#agents">Agent stats</a><a href="#goals">Goals</a><a href="#dashboards">Live dashboards</a><a href="#sessions">Sessions</a><a href="#run">Run agent</a></nav><div class="content">
...
function showLocalWork(data){const local=data.local_work||{},intents=local.issue_intents||{},leases=local.dos_leases||{},target=document.querySelector("#local-work");target.replaceChildren();for(const issue of intents.ids||[])target.append(row(issue,"active local intent","intent"));for(const lane of leases.ids||[])target.append(row(lane,"active DOS lane lease","lease"));if(!target.children.length)target.append(empty("No active local issue intents or DOS lane leases."));const stats=document.querySelector("#stats");stats.append(stat(intents.active||0,"Local issue intents",intents.readable?"active repository claims":"intent state unavailable"),stat(leases.active||0,"DOS lane leases",leases.readable?"active local lanes":"DOS state unavailable"))}
...
func handlerWithAllSources(s *store, live *liveAdapter, goals goalLister, sessions SessionSource, local LocalWorkSource, root string) http.Handler {
	...
	installSessionRoutes(mux, sessions)
