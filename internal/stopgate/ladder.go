...
func StageMessage(stage Stage, consecutive, maxN int) string {
	switch stage {
	case StageWarn:
		return fmt.Sprintf("fak guard: the last %d turns each closed while the capability floor was still waiting for a shape it can admit, so the same approach keeps returning. Good moment to try a fresh angle: if the remaining work is reachable under this floor, take a different allowed action now — a different tool, a narrower command, or a path the floor welcomes. If a protected boundary blocks progress, land safe verified sub-components, emit a scoped ABSTAIN record with checkable next steps, and ensure required witnesses are satisfied. (Auto-continue %d of %d before fak lets the turn end.)", consecutive, consecutive, maxN)
	case StageFinal:
		return fmt.Sprintf("fak guard: last auto-continue (%d of %d). After %d turns still waiting on a shape the floor can admit, fak will let the session wrap up. If there is an allowed way forward, take it on this turn; otherwise land any verified sub-components, record a scoped ABSTAIN verdict with the blocking boundary and checkable next steps, and satisfy all witness gates.", consecutive, maxN, maxN)
	default:
...
func SameStageMessage(stage Stage, sameConsecutive, stop int) string {
	switch stage {
	case StageWarn:
		return fmt.Sprintf("fak guard: you have now ended %d turns in a row proposing the IDENTICAL refused action — the capability floor is setting aside the very same tool call, for the same reason, each time. Repeating it will not change the verdict. Try a genuinely different angle now: a different tool, a narrower command, or a path the floor welcomes. The in-band `[fak]` note labels the block as `Tool (REASON/DISPOSITION)` — let that reason point the way. If a protected boundary blocks progress, land safe verified sub-components, emit a scoped ABSTAIN record with checkable next steps, and ensure required witnesses are satisfied. (Auto-continue %d of %d identical repeats before fak lets the turn end.)", sameConsecutive, sameConsecutive, stop)
	case StageFinal:
		return fmt.Sprintf("fak guard: last auto-continue (%d of %d). You have proposed the IDENTICAL refused action %d turns running; one more and fak will let the session wrap up. This is a genuine repeat, not exploration — if there is an allowed way forward, take a DIFFERENT action on this turn; otherwise land any verified sub-components, record a scoped ABSTAIN verdict with the blocking boundary and checkable next steps, and satisfy all witness gates.", sameConsecutive, stop, sameConsecutive)
	default:
		return ContinueReason
	}
}
