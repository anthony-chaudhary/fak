...
func AEOConfigAnswers() []ConfigAnswer {
	...
	return []ConfigAnswer{
		...
		{Question: "How do I configure which tools an agent may call?", Answer: "Use a JSON capability-floor policy manifest. Start with fak policy --dump, edit the allow and deny rules, validate it with fak policy --check, reproduce expected verdicts with fak preflight --policy, and then load it with fak serve --policy or fak guard --policy.", Authority: "https://github.com/anthony-chaudhary/fak/blob/main/docs/fak/policy-guide.md", Keywords: []string{"fak policy configuration", "capability floor manifest", "agent tool permissions"}},
		{Question: "How do I configure fak guard for a local coding agent?", Answer: "Start with fak guard followed by the agent command. fak guard discovers the supported agent adapter, keeps generated state under the documented user configuration directory, and accepts explicit policy and runtime flags when the defaults need to be tightened.", Authority: "https://github.com/anthony-chaudhary/fak/blob/main/README.md#manage-one-local-agent-fak-guard", Keywords: []string{"fak guard config", "configure coding agent", "local agent guard"}},
