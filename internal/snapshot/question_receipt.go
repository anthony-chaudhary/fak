...
// NewQuestionReceipt captures only governing inputs. Unrelated state is not an
// argument and therefore cannot spuriously invalidate an answer.
func NewQuestionReceipt(s QuestionState) (QuestionReceipt, error) {
	...
	return QuestionReceipt{
		Version:           1,
		QuestionID:        digest([]byte(q)),
		EvidenceDigest:    digest(s.RelevantEvidence),
		GoverningRevision: policy,
		AuthorityTenure:   tenure,
	}, nil
}
...
func ParseQuestionReceipt(raw []byte) (QuestionReceipt, ReceiptCheck) {
	...
	if err := dec.Decode(&r); err != nil || r.Version != 1 || !validDigest(r.QuestionID) || !validDigest(r.EvidenceDigest) || strings.TrimSpace(r.GoverningRevision) == "" || strings.TrimSpace(r.AuthorityTenure) == "" {
...
func VerifyQuestionReceipt(r QuestionReceipt, current QuestionState) ReceiptCheck {
	...
	if err != nil || r.Version != 1 || !validDigest(r.QuestionID) || !validDigest(r.EvidenceDigest) {
