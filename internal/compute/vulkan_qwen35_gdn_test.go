...
// TestQwen35GDNVulkanValueRMSParallelParity witnesses the Strix P0 leaf
// "parallelize the GDN value-state RMS reduction"
// (anthony-chaudhary/fak#12532): the post-recurrence RMS factor was produced by
// a serial lane-0 walk of readout[] while the other 63 invocations waited
// behind two barriers. Each lane now accumulates a strided slice of
// readout[d]^2, the shared tree reduces across the group, and lane 0 broadcasts
// one rmsInv per value head.
//
// The test is split so the source contract always runs (no GPU required) and
// the numeric parity runs whenever a Vulkan device is present:
//
//  1. Shader-contract: the recurrent shader must contain the cooperative
//     strided value-state accumulation + shared tree reduction and must NOT
//     reintroduce the serial lane-0 walk over pc.vHd. This fails
//     deterministically if the serialization returns.
//  2. Numeric parity: executing the shader through Qwen35GDNPreprojected on
//     deterministic Qwen3.8 GDN geometry, including non-power-of-two value
//     widths, must match the scalar oracle within the established tolerance,
//     proving the cooperative reduction preserves one exact normalization
//     factor per value head.
func TestQwen35GDNVulkanValueRMSParallelParity(t *testing.T) {
	t.Run("ShaderContract", func(t *testing.T) {
		candidates := []string{
			"shaders/qwen35_gdn_recurrent.comp",
			"internal/compute/shaders/qwen35_gdn_recurrent.comp",
		...
		// Cooperative value-state RMS reduction seams the leaf requires.
		requiredTokens := []string{
			...
			"rmsPart[lane] += rmsPart[lane + s];",
			"rmsInv = inversesqrt(rmsPart[0] / float(pc.vHd) + pc.eps);",
		}
		for _, tok := range requiredTokens {
			if !strings.Contains(content, tok) {
				t.Errorf("shader %s missing cooperative-RMS token %q", foundPath, tok)
			}
		}

		// The serial lane-0 walk that must NOT come back: a single-threaded loop
		// across the entire value-head dimension accumulating the RMS sum.
		serialTokens := []string{
			"if (lane == 0) {\n            float sum = 0.0;",
			"for (int d = 0; d < pc.vHd; ++d) sum += readout[d] * readout[d];",
			"rmsInv = inversesqrt(sum / float(pc.vHd) + pc.eps);",
		}
		for _, tok := range serialTokens {
			if strings.Contains(content, tok) {
				t.Errorf("shader %s reintroduced the serial lane-0 value-state RMS walk: %q", foundPath, tok)
			}
		}
	})
	...
	t.Run("NumericParity", func(t *testing.T) {
		...
		testGDN := func(t *testing.T, tokens, nK, nV, kHd, vHd, kernel int, nonzeroRecState bool, inputScale float32) {
			...
			if worst > 2e-4 {
				t.Fatalf("parallel value-state RMS reduction changed output: worst max abs delta %g > 2e-4", worst)
			}
		}

		// vHd=65 makes the strided accumulation span two passes per lane and
		// leaves 63 of 64 lanes idle in the final (64th) slot, exercising the
		// non-power-of-two logical width the current geometry supports.
		testGDN(t, 2, 1, 1, 2, 65, 3, false, .025)
		testGDN(t, 5, 2, 4, 4, 65, 3, true, .025)
		testGDN(t, 3, 1, 2, 4, 65, 3, true, .00025)
	})
