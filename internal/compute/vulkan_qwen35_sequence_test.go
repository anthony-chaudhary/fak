...
func TestQwen35VulkanSequenceKVReserveGeometric(t *testing.T) {
	...
	t.Run("DeviceKVReserveAndOutputParity", func(t *testing.T) {
		...
		// Compare both allocation policies with the input so shared corruption cannot pass.
		originalPlanes := [][]float32{allKRawData, allKData, allVData}
		for plane, pair := range [][2]*vslice{{cacheExactKRaw, cacheGeomKRaw}, {cacheExactK, cacheGeomK}, {cacheExactV, cacheGeomV}} {
			...
			for i := 0; i < totalTokens*kvWidth; i++ {
				if readExact[i] != originalPlanes[plane][i] || readGeom[i] != originalPlanes[plane][i] {
					t.Fatalf("plane %d buffer mismatch at float %d: exact=%g geom=%g input=%g", plane, i, readExact[i], readGeom[i], originalPlanes[plane][i])
				}
			}
		}
