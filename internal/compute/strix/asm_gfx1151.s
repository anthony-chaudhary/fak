...
write_output:
    ...
    .globl  wmma_dequant_fp4_wave32_16x16x16
    .p2align 8
    .type   wmma_dequant_fp4_wave32_16x16x16,@function

wmma_dequant_fp4_wave32_16x16x16:
    // Kernel Argument Offsets (s[0:1] = kernarg pointer)
    //   0x00: Global Input Matrix A pointer (FP16/BF16, [M, K])
    //   0x08: Packed FP4 (E2M1) Weight Matrix B pointer ([K, N], 2 nibbles/byte)
    //   0x10: FP16 per-block scale pointer (one scale per 32-value block)
    //   0x18: Output Matrix C pointer (FP32, [M, N])
    //   0x20: Dimension K (int32)
    //   0x24: Stride N (int32)

    s_load_dwordx4 s[4:7], s[0:1], 0x00      // s[4:5]=MatrixA, s[6:7]=PackedFP4B
    s_load_dwordx4 s[8:11], s[0:1], 0x10     // s[8:9]=Scales, s[10:11]=MatrixC
    s_load_dwordx2 s[12:13], s[0:1], 0x20    // s[12]=K, s[13]=StrideN
    s_waitcnt lgkmcnt(0)
    ...
    // Address computation for 128-byte coalesced FP4 weight burst
    v_add_co_u32 v2, vcc_lo, s6, v1          // Weight address low
    v_add_co_ci_u32_e32 v3, s7, 0, vcc_lo    // Weight address high
...
main_fp4_wmma_loop:
    // 128-bit Vector Load: 16 packed FP4 bytes per lane * 32 lanes = 512 bytes per Wave32
    // wavefront. Exactly 4x 128-byte coalesced memory transactions on the 256-bit LPDDR5X bus.
    // Zero LDS spill: packed FP4 bytes stream directly into VGPRs.
    global_load_dwordx4 v[4:7], v[2:3], off   // packed FP4 (E2M1), 2 nibbles/byte
    global_load_dwordx4 v[8:11], v[0:1], off  // Load Matrix A activation slice
    ...
    // ON-THE-FLY IN-REGISTER E2M1 DEQUANT (block-32 + f16 scale):
    // This is an on-the-fly in-register dequant feeding the tensor core. Each packed byte
    // holds two E2M1 (sign/exp/mantissa) nibbles that are split with v_and_b32 0x0f0f0f0f
    // and v_lshrrev_b32 4; the decoded values are consumed directly at the WMMA operand.
    // There is NO intermediate LDS staging and NO full-tensor FP32 expansion.
    // Extract lower nibble (nibble 0) from v4 into v24
    v_and_b32 v24, 0x0f0f0f0f, v4
    // Extract upper nibble (nibble 1) from v4 into v25
    v_lshrrev_b32 v25, 4, v4
    v_and_b32 v25, 0x0f0f0f0f, v25

    // In-register E2M1 nibble decode + per-block f16 scale application (zero LDS traffic)
    v_cvt_f32_u32 v24, v24
    v_cvt_f32_u32 v25, v25
    v_mul_f32 v24, v24, v12
    v_mul_f32 v25, v25, v12
    ...
    // Pack the dequantized pairs in VGPRs for the WMMA input operand B
    v_pack_b32_f16 v28, v24, v25
    v_pack_b32_f16 v29, v26, v27

    // Issue Wave32 RDNA 3.5 FP4 WMMA Matrix Instruction:
    // v_wmma_f32_16x16x16_fp4 accumulates D = A * B + C directly in-register with the
    // on-the-fly in-register dequantized E2M1 block-32 operands.
    v_wmma_f32_16x16x16_fp4 v[16:23], v[8:11], v[28:29], v[16:23]

    // Advance K iteration pointer by 16 columns (128-byte block step)
    s_sub_i32 s12, s12, 16
    s_cmp_gt_i32 s12, 0
    s_cbranch_scc1 main_fp4_wmma_loop
...
write_fp4_output:
    ...
    .rodata
    .p2align 6
    .amdhsa_kernel wmma_dequant_fp4_wave32_16x16x16
        .amdhsa_group_segment_fixed_size 0
        .amdhsa_private_segment_fixed_size 0
        .amdhsa_user_sgpr_dispatch_ptr 0
        .amdhsa_user_sgpr_kernarg_segment_ptr 1
