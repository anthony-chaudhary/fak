...
2. Run the **timed** prefill bench at P=256/512/1024 against llama.cpp Q8_0; record lineage
   (version + UTC + commit + sanitized machine + harness).
3. Check the roofline's bottleneck prediction (which stage dominates at each P) against the
   measured profile; aim the attention/graph work at the stage the model flags.
