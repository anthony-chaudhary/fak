...
func promptRule(id, imperative, witness string) PromptRule {
	return PromptRule{ID: id, Imperative: imperative, Witness: witness}
}
...
func WorkRules(issue int, lane string) []PromptRule {
	return []PromptRule{promptRule(
		...
		promptRule(
			"proof-by-default",
			fmt.Sprintf("Match the proof to the defect: visual/TUI bugs need a captured "+
		...
		promptRule(
			"browser-display",
			"Keep browser automation off the operator desktop. Use headless browser mode and "+
...
func GitLawRules(issue int, lane, branch string) []PromptRule {
	return []PromptRule{promptRule(
		"main-only",
		fmt.Sprintf("Work on the configured development branch `%s` ONLY - never "+
		...
		promptRule(
			...
			fmt.Sprintf("Reference `#%[1]d` in the subject AND end it with a `(fak %[2]s)` "+
		...
		promptRule(
			"no-history-rewrite",
			fmt.Sprintf("No push / tag / force-push / history-rewrite / reset / clean / "+
				"checkout-of-tracked-files - just commit on `%s`", branch),
			"NEVER_AMEND_SHARED"),
		promptRule(
			"pathspec-race-recovery",
			"On a race refusal, preserve your working-tree edits, refresh from the current "+
