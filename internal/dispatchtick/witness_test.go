...
func TestClassifyNoCommitReason(t *testing.T) {
	...
	cases := []struct {
		...
		{"prompt-only off-trunk token", "- main-only: work on main - witness `OFF_TRUNK`\n", 2000, NoCommitUnknown},
		{"off-trunk guard refusal", "commit refused: OFF_TRUNK\n", 2000, NoCommitOffTrunk},
		{"anchored off-trunk refusal", "hook output\nOFF_TRUNK: trunk-only commit refused\n", 2000, NoCommitOffTrunk},
		{"prompt plus runtime refusal", "- witness `OFF_TRUNK`\nhook output\npush refused: OFF_TRUNK\n", 2000, NoCommitOffTrunk},
