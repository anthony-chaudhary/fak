...
func TestMarkdownContractIsCompleteAndIdempotent(t *testing.T) {
	...
	for _, p := range paths {
		...
		want = migrateMarkdownGenerator(want)
...
func TestMarkdownContractNamesGoGenerator(t *testing.T) {
	s := card(t)
	tmp := t.TempDir()
	paths, err := s.WriteMarkdown(tmp)
	if err != nil {
		t.Fatal(err)
	}
	for _, p := range paths {
		...
		text := string(raw)
		if strings.Contains(text, "tools/lightgap_scorecard.py") {
			t.Fatalf("%s still names deleted Python generator", filepath.Base(p))
		}
		if !strings.Contains(text, "GENERATED by fak score lightgap") {
			t.Fatalf("%s lacks Go generator attribution", filepath.Base(p))
		}
	}
}
