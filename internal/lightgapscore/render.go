...
// regeneration in place rewrites only stale generator attribution and commands.
func (s *Scorecard) WriteMarkdown(dir string) ([]string, error) {
	...
	for _, name := range names {
		...
		data = migrateMarkdownGenerator(data)
...
func migrateMarkdownGenerator(data []byte) []byte {
	s := string(data)
	s = strings.ReplaceAll(s, "GENERATED by tools/lightgap_scorecard.py", "GENERATED by fak score lightgap")
	s = strings.ReplaceAll(s, "python tools/lightgap_scorecard.py", "fak score lightgap")
	return []byte(s)
}
