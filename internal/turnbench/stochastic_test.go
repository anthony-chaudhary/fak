...
func TestBreakEven_WriteArtifact(t *testing.T) {
	if raceDetectorEnabled {
		// The full-trial sweep is a CPU-bound artifact producer, not a concurrency
		// witness: under instrumentation it ran past the race job's per-package
		// timeout and was killed. TestBreakEven_Determinism still covers the sweep
		// at 64 trials, and the artifact is regenerated by the un-instrumented run.
		t.Skip("full-trial break-even sweep is not meaningful under go test -race instrumentation")
	}
	base, cm := BaseTrace(), DefaultCostModel()
	rep := RunBreakEvenSweep(context.Background(), base, DefaultHitRateGrid(), nil, cm, stochasticTrials, stochasticSeed)
	if rep.AppVersion == "" {
