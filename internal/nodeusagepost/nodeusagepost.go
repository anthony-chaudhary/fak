...
// scoreboard fallback (below); the channel resolver does not — a node-usage post must
// go to the node-usage channel, never silently to #scoreboard.
var (
...
func ResolveChannel() string {
	...
	return envFileValue("FAK_NODE_USAGE_CHANNEL")
}
...
// configures every workspace. (scoreboard.envFileValue is unexported, so the six-line
// walk-up is repeated here rather than exported — it is file I/O, not logic worth
// coupling.)
