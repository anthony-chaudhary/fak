...
func TestAuditRepositoryDiscoversNativeCoverageDebt(t *testing.T) {
	...
	if report.Coverage.CoveredLeaves != 25 {
		t.Fatalf("covered leaves=%d, want thirteen capability leaves plus twelve benchmark-governance infrastructure leaves", report.Coverage.CoveredLeaves)
	}
	if report.Coverage.ClassifiedLeaves != 25 || report.Coverage.UnclassifiedLeaves == 0 {
