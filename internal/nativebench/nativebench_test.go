...
func TestAuditRepositoryDiscoversNativeCoverageDebt(t *testing.T) {
	...
	if report.Coverage.CoveredLeaves != 33 {
		t.Fatalf("covered leaves=%d, want twenty-one capability leaves plus twelve benchmark-governance infrastructure leaves", report.Coverage.CoveredLeaves)
	}
	if report.Coverage.ClassifiedLeaves != 33 || report.Coverage.UnclassifiedLeaves == 0 {
