...
// Triggers WARN if turn1Tokens > 25000 or (baselineTokens > 0 && float64(turn1Tokens) > 2.0*float64(baselineTokens)).
func CheckPromptTokenAlarm(turn1Tokens int, baselineTokens int) TelemetryAlarm {
	triggered := turn1Tokens > 25000 || (baselineTokens > 0 && float64(turn1Tokens) > 2.0*float64(baselineTokens))
	if triggered {
		var detail string
		if turn1Tokens > 25000 && (baselineTokens > 0 && float64(turn1Tokens) > 2.0*float64(baselineTokens)) {
			detail = fmt.Sprintf("turn1 tokens (%d) exceeds 25,000 and 2.0x baseline (%d)", turn1Tokens, baselineTokens)
		} else if turn1Tokens > 25000 {
			detail = fmt.Sprintf("turn1 tokens (%d) exceeds hard cap of 25,000", turn1Tokens)
		} else {
			detail = fmt.Sprintf("turn1 tokens (%d) exceeds 2.0x baseline (%d)", turn1Tokens, baselineTokens)
		}
	...
	return TelemetryAlarm{
		...
		Detail:    fmt.Sprintf("turn1: %d, baseline: %d", turn1Tokens, baselineTokens),
	}
}

// CheckLatencyAlarm checks for turn latency spikes.
// Triggers WARN if currentLatencySec > 15.0 or (medianLatencySec > 0 && currentLatencySec > 2.5*medianLatencySec).
func CheckLatencyAlarm(currentLatencySec float64, medianLatencySec float64, consecutiveSpikes int) TelemetryAlarm {
	triggered := currentLatencySec > 15.0 || (medianLatencySec > 0 && currentLatencySec > 2.5*medianLatencySec)
	if triggered {
		var detail string
		if currentLatencySec > 15.0 && (medianLatencySec > 0 && currentLatencySec > 2.5*medianLatencySec) {
			detail = fmt.Sprintf("current latency %.2fs exceeds 15.0s cap and 2.5x median (%.2fs), consecutive spikes: %d", currentLatencySec, medianLatencySec, consecutiveSpikes)
		} else if currentLatencySec > 15.0 {
			detail = fmt.Sprintf("current latency %.2fs exceeds hard cap of 15.0s, consecutive spikes: %d", currentLatencySec, consecutiveSpikes)
		} else {
			detail = fmt.Sprintf("current latency %.2fs exceeds 2.5x median (%.2fs), consecutive spikes: %d", currentLatencySec, medianLatencySec, consecutiveSpikes)
		}
		return TelemetryAlarm{
...
const maxDBBytes = 5 * 1024 * 1024 * 1024 // 5 GB

// CheckDatabaseBloatAlarm checks for database size bloat or excessive freelist fragmentation.
// Triggers WARN if dbBytes > 5 * 1024 * 1024 * 1024 (5 GB) or (pageCount > 0 && float64(freelistPages)/float64(pageCount) > 0.50).
func CheckDatabaseBloatAlarm(dbBytes int64, freelistPages int64, pageCount int64, pageSize int64) TelemetryAlarm {
	...
	triggered := dbBytes > maxDBBytes || (pageCount > 0 && freelistRatio > 0.50)
	if triggered {
		var detail string
		if dbBytes > maxDBBytes && (pageCount > 0 && freelistRatio > 0.50) {
			detail = fmt.Sprintf("db size (%d bytes) exceeds 5GB and freelist ratio (%.2f%%) exceeds 50%%", dbBytes, freelistRatio*100)
		} else if dbBytes > maxDBBytes {
			detail = fmt.Sprintf("db size (%d bytes) exceeds 5GB threshold", dbBytes)
		} else {
			detail = fmt.Sprintf("freelist ratio (%.2f%%, %d/%d pages) exceeds 50%% threshold", freelistRatio*100, freelistPages, pageCount)
		}
		return TelemetryAlarm{
...
// Offset 16-17: page size (if 1 => 65536, else uint16 big endian).
// Offset 36-39: freelist page count (uint32 big endian).
...
func EvaluateTelemetryHealth(promptTokens, baselinePrompt int, latencies []float64, dbPath string) TelemetryHealthReport {
	...
	if len(latencies) > 0 {
		...
		for i := len(latencies) - 1; i >= 0; i-- {
			lat := latencies[i]
			isSpike := lat > 15.0 || (median > 0 && lat > 2.5*median)
			if isSpike {
				consecutiveSpikes++
			} else {
				break
