...
// ScorecardMarkdownDoc is the committed-snapshot front matter + prose for
// `fak milestone-scorecard --markdown`, kept beside the KPIs (the shape
// internal/skilleffectiveness.MarkdownDoc established) so the published page and the
// card can never drift: the header numbers are read off the SAME payload the --json
// surface emits, never re-derived. Pair it with pkg/scorecard.Markdown to render the
// body; the cmd shell only chooses the surface.
func ScorecardMarkdownDoc(p scorecard.Payload) scorecard.MarkdownDoc {
	return scorecard.MarkdownDoc{
		Title: "fak milestone scorecard - how far is the climb from MATURED and the roadmap from done",
		Description: "fak's deterministic milestone scorecard: the model x backend maturity CLIMB's " +
			"distance from the MATURED (M4) floor plus the tracked epics' open ROADMAP children, " +
			"folded into one retire-able milestone_debt integer with a worst-first work list.",
		Heading: "Milestone scorecard",
		AutoGen: "Auto-generated by `fak milestone-scorecard --markdown`. Do not hand-edit; re-run the tool.",
		Law: "The law: a milestone is only climbed when a CI-runnable oracle witnesses it. Every cell " +
			"below the MATURED floor owes its remaining rung-steps and every measured discrete epic owes " +
			"its open children, so the headline integer is a distance to finish -- never a self-report.",
		DebtKey: DebtKey,
		HeaderExtra: fmt.Sprintf(" - %v/%v cell(s) matured (%v) - %v climb rung(s) - %v open roadmap child(ren) over %v discrete epic(s)",
			p.Corpus["matured"], p.Corpus["cells"], p.Corpus["matured_floor"],
			p.Corpus["climb_debt"], p.Corpus["roadmap_debt"], p.Corpus["discrete_epics"]),
	}
}
