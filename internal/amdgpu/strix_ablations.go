...
func validateAblationSelectors(selected []string) (int, error) {
	if len(selected) == 0 {
		return len(strixAblationCatalog), nil
	}
	sel := map[string]bool{}
	...
	if len(unknown) > 0 {
		return 0, fmt.Errorf("amdgpu: unknown ablation selector(s): %s", strings.Join(unknown, ", "))
	}
	count := 0
	for _, a := range strixAblationCatalog {
	...
	if count == 0 {
		return 0, fmt.Errorf("amdgpu: zero ablation arms selected")
	}
	return count, nil
}
...
func RunStrixAblations(ctx context.Context, target *StrixTarget, selected []string, gitTip ...string) ([]StrixAblationResult, error) {
	if _, err := validateAblationSelectors(selected); err != nil {
		return nil, err
	}
...
func executeStrixAblationCommand(ctx context.Context, target *StrixTarget, envVars, testPattern string) (string, time.Duration, error) {
	...
	if !ok || sb.WorkDir == "" || !sha256RE.MatchString(sb.BinarySHA256) || !sha256RE.MatchString(sb.ShaderBundleSHA256) {
		return "source binding mismatch: missing prepared candidate source/build binding", 0, fmt.Errorf("amdgpu: ablation refused without prepared candidate source/build binding")
	}
	envPrefix := ""
	if envVars != "" {
		envPrefix = envVars + " "
	}
	testCmd := fmt.Sprintf(`%sFAK_VULKAN_SPIRV=%s FAK_VULKAN_REQUIRE_DEVICE=1 FAK_VULKAN_EXPECT_DEVICE=8060S %s -test.run %s -test.v`, envPrefix, shellQuote(sb.WorkDir+"/build/spirv"), shellQuote(sb.WorkDir+"/build/compute.test"), shellQuote(testPattern))
...
func completeAblationEvidence(ctx context.Context, target *StrixTarget, out string, dur time.Duration, r StrixAblationResult) StrixAblationResult {
	sb, _ := SourceBindingFromContext(ctx)
	// The exact executed command is already bound into the raw output and artifact hashes;
	// use the raw transcript digest as the ablation command identity.
	r.Evidence = executionEvidenceFromOutput(out, sb)
	_ = dur
	return r
}
...
func extractAblationMetrics(out string, feature string, baselineAliases, candidateAliases []string) (ablationMetrics, error) {
	...
	lookupSamples := func(m map[string]any, keys ...string) (int, bool) {
		for _, k := range keys {
			if v, ok := m[k]; ok {
				switch x := v.(type) {
				case float64:
		...
		if strings.HasPrefix(trimmed, "{") || strings.Contains(trimmed, `{"`) {
			...
			if err := json.Unmarshal([]byte(trimmed[idx:]), &raw); err == nil {
				...
				if baseArm, ok := raw["baseline_arm"].(map[string]any); ok {
					...
					if n, ok := lookupSamples(baseArm, "samples", "sample_count"); ok {
						res.BaselineSamples = n
					}
				}
				if candArm, ok := raw["candidate_arm"].(map[string]any); ok {
					...
					if n, ok := lookupSamples(candArm, "samples", "sample_count"); ok {
						res.CandidateSamples = n
					}
				}
				...
				if n, ok := lookupSamples(raw, "baseline_samples", "baseline_sample_count"); ok {
					res.BaselineSamples = n
				}
				if n, ok := lookupSamples(raw, "candidate_samples", "candidate_sample_count"); ok {
					res.CandidateSamples = n
				}
			}
		} else {
	...
	if res.BaselineLatencyUS <= 0 || res.CandidateLatencyUS <= 0 || res.CosineParity <= 0 || res.BaselineSamples <= 0 || res.CandidateSamples <= 0 {
...
func runTargetAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "FAK_VULKAN_Q4K_PROFILE=1", "^TestVulkanQ4KRealShapeProfile$")
	...
	baselineSamples, candidateSamples, sampleErr := extractExplicitProfileSampleCounts(outStr)
	if parseErr != nil || sampleErr != nil || cpuNS <= 0 || gpuNS <= 0 || cosine <= 0 {
		return StrixAblationResult{
			Dimension: "target",
			Feature:   "cpu_vs_vulkan_gpu",
			Verdict:   "REGRESSION",
		}, fmt.Errorf("q4k profile metrics missing or invalid (cpuNS=%d, gpuNS=%d, cosine=%f): metrics=%v samples=%v", cpuNS, gpuNS, cosine, parseErr, sampleErr)
	}
	...
	return completeAblationEvidence(ctx, target, outStr, dur, StrixAblationResult{
		Dimension:   "target",
		Feature:     "cpu_vs_vulkan_gpu",
		BaselineArm: StrixArmResult{Name: "cpu_q4_reference", LatencyUS: cpuUS, Samples: baselineSamples},
...
func extractExplicitProfileSampleCounts(out string) (int, int, error) {
	...
	return 0, 0, fmt.Errorf("explicit baseline_samples and candidate_samples are required")
}
...
func runTopologyAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^(TestVulkanRMSNormMatMulApprox|TestVulkanRMSNormMatMulArgmaxMatchesVulkanChain)$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "topology",
	...
	return completeAblationEvidence(ctx, target, outStr, dur, StrixAblationResult{
		...
		BaselineArm: StrixArmResult{
			...
			LatencyUS: m.BaselineLatencyUS, Samples: m.BaselineSamples,
			AllocatedBytes: m.BaselineAllocBytes,
		},
		CandidateArm: StrixArmResult{
			Name:      "fused_rmsnorm_matmul",
			LatencyUS: m.CandidateLatencyUS, Samples: m.CandidateSamples,
...
func runQuantizationAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^(TestVulkanMatMulApprox|TestVulkanQ8MatMulApprox|TestVulkanQ4KMatMulMatchesCPUReference)$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "quantization",
...
func runQ2KvsQ4KAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^(TestVulkanQ4KMatMulMatchesCPUReference|TestVulkanQ2KMatMulMatchesCPUReference)$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "quantization",
...
func runResidencyAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^(TestVulkanResidencyRoundTrip|TestVulkanHostVisibleBufferDoesNotRecycleAsDeviceLocal)$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "residency",
...
func runContiguizeAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^(TestRADVContiguizeShader_ChannelEntropy|TestRADVContiguizeShader_Parity)$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "layout",
	...
	return completeAblationEvidence(ctx, target, outStr, dur, StrixAblationResult{
		...
		BaselineArm: StrixArmResult{
			...
			LatencyUS: m.BaselineLatencyUS, Samples: m.BaselineSamples,
			AllocatedBytes:  m.BaselineAllocBytes,
			DRAMBandwidthGB: m.BaselineBandwidth,
		},
		CandidateArm: StrixArmResult{
			Name:      "contiguized_f16_kv_scratch",
			LatencyUS: m.CandidateLatencyUS, Samples: m.CandidateSamples,
			AllocatedBytes:  m.CandidateAllocBytes,
			DRAMBandwidthGB: m.CandidateBandwidth,
		},
		Speedup:      speedup,
		LiftRatio:    speedup,
		CosineParity: m.CosineParity,
...
func runPrefillSequenceAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^TestVulkanQwen35Sequence")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "prefill",
	...
	m, mErr := extractAblationMetrics(outStr, "prefill_sequence_vs_serial", []string{"serial_prefill_us"}, []string{"sequence_prefill_us"})
	if mErr != nil {
		return StrixAblationResult{Dimension: "prefill", Feature: "prefill_sequence_vs_serial", Verdict: "REGRESSION"}, mErr
	}
	baselineUS, candidateUS := m.BaselineLatencyUS, m.CandidateLatencyUS
	...
	return completeAblationEvidence(ctx, target, outStr, dur, StrixAblationResult{
		...
		BaselineArm: StrixArmResult{
			...
			LatencyUS: baselineUS, Samples: m.BaselineSamples,
			AllocatedBytes: m.BaselineAllocBytes,
		},
		CandidateArm: StrixArmResult{
			Name:      "vulkan_sequence_prefill",
			LatencyUS: candidateUS, Samples: m.CandidateSamples,
			AllocatedBytes: m.CandidateAllocBytes,
		},
		Speedup:      speedup,
		LiftRatio:    speedup,
...
func runDecodeResidentAblation(ctx context.Context, target *StrixTarget) (StrixAblationResult, error) {
	outStr, dur, err := executeStrixAblationCommandFn(ctx, target, "", "^TestVulkanQwen35GDNDecodeMatchesCPUOracleInPlace$")
	if err != nil || !strings.Contains(outStr, "PASS") {
		return StrixAblationResult{
			Dimension: "decode",
	...
	m, mErr := extractAblationMetrics(outStr, "decode_resident_vs_host_fallback", []string{"host_fallback_decode_us"}, []string{"resident_decode_us"})
	if mErr != nil {
		return StrixAblationResult{Dimension: "decode", Feature: "decode_resident_vs_host_fallback", Verdict: "REGRESSION"}, mErr
	}
	baselineUS, candidateUS := m.BaselineLatencyUS, m.CandidateLatencyUS
	speedup := float64(baselineUS) / float64(candidateUS)
	verdict := "VERIFIED_LIFT"
