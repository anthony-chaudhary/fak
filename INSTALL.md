...
```sh
fak version          # prints the installed version, e.g. 0.44.0
```
...
| --- | --- | --- |
| `FAK_VERSION` | Pin a version, e.g. `0.44.0` | latest release |
...
```sh
FAK_VERSION=0.44.0 FAK_INSTALL_DIR="$HOME/.local/bin" \
...
```sh
docker build --build-arg APP_VERSION=0.44.0 -t fak:0.44.0 .
```
