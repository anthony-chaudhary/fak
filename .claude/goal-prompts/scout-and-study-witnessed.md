...
1. **CRAWL one signal.** Read what the crawlers already surfaced — do not re-scan
   arXiv yourself:
...
3. **STUDY via /study-repo.** Drive `.claude/skills/study-repo/SKILL.md` — do not
   ...
   README pitch, extract candidate borrows each grounded at a real `path:line@sha`,
   decide inspire-vs-integrate on the LICENSE.

4. **WITNESS each borrow, then FILE small.** For each candidate run
   `/field-borrow`'s witness step — dogfood `fak_feature_query` / `fak index`
   (+ a raw Grep to guard false-ABSENT) → PRESENT / PARTIAL / ABSENT.
   - PRESENT → fak already has it: DROP it, record the card. A witnessed "already
     had this" is a good result, not a miss.
   - PARTIAL / ABSENT → ground it at a fak seam (`path:line` in fak), dedup
     (`gh issue list --search`), and file a SMALL independently-shippable leaf under
     the right epic, carrying BOTH anchors (source `path:line@sha` + fak seam), the
     dogfood witness, and a first checkable step. NEVER a "adopt everything" monolith.
...
5. **REGISTER the trail.** Write a dated `docs/notes/CONCEPT-STUDY-<repo>-<date>.md`
   ...
   `git add -A`; no `Co-Authored-By`. Confirm `fak index freshness` is clean for it.

6. **STOP.** One studied, witnessed, registered lead is a complete pass. Do not spin
   to a second lead — the cadence is the throughput.

## Hard boundaries (enforced below you)

- A crawl/study is NOT a ship. You file backlog; you NEVER `gh issue close` and never
  report an issue resolved. Ancestry (`Fixes #N` on the trunk) does that, later, by a
  different worker.
...
Report faithfully: the lead studied (+ `@sha`), the leaves filed (# each, with
PRESENT dropped), the registration note, or `empty pass` + why nothing was studied.
