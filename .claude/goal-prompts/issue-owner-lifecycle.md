...
3. **Start root work.** Reproduce the gap and implement the smallest real end-to-end root spine. `BOUNDED` owners do not launch children; they begin root edits and carry implementation and witness directly. Planning, audits, and child launches never substitute for owner implementation.
...
5. **Keep working through refusals.** A refused child launch or lease reduces concurrency; it does not erase the issue. If a child launch is refused, the next action is root implementation, not another launch mechanism. Continue every safe root-spine step yourself. Never bypass a typed refusal.
...
8. **Close exhaustively.** Account for every child, process, exact-tree lease, ticket intent, and effect. Orphan child closeout remains mandatory: stop and account for any child that outlives its parent before exit. Stop children and release claims/leases before exit. File follow-ups before naming them.
