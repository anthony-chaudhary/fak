...
- Apply the native-tuning default in `AGENTS.md`: record candidate source/configuration/workload identity and the full configuration delta in the plan. Refresh incompatible reference measurements when objective or load changes; reassess compatible history for SLO-only changes. Justify inseparable bundles and credit only the bundle. Record independent validity/budget review before an expensive run, bound to the exact candidate, objective, load, SLO, and planned cost. Candidate changes reopen only affected review and witnesses; existing user authorization, budget, and security limits remain in force.
