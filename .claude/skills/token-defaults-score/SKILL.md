...
5. **Commit only the scorecard lane, by explicit path** — run `fak sync reconcile --apply`, then commit via `fak commit --path` (never `git add -A`). End the
