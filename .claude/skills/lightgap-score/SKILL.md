...
python tools/lightgap_scorecard.py --unrun         # the comparisons nobody has run
python tools/lightgap_scorecard.py --check         # honesty gate; exit 1 on lightgap_debt
python tools/lightgap_scorecard.py --segment local-first    # one buyer, in full
python tools/lightgap_scorecard.py --facet raw-speed        # one axis, across buyers
python tools/lightgap_scorecard.py --ceilings      # every anchor and its derivation
...
```bash
python tools/lightgap_scorecard.py --json > /tmp/lightgap-after.json
python -m pytest tools/lightgap_scorecard_test.py -q
...
git commit -s -F <msgfile> -- \
  tools/lightgap_scorecard.py tools/lightgap_scorecard_test.py \
  tools/lightgap_scorecard.data docs/lightgap-scorecard \
  .claude/skills/lightgap-score
```
