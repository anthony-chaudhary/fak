...
4. **Atomic S0/S1 Steps (Subdivide or Abstain)**: Decompose work into single-concern leaves (1–3 files touched per step). Keep exactly one step `in_progress` in `todowrite`. Fail-to-abstain on high-difficulty boundaries (e.g. frozen ABI, kernel SIMD).
5. **Failure Memory Scratchpad**: Every error or guard refusal is recorded in `# Scratch / last-refusal` in `GOAL.md`. The agent pivots strategy immediately and never loops on repeated identical failing tool calls.
...
3. If a command or tool fails:
   - Log the failure and exit code to `# Scratch / last-refusal` in `GOAL.md`.
   - Halt repeat attempts. Read the error, change tack, or pivot strategy.
...
- **Exit code != 0**: Append the failure to `# Scratch / last-refusal`, formulate a targeted fix, and re-run.
