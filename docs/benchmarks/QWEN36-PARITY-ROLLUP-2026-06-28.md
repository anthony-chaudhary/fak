...
| 3 | **#71 on-device prefill tok/s** (pp22, clean) | prefill **0.6** @P=29 / **4.5** @P=421 (q4_k Metal, #70); bar **51.55** @pp22 | M3 Pro, `-tags fakmetal`, **no co-resident llama-server** (36 GiB swap rule) | `FAK_QPROFILE=1 fakchat -gguf Qwen3.6-27B.q4_k_m.gguf -tok <dir> ...` → record `[metalprof-hybrid …]` |
...
| 5 | **#70 q4_k device GEMM matmul-only split** | code shipped; whole-path prefill 4.5 tok/s @P=421 (~11× under 51.55) | M3 Pro, `-tags fakmetal`, no co-resident llama-server | `go test ./internal/model -tags fakmetal -run MetalQ4K -count=1`; then `FAK_QPROFILE=1` pp22 prefill |
