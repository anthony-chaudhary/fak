...
| [SERVED-INLINE-DEDUP-RESULTS.md](SERVED-INLINE-DEDUP-RESULTS.md) | results | WITNESSED replay measuring how often the vDSO served-inline fast path (`--vdso-proxy-fill`) actually fires on a named agent read pattern, decomposed by win class (#1350). | WITNESSED: 5/14 (35.7%) read-only proposals served inline on read-only-shaped names (all cross-turn re-reads); 0/14 on Claude-native `Read`/`Grep` names — the read-only NAME gate does not recognize them, so keep `--vdso-proxy-fill` opt-in |
...
| [FANRUN-50-SUBAGENTS](../_witnesses/issue-10855-subagent-50-fanrun/README.md) | results | 50+ sub-agent fan-out scaling under strict resource bounds, measuring prefix token elision and vDSO dedup (#10855). | MEASURED: 8,056.5 agents/sec at N=50 with 100,352 prefix tokens elided and 147 vDSO dedup hits (linear scaling) |
...
| [FULLSPAN-TAILLOAD-RESULTS.md](FULLSPAN-TAILLOAD-RESULTS.md) | results | One process, all four clock bands in a single causal trace with deny-consequence classes, plus the µs decide fold's tail quiet vs under same-process load. | OBSERVED (research-grade): same-process load leaves decide p50 nearly flat but degrades p99 ×2.1–2.5 (13.5–14 µs → 30–34 µs) and triples the >100 µs share; trace walks B6→B4→B2→B0 with every DENY carrying retry_turn/forked_outcome/clean_stop |
...
| [QWEN36-LOAD-PROFILE-440.md](../notes/QWEN36-LOAD-PROFILE-440.md) | results | Phase-attributed load profile of the Qwen3.6-27B GGUF->Q8 path, with a same-host arena-reuse before/after. | Witnesses the #440 page-churn fix at -4.0% peak RSS / -4.0% page faults (measured 2026-06-26) |
...
| [QWEN36-PARITY-ROLLUP-2026-06-28.md](../notes/QWEN36-PARITY-ROLLUP-2026-06-28.md) | rollup | One authoritative reconciliation of the Qwen3.6-27B-vs-llama.cpp parity status: PROVEN vs `not yet`, with gated repros | Correctness parity PROVEN at architecture level but REFUTED at 27B scale (token-3 argmax flip); speed parity `not yet` — 1.2 tok/s vs the 7.29 tok/s llama.cpp-Metal bar (recorded prior Mac witnesses, none re-measured here) |
| [MAC-THREEWAY-BENCH-2026-09-03.md](../notes/MAC-THREEWAY-BENCH-2026-09-03.md) | simulated fixture | Deterministic validator-fixture preview of a planned Apple M3 Pro Qwen3.8-27B three-way comparison; [#2723 comments](https://github.com/anthony-chaudhary/fak/issues/2723) record the HOLD and missing runtime evidence. | `[SIMULATED]`: 7.61 / 7.38 / 8.07 decode tok/s and the 12.60 ms shared-prefix TTFT are fixture values, not on-device measurements; no raw matched-envelope packet is committed. |
| [QWEN-PERFORMANCE-INDEX.md](QWEN-PERFORMANCE-INDEX.md) | index | Generated current Qwen front-door readout plus retained envelope-specific witnesses. | Separates accepted Metal evidence, an approximate near-match, and a failed-quality CUDA diagnostic. |
| [QWEN38-27B-LATEST.md](QWEN38-27B-LATEST.md) | results | Detailed Qwen3.8-27B accepted, approximate, and diagnostic result lifecycle. | Current values are generated from the native-performance graph and reaped after their review dates. |
...
| [STRIX-HALO-BENCHMARK-RESULTS.md](STRIX-HALO-BENCHMARK-RESULTS.md) | results | Authoritative physical validation baseline & candidate index on AMD Strix Halo (Ryzen AI MAX+ 395 / Radeon 8060S gfx1151). | 19/19 sub-kernels PASS; 5/5 ablations VERIFIED_LIFT (167.5x GPU Q4_K speedup, 2.69x f16 KV contiguization, 1.63x fused norm-matmul) |
| [LOCAL-HARDWARE-SETUP.md](LOCAL-HARDWARE-SETUP.md) | guide | Popular local hardware setup guide for benchmarks across Apple Silicon, NVIDIA RTX, AMD Radeon, and CPU systems. | Setup prerequisites, execution workflow, and privacy expectations for fak bench local |
...
| [VLLM-EP-EPLB-MOE-BASELINE-RUNBOOK.md](VLLM-EP-EPLB-MOE-BASELINE-RUNBOOK.md) | runbook | The reproducible vLLM Expert-Parallel + EPLB MoE serve command and artifact schema — the SOTA serving floor native fak EP-sharded GGUF work must cite (#1733). | Status: pending measurement. Carries the EP/EPLB serve command, TTFT/TPOT/ITL + before/after expert-balance capture, and the `fak.vllm-ep-eplb-moe-baseline.v1` schema — no number until a real EP node runs it. |
...
| [RADIXATTENTION-RESULTS.md](RADIXATTENTION-RESULTS.md) | results | Head-to-head of fak's KV-cache reuse against SGLang's RadixAttention on cache hit rate, with a 4.58x live speedup. | 4.58× live wall-clock speedup on SmolLM2-135M Q8 (agents workload); WITNESSED kernel-owned 77–88% cache hit rate inside SGLang's verified 50–99% band (MEASURED) |
| [speculative-hardware-simulation-methodology.md](speculative-hardware-simulation-methodology.md) | doctrine | Disciplined 5-stage evaluation funnel for speculative decoding and token prediction cutting hardware evaluation spend by >90%. | 5-stage simulation-first funnel decoupling acceptance rate from hardware latency, with Pareto frontier filtering (#10845) |
| [VLLM-HEADTOHEAD-RESULTS.md](VLLM-HEADTOHEAD-RESULTS.md) | pending | A gateway adjudication-tax witness and vLLM engine bench; pending-measurement — every vLLM cell is a placeholder. | Status: pending-measurement (GATED scaffold). No vLLM GPU run has landed yet — every numeric cell in the vLLM tables is a placeholder marked `not yet`; the only real vLLM-comparison numbers are the measured SGLang sibling's (0.75× at peak, ~3% tax at saturation). |
...
| [LOCAL-MODEL-CODING-WITNESS-2026-06-27.md](../notes/LOCAL-MODEL-CODING-WITNESS-2026-06-27.md) | results | Measured results of a small local model behind fak manage on a minimal coding task, with an honest local-vs-frontier A/B. | MEASURED — the capability axis was run live: both 3B and 7B Qwen2.5-Coder rungs produced the correct fix (1-fail → 2-pass); governance witnessed via replay-trace (8 adjudicated verdicts, 3 dangerous calls denied) |
| [LOCAL-MODEL-CODING-WITNESS-RUNBOOK.md](LOCAL-MODEL-CODING-WITNESS-RUNBOOK.md) | runbook | End-to-end command sequence to measure a small local model behind fak manage on a minimal coding task, with A/B. | Status: the path is ASSEMBLED; the measured numbers are PENDING a real run. |
...
| [TERMINAL-BENCH-4-REPRODUCTION.md](TERMINAL-BENCH-4-REPRODUCTION.md) | runbook | End-to-end benchmark manual comparing fak native harness against OpenCode baseline on Terminal-Bench 4. | Status: harness & test suite shipped; live evaluation results gated under BENCHMARK-AUTHORITY.md |
| [NATIVE-IMPLEMENTATION-COMPARISONS.md](NATIVE-IMPLEMENTATION-COMPARISONS.md) | contract | Machine-readable spine comparing native implementations against strongest practical alternatives. | Declares shared-workload contracts across native capabilities with tuned baseline & next-best comparisons (#10720) |
