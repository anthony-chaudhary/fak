...
> Q4_K_M where noted). llama.cpp = `llama-cpp-python` 0.3.30 CPU wheel (what
> `pip install llama-cpp-python` gives). All fak benchmarks run via WSL on Windows (native exec is WDAC-blocked here).
>
