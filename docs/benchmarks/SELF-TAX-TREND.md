...
| epic #607 (Regime B, #623) | cross-agent bare `claude` vs `fak manage -- claude` (pong, tool-light) | **total-ingested 1.56× = +28,986 tok** (5/5 ALLOW, 0 deny) | `+` (net cost) | WITNESSED (kernel counters) | `experiments/ablate/cross-agent-pong-opus.json` (doc: `docs/benchmarks/ABLATE-RESULTS.md`) | `python tools/cross_agent_ablate.py report --reps <reps.json>` |
