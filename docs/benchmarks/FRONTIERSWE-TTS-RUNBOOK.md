...
| Collect | `frontierswe` is a `LevelE2E` bench-catalog row (`internal/benchcatalog`); `fak nightrun` ranks its zero-asset describe/TTS-projection arm (novelty × value × staleness) and auto-collects it to `docs/nightrun/collected.jsonl` — the projected-TTS floor the live run is later checked against. The live arm stays this runbook's gated recipe, never auto-run. | RUNNABLE NOW (offline; a real `bench-frontierswe` `collected` row is banked; `tools/bench_dx_scorecard.py` green — the registry covers the tree) | `fak nightrun` / `internal/benchcatalog` (C18, [#1724](https://github.com/anthony-chaudhary/fak/issues/1724)) |
