...
| [Official MCP Registry](https://github.com/modelcontextprotocol/registry) | The canonical MCP registry | Wired via [`server.json`](https://github.com/anthony-chaudhary/fak/blob/main/server.json) (@0.37.0) + ghcr image; one interactive `mcp-publisher publish` step is owner-only | not yet (owner publish) |
