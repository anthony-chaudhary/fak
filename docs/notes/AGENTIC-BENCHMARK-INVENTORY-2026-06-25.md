...
| **Terminal-Bench** | Terminal-Bench 2.0 is a hard CLI benchmark with 89 tasks, unique environments, human-written solutions, and comprehensive tests; Terminal-Bench 3.0 and Science are in development. | Measures the developer-terminal surface fak already gates. | fak can win on "same resolve, safer terminal" by enforcing command/file/network policy, redacting secrets, preserving evidence, and reducing repeated setup. Raw pass@1 still belongs mostly to the base model. | Harbor/Terminal-Bench runner using `fak manage` or `fak serve`, with per-command verdict and test outcome logs. |
