---
title: "GLM-5.2 L7 true DSA sparsity (#3088): triage — the sparse arm is un-witnessable on sm_80; the honest witness is the full-MLA ctx curve (2026-07-06)"
...
> [#3088](https://github.com/anthony-chaudhary/fak/issues/3088) — the **Lane E / L7**
...
> full-MLA ctx-scaling-curve protocol that produces the honest WITNESSED artifact — the
> one that decides whether L7 is worth kernel work at all.
>
> **What this is NOT.** Not the benchmark, and **not a sparse-vs-dense A/B** — no sparse
> arm can run on this iron today (§2). The only served measurement referenced is the
...
[ceiling map](GLM52-DGX-THEORETICAL-CEILING-2026-07-06.md) §5). The issue: llama.cpp runs
...
tolerance. **Accept:** a ctx→{sparse vs full-MLA tok/s} table with the parity check;
...
(ceiling doc, 2026-07-06 correction — "the two nodes are NOT identical"). L7 is a
**GPU server 3 · Lane E** lever, per both the corrected ceiling §6 and the
[costed lever map](GLM52-L1-L10-COSTED-LEVER-MAP-2026-07-06.md) §3. Any witness run lands
on server 3.

## 2. Why the sparse arm is un-witnessable on this hardware today

The issue's Assumption 1 — *"a DSA indexer sparse-path implementation (even WIP upstream)
exists to port or enable"* — is **false for sm_80**, on three grounds:

1. **llama.cpp serves `glm_moe_dsa` as full MLA today.** Its sparse indexer is WIP
   upstream — and *implementing* it is explicitly excluded by the issue's own Out-of-scope
   clause ("not upstream llama.cpp's own DSA indexer implementation"). So there is nothing
   in-scope to "enable."
2. **The existing fused sparse-DSA kernels (DeepSeek lineage) have an sm_90 floor** — the
   ...
   change (lever map §5 L7).
3. **The pure-fak DSA path is out-of-lane for this issue.** fak's own DSA index-selection /
   sparse-attention kernels exist
   ([index selection](GLM52-DSA-INDEX-SELECTION-ON-PURE-KERNEL-2026-06-23.md) ·
   [sparse attention](GLM52-DSA-SPARSE-ATTENTION-ON-PURE-KERNEL-2026-06-23.md) ·
...
question ("what is the DSA output-parity tolerance?") stays **unknown — needs operator
input**, and is moot until a sparse arm exists.

## 3. Generation classification — `gen/second-next`

Classified per [docs/generation.md](../generation.md); label `gen/second-next` + milestone
*Generation G2 — Second Next Gen* applied on the issue (this record is the basis).
...
- **Why not `gen/now`:** the `gen/now` test requires "no dependency on a future
  ...
  (§2) — marking it now-horizon would be the *current-work laundering* anti-pattern.
...
- **Why `gen/second-next`:** it is an architectural option that needs **sizing** (the §4
  curve) and a feasibility decision before it can become active product work — the G2
  definition verbatim.
- **Re-promotion triggers (closed set):**
  1. upstream llama.cpp's sparse indexer lands with **sm_80-capable** kernels → re-test as
     a rebuild + config lever (`gen/now`);
  2. the measured §4 curve shows a **≥1.3× cliff at 32k** → the prize justifies scoping
     sm_80-feasible indexer kernel work (`gen/next`);
  3. the lab gains **sm_90+** iron → the fused-kernel path becomes a config/port lever.
- **Demotion/retirement:** a **flat curve to 32k** ⇒ retire L7 as *measured-no-prize* on
  this hardware generation (the curve is the retirement evidence, not a guess).

## 4. The honest witness — the full-MLA ctx-scaling curve (sizes the prize)
...
{2k, 8k, 32k} on the resident serve. Flat ⇒ deprioritize; a cliff at depth ⇒ quantifies the
DSA headroom. Run **downstream of L1 (#3075)** at a **fixed FA setting** (record which),
node **GPU server 3 · Lane E**. Tools verified present in-tree:
`tools/glm52_mgpu_serve.sh` honors `CTX`/`PORT`/`DEVICES` (→ `--ctx-size`);
...
|--:|--:|--:|--:|
| 2,048 | PENDING | PENDING | 1.00× (def) |
| 8,192 | PENDING | PENDING | PENDING (ESTIMATED ~0.9–1.0×, lever map) |
| 32,768 | PENDING | PENDING | PENDING — **this cell is the decision** |
...
Headline to report on the issue: **the curve shape** (the prize L7 would capture) — not a
sparse-vs-dense speedup, which no run on this iron can honestly produce (§2). The curve
...
([docs/private-comms-channel.md](../private-comms-channel.md)) — no bridge session is
...
rule, so the §4 cells stay **PENDING**. Same gate as every sibling in the #3073 cohort.

**Smallest next step.** A GPU server-3 operator (post-L1, fixed FA) runs the §4 loop and
...
runnable (re-promotion triggers, §3) and the A/B landing, or (b) an explicit re-scope of
the issue to the curve-only witness — **not this triage note**.
...
The #3073 lever cohort shares this shape — one lever, one WITNESSED
`experiments/benchmark/runs` artifact, triage note first when the witness is
operator-gated: [#3076 L4](GLM52-DGX3-LANEA-L4-FA-CUDAGRAPH-TRIAGE-2026-07-06.md) ·
[#3077 L5](GLM52-L5-QUANT-SWEEP-TRIAGE-2026-07-06.md) ·
[#3079 L2](GLM52-DGX2-LANEB-L2-CONTBATCH-TRIAGE-2026-07-06.md) ·
[#3080 KV budget](GLM52-DGX2-LANEB-KV-BUDGET-TRIAGE-2026-07-06.md). L7 differs from all of
...
lever map §3) — measure it downstream of everything cheaper.

*Companions:* [ceiling + lever map](GLM52-DGX-THEORETICAL-CEILING-2026-07-06.md) ·
[costed lever map (L7 §5)](GLM52-L1-L10-COSTED-LEVER-MAP-2026-07-06.md) ·
