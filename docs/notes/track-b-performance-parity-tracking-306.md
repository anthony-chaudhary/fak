...
|---|---|---|---|
| **B-001** #289 prefill | 🟡 **Scaffold shipped; perf CUDA-gated** | `PrefillCostModel` (exact FLOP/byte roofline, surfaces the O(P²) attention term), `PrefillGEMM` **bit-exact** to `Backend.BatchedMatMul`, `PrefillGraphCapturer` HAL seam (CUDA-satisfied under `-tags cuda`). CPU suite green. | [`internal/compute/prefill.go`](../../internal/compute/prefill.go) · notes [`PREFILL-B001-NOTES.md`](../../internal/compute/PREFILL-B001-NOTES.md) |
| **B-002** #287 Vulkan | 🟡 **Real backend; numerical parity; throughput NOT parity** | Real Windows Vulkan backend (`-tags vulkan`) reaching **argmax-exact greedy decode, prefill cosine 1.0** on a real AMD **RX 7600** (SmolLM2-135M). Still **~60× slower** than llama.cpp CPU; named, fixable cause = **per-dispatch CPU/driver overhead** (not compute efficiency). | [`internal/compute/vulkan.go`](../../internal/compute/vulkan.go) · witness [`docs/benchmarks/VULKAN-AMD-RESULTS.md`](../benchmarks/VULKAN-AMD-RESULTS.md) |
...
| **B-004** #282 continuous batching | 🟡 **Core kernel shipped; production scheduler is a stub** | Multi-user batched decode `StepBatch` + ragged `StepBatchActive` (idle-lane skip), **bit-identical to serial** (`TestBatchedDecodeMatchesSerial`); work eliminated ∝ idle lanes. `NativeScheduler` is an explicit **"shape proof, not production"** — no paged KV, no preemption, no admission/fairness. | [`internal/model/batch.go`](../../internal/model/batch.go) · [`internal/modelengine/nativesched.go`](../../internal/modelengine/nativesched.go) · commit `76b931a` |
| **B-005** #279 fused kernels | 🟡 **Built + GPU-gated** | Fused **flash / online-softmax attention** CUDA kernel (MHA/GQA/MQA), correctness-gated cosine ≥ 0.999; Vulkan FFN-tail + Q8 fused-decode kernels. Correctness witnessed; **throughput gain is a GPU measurement** (`tools/run_486_acceptance_on_gpu.sh`). | [`internal/compute/cuda_flash_test.go`](../../internal/compute/cuda_flash_test.go) · `cuda_kernels.cu` · commit `49d445b` |
...
| **B-007** #275 INT4/INT2 | 🟡 **INT4 shipped; INT2 absent** | **Q4_K** full load+run path (CPU ref `q4kRowDot`, CUDA `k_q4k_gemm`, Metal fused dequant) + **Q4_0** load/stream; Q4_K device GEMM stays int4-resident (`#485`). **INT2 / Q2 is not present** — not in the `Dtype` enum, no forward kernel (Q2_K can *load* but has no compute path). | [`internal/compute/quant_q4k.go`](../../internal/compute/quant_q4k.go) · [`internal/ggufload/gguf_dequant.go`](../../internal/ggufload/gguf_dequant.go) · commits `e1513da`/`a8cb3fc` |
| **B-008** #272 dynamic batching | 🟡 **Ragged work-elimination shipped; full dyn scheduler deferred** | `StepBatchActive` compacts active lanes into a sub-panel (padding/idle work removed, measured by `LastStepMACs()`); admission lives only in the `NativeScheduler` stub. No dynamic batch-size policy, no fairness/priority, no p99 SLA admission. | [`internal/model/batch.go`](../../internal/model/batch.go) · commit `76b931a` |
...
3. **Production-scheduler children (B-004, B-008).** The batching *kernel* ships and is
   bit-exact, but the acceptances ("compatible with vLLM continuous batching, throughput
   within 10% of direct vLLM"; "near-linear scaling 1.8× on 2× requests, p99 within 1.5×")
   require the production scheduler that `nativesched.go` is explicitly **not** (paged KV,
   preemption, fairness, admission) — tracked by sibling serving issues #28/#31/#34/#35/#36.
