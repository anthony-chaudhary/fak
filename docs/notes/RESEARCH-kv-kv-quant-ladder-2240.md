...
the ladder (#1474 satisfied through this) · R3 bench curves vs published TRT-LLM/LMDeploy
points.
...
3. **R3b** — CacheGen-class bitstream codec behind StageSpan/RestoreSpan for `TierDisk`
   (§c.5). CPU-witnessable; ships with round-trip + ratio + drift tests.
4. **R3a** — fp8 KV on the CUDA path (§c.4). Blocked on a CUDA host + device attention
   kernels; design seam named, nothing more claimed.
5. **R3 bench** — capacity-vs-accuracy curves (tokens resident per GB per precision) as a
