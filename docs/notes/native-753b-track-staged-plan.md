...
| P1 E2E load (loader half ✅) | a complete `glm_moe_dsa` GGUF resolves its FULL tensor set + splits experts on load — **shipped `2e632ef`** (`TestGLMMoeDsaGGUFFullFixtureLoads`: 25 GGUF tensors → 31 canonical, every tensor resolved, no shape error); the **forward-vs-oracle** half (GGUF-loaded argmax == safetensors argmax) is the remaining rung | loader: ✅; forward oracle: ◻ | P1 Expert splitter |
