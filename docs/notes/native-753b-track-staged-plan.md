...
- **Claims discipline:** today's "CPU-offload" is compute-placement (the `--n-cpu-moe`
  equivalent). The collectives now reach across **processes** (`DistComm`, host f32,
  byte-exact vs cpu-ref) but NOT yet across **devices**. **Do not claim multi-GPU** until a
  non-cpu-ref **device** `compute.CollectiveBackend` (the NCCL/RCCL backend) all-reduces a
  device tensor across GPUs. The `pageIn` primitive HAS landed (`f54e01a`,
