...
| P1 E2E (load ✅ + forward ✅) | a complete `glm_moe_dsa` GGUF resolves its FULL tensor set + splits experts (**`2e632ef`**) AND `LoadModel`→`Forward` RUNS the native glm_dsa forward (MLA + DSA indexer + MoE routing + shared experts) to finite logits (**`d3e1c1d`**, `...Forwards`). fak loads AND forwards a real-arch glm_moe_dsa GGUF. Remaining: the **argmax-vs-safetensors-oracle** match (correctness, not just runs) | loader ✅; forward-capable ✅; argmax oracle ◻ | P1 Expert splitter |
