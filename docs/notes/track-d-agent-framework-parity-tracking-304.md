...
|---|---|---|---|
| **D-001** #255 benchmark (P0) | 🟡 **Scaffold shipped; published run bench-node-gated** | A host-runnable N=1/100/500/1000 fan-out scale harness (**N≥1024 capable**) that computes coordination-overhead-vs-N=1 and cross-agent-reuse-uplift **deterministically** (turnbench is in-process kernel arithmetic, not model calls), wrapping the already-witnessed fan-out engine. `cmd/fanbench --grid canonical` now emits the exact D-001 ladder as a user-facing command path. **CPU smoke green** (`go test ./internal/bench -run FanScale` → `ok`). The published HEADLINE — live-model wall-clock + the LangGraph/AutoGen/CrewAI comparison + the results table — is explicitly **deferred** to a bench node (`DeferredRun`), not fabricated here. | [`internal/bench/fanscale.go`](https://github.com/anthony-chaudhary/fak/blob/main/internal/bench/fanscale.go) · [`internal/bench/fanscale_test.go`](https://github.com/anthony-chaudhary/fak/blob/main/internal/bench/fanscale_test.go) · [`internal/turnbench/fanout.go`](https://github.com/anthony-chaudhary/fak/blob/main/internal/turnbench/fanout.go) · [`cmd/fanbench/main.go`](https://github.com/anthony-chaudhary/fak/blob/main/cmd/fanbench/main.go) |
...
- **On-disk evidence:** `internal/bench/fanscale.go` + `fanscale_test.go`, `internal/turnbench/fanout.go`,
  ...
  `internal/a2achan` + `internal/comm/comm.go` + `internal/agenttopo` +
  `tools/schemas/shared-*.json` + `examples/shared-task-record` (D-007); `internal/turnbench/` + `examples/trajectory` (D-008).
- **Witnessed gate:** `go test ./internal/bench -run FanScale -count=1` → `ok ... 0.440s` (D-001);
  `go test -race ./internal/a2achan ./internal/comm ./internal/agenttopo -count=1`
  → all `ok` (D-007, 2026-06-26); the shared-state rung's `tools/schemas/shared-*.json` + `examples/shared-task-record`
  fixtures self-validate as data (the `internal/sharedtask` runtime `go test` witness lands once the fold is wired).
  Both on WSL Ubuntu-24.04; native Windows `go test` is OS-blocked — see AGENTS.md.
