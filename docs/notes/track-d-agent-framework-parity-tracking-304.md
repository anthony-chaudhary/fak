...
| **D-003** #250 CrewAI (P1) | 🟡 **Wire-supported; bespoke adapter/example acceptance open** | CrewAI's `LLM(base_url=...)` (routed through LiteLLM) reaches the gateway today (matrix row 61, "Yes"). The issue's acceptance — a CrewAI **task adapter**, a **manager-role reduction** doc, an **example crew**, a **performance comparison** — is not shipped. | [`docs/integrations/compatibility-matrix.md`](../integrations/compatibility-matrix.md) (row 61) |
...
is bench-node-gated (D-001), 2 are wire-supported with their adapter/benchmark acceptance
open (D-003, D-004), 2 are partial scaffolds whose public surface is tracked by a separate
epic (D-007→#639, D-008→#498), and 2 are unimplemented features (D-005, D-006). So #304 stays
**OPEN** — correctly.
...
| D-003 #250 | Build a minimal CrewAI example crew pointed at fak (`LLM(base_url=...)`), document the manager-role coordination reduction, measure vs native | host-tractable (Python example) |
