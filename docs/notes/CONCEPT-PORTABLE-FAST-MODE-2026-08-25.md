...
| Provider service/speed tiers | **ABSENT** | `internal/modelroute/providercontract.go:39-61` records API, cache, retry, usage, and maturity facts but no tier capability. `internal/agent/adapters.go` has no typed tier fields. `internal/gateway/http.go:1200-1202` advertises empty tier lists. | #8962. |
