...
description: "Classifies #2864 (fail-closed observer/middleware hook contracts) as gen/next, records that its enforcement contract already shipped unwired under the closed #2904, and names the two residuals plus the Likely-files trap that mis-routes this track."
---
...
`hermes-inspired` was missing from #2864 and is missing batch-wide — #2865,
#2866, and #2867 also lack it despite descending from the same decomposition.
Repaired here for #2864 only. The rest want a sweep, which would also have
surfaced the #2864/#2904 duplication at filing time: both issues carry the label
semantics but only one carried the label.
