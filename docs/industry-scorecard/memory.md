...
- **SOTA bar:** PagedAttention reduces KV-cache memory waste from 60-80% (reserve-for-max) to under 4%, enabling near-95% GPU memory utilization for KV cache and proportionally larger max batch size / concurrent-sequence counts on fixed hardware.
- **Leading systems:** vLLM (PagedAttention), vAttention (paging without PagedAttention), SGLang (token-level KV pool)
...
- **SOTA bar:** SGLang's RadixAttention prefix reuse yields ~29% higher throughput than vLLM on shared-context workloads, and cache-aware routing lifts prefix-cache hit rate up to 3.8x; the win scales with prefix-share fraction and is marginal when requests share no prefix.
- **Leading systems:** SGLang (RadixAttention), vLLM (Automatic Prefix Caching), TokenLake (segment-level prefix pool)
- **Source:** [https://www.spheron.network/blog/vllm-vs-tensorrt-llm-vs-sglang-benchmarks/](https://www.spheron.network/blog/vllm-vs-tensorrt-llm-vs-sglang-benchmarks/) (2026)
...
- **SOTA bar:** PagedAttention achieves ~96.3% KV-cache memory utilization (<4% waste) vs 20-40% for prior systems like Orca; eliminates the 60-80% fragmentation/over-reservation waste of contiguous-allocation engines
- **Leading systems:** vLLM (PagedAttention), SGLang, TensorRT-LLM
...
- **SOTA bar:** SGLang RadixAttention delivers up to ~6.4x throughput on prefix-heavy workloads vs non-caching baselines; reported cache hit rates of 75-95% on few-shot/multi-turn workloads where naive paged engines stay far lower
...
- **SOTA bar:** CacheBlend reuses KV for all chunks (prefix or not) with ~15% recomputation, cutting TTFT 2.2-3.3x and raising throughput 2.8-5x vs prefix caching, with negligible quality loss; LMCache reports 3-10x TTFT reduction overall
- **Leading systems:** LMCache (CacheBlend)
- **Source:** [https://blog.lmcache.ai/en/2024/10/09/beyond-prefix-caching-how-lmcache-speeds-up-rag-by-4-5x-by-one-line-of-change/](https://blog.lmcache.ai/en/2024/10/09/beyond-prefix-caching-how-lmcache-speeds-up-rag-by-4-5x-by-one-line-of-change/) (2024-10)
- **fak:** no-claim — no number (stub)
...
- **SOTA bar:** LMCache offloads across CPU RAM, local disk, Redis/Valkey, Mooncake, InfiniStore, S3, NIXL and GDS with far higher CPU-load bandwidth than vLLM native offload (claimed up to 7x faster access to ~100x more KV); Mooncake pools CPU/DRAM/SSD/RDMA into a disaggregated store
- **Leading systems:** LMCache, Mooncake (Mooncake Store)
- **Source:** [https://github.com/lmcache/lmcache](https://github.com/lmcache/lmcache) (2026-01)
...
- **SOTA bar:** No clean universal SOTA: H2O/StreamingLLM/SnapKV/Quest hold quality at moderate budgets but accuracy of all practical methods drops sharply below ~1024-token budgets, while oracle top-k stays near-lossless even at 256, exposing a real prediction gap
- **Leading systems:** H2O, StreamingLLM, SnapKV, Quest
- **Source:** [https://arxiv.org/pdf/2406.10774](https://arxiv.org/pdf/2406.10774) (2024-06)
- **fak:** no-claim — no number (stub)
...
- **SOTA bar:** NIXL moves KV point-to-point over RDMA/InfiniBand, RoCE/UCX, TCP, NVMe-oF and S3 with non-blocking transfers; Dynamo overlaps layerwise KV generation with transfer so forward passes continue, hiding network latency to limit TTFT inflation from cold/cross-GPU transfers
- **Leading systems:** NVIDIA NIXL, NVIDIA Dynamo, Mooncake
- **Source:** [https://developer.nvidia.com/blog/enhancing-distributed-inference-performance-with-the-nvidia-inference-transfer-library/](https://developer.nvidia.com/blog/enhancing-distributed-inference-performance-with-the-nvidia-inference-transfer-library/) (2025-04)
- **fak:** no-claim — no number (stub)
...
- **SOTA bar:** Dynamo's KV-aware router scores request-to-cached-block overlap across all GPUs and routes to minimize recomputation while balancing load; Baseten reports ~2x faster inference using Dynamo KV-cache-aware routing
- **Leading systems:** NVIDIA Dynamo (KV-aware router), SGLang router, Baseten
- **Source:** [https://www.baseten.co/blog/how-baseten-achieved-2x-faster-inference-with-nvidia-dynamo/](https://www.baseten.co/blog/how-baseten-achieved-2x-faster-inference-with-nvidia-dynamo/) (2025-01)
- **fak:** no-claim — no number (in-flight)
...
- **SOTA bar:** DeepSeek MLA stores ~70 KB/token vs 192-328 KB/token for GQA models (~2.7-4.7x smaller); DeepSeek-V2 MLA cut KV size ~93.3% vs its 67B dense baseline
- **Leading systems:** DeepSeek-V2/V3 (MLA), GQA-based models (Llama-3, Qwen)
- **Source:** [https://arxiv.org/html/2502.14837v1](https://arxiv.org/html/2502.14837v1) (2025-02)
- **fak:** no-claim — no number (stub)
...
- **SOTA bar:** vLLM FP8 KV-cache: negligible-to-minimal loss (<=1-2 points; Qwen3.5-27B fully recovers AUC@1M, <=0.7pt on reasoning) and is now the recommended default for long-context decode. KIVI K4V2 achieves ~5.3x near-lossless compression; sub-4-bit (KIVI 2-bit, KVQuant) pushes further. Lossy on prefill-heavy / head_dim=256 / short context.
- **Leading systems:** vLLM (--kv-cache-dtype fp8, E4M3), KIVI (2-bit asymmetric), KVTuner (mixed-precision), KVQuant
