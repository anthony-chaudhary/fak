...
| **GPT-NeoX** (parallel residual, two learned-bias LayerNorms, interleaved fused QKV, partial rotary) | `gpt_neox` | CPU forward numerically witnessed in CI (weight-free oracle) | `family_gptneox_cpu_oracle_test.go` `TestGPTNeoXCPUNumericOracle` and `…PartialRotary` — production forward vs an independent HF-semantics scalar reference at **both** published `rotary_pct` lineages (1.0 and the Pythia default 0.25), with a `…IsSensitive` non-vacuity gate. The 0.25 half is what caught the partial-rotary `inv_freq` denominator bug. No real-checkpoint HF export yet |
| **StableLM** (pre-norm, learned-bias LayerNorm, partial rotary) | `stablelm` | CPU forward numerically witnessed in CI (weight-free oracle) | `family_stablelm_cpu_oracle_test.go` `TestStableLMCPUNumericOracle` — production forward (prefill + cached decode) vs an independent HF-semantics scalar reference at both published lineages (`use_qkv_bias` off, stablelm-3b-4e1t, and on, the StableLM-2 line), with a `…IsSensitive` non-vacuity gate. No real-checkpoint HF export yet |

The mechanical-axis loader also parses several more families' metadata (Cohere
parallel-residual, DeepSeek-V2 MLA, MiniMax-M3
