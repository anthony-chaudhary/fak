...
fak cadence   [--json] [--check] [--append-history] [--window N]   # consolidated regular-cadence report: folds scores + maturity + work-done + releases into one control-pane envelope; --append-history writes the durable ledger with standing_score + difficulty fields (docs/cadence/history.jsonl)
fak maturity  [next] [--json|--markdown] [--compare base.json]   # feature-maturity lifecycle scorecard: places every declared capability (one per internal/<leaf> lane) on a closed ladder (proposed→prototyped→tested→dogfooded→default, +benchmarked badge) and emits the next work item that would mature each; `fak maturity next` is the ranked backlog. Immaturity is never a defect — only a ladder-skip (fak relies on it but it has no tests) is.
