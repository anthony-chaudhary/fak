...
One precise pre-identity-receipt checkpoint shape is resumed without refetching completed pages. Its issues receipt retained an exact `classified_pull_count` but its normalized records retained only issue rows, so the mixed PR identities cannot be recovered from the dedicated pulls records. After pulls completes, the receipt therefore uses `evidence_mode: "legacy_count_only"`, `identity_basis: "legacy_checkpoint_counts"`, and `evidence_reason: "legacy_mixed_pull_identities_not_retained"`. The mixed and dedicated endpoint counts remain exact, and the normalized pulls records retain exact dedicated identities, but `relation_evidence: "unavailable"` explicitly marks `overlap`, `only_in_mixed`, and `only_in_dedicated` unavailable; their lists and counts are JSON `null`, never empty or projected.

Legacy count-only evidence records `symmetric_difference_lower_bound = abs(mixed_count-dedicated_count)` and `symmetric_difference_upper_bound = mixed_count+dedicated_count`. The same 1,000-identity policy evaluates this interval conservatively: verdict `accepted` requires the total upper bound and both endpoint cardinality upper bounds to fit their declared limits; `rejected` means a cardinality-derived lower bound already exceeds a limit; otherwise the verdict is `compatible_unproven`. That middle verdict permits bounded temporal count drift to remain inspectable and resumable, but the corpus stays typed `partial` with `accepted: false`; neither `study-forge validate` nor `study-monitor` promotes it to complete exact evidence. Only the rare count-only shape whose entire possible interval proves policy acceptance may carry a complete accepted verdict. Exact captures and already-upgraded `legacy_checkpoint_projection` corpora continue to use strict identity-set reconciliation.
