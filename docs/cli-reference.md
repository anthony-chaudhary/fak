...
`capture` is REST-first. It resolves the repository's default-branch revision, follows the terminal pagination link for issues, pulls, discussions, releases, labels, and milestones, partitions pull-request rows out of the mixed issues endpoint, applies the inclusive RFC3339 cutoff, and writes canonical records plus `fak-studyforge-receipt/1` evidence. Each source receipt preserves the page chain, a page-chain checksum, fetched/normalized/unique counts, excluded-row counts, record checksums, and API metadata. The corpus remains explicitly `partial` until all six sources have reached terminal pages and reconciled; inaccessible sources retain failure evidence and the command exits nonzero.
