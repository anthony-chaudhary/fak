...
fak study-forge capture --repository owner/name --cutoff 2026-08-26T22:00:00Z --out /tmp/owner-name.corpus.json --checkpoint-pages 10
fak study-forge validate --receipt /tmp/owner-name.corpus.json
```

`capture` is REST-first. It resolves the repository's default-branch revision, follows the terminal pagination link for issues, pulls, discussions, releases, labels, and milestones, applies the inclusive RFC3339 cutoff, and writes canonical records plus additive `fak-studyforge-receipt/1` evidence. The dedicated pulls traversal is authoritative for normalized PR records. The mixed issues traversal contributes issue-only records plus sorted PR identities as cross-check evidence; it never duplicates PRs into the normalized index. Each source receipt preserves its crawl start/end, page chain, page-chain checksum, fetched/normalized/unique counts, excluded-row counts, record checksums, and API metadata.

Once both traversals are terminal, `receipt.non_atomic_delta` normally carries `evidence_mode: "exact_identity"` and records the exact PR identity-set overlap, `only_in_mixed`, and `only_in_dedicated` sets and counts, with both source crawl windows. Nonzero drift is accepted under the declared `bounded_identity_delta` policy (at most 1,000 identities on either side and in total); equality is not required because GitHub endpoints are not snapshot-atomic. Missing or contradictory evidence, duplicate identities within either endpoint, pagination/checksum failures, or policy overflow fail validation. Overflow leaves a validated `partial` checkpoint with `accepted: false` so the evidence is inspectable without claiming a complete corpus.

One precise pre-identity-receipt checkpoint shape is resumed without refetching completed pages. Its issues receipt retained an exact `classified_pull_count` but its normalized records retained only issue rows, so the mixed PR identities cannot be recovered from the dedicated pulls records. After pulls completes, the receipt therefore uses `evidence_mode: "legacy_count_only"`, `identity_basis: "legacy_checkpoint_counts"`, `identity_sets_available: false`, and `evidence_reason: "legacy_mixed_pull_identities_not_retained"`. The mixed and dedicated endpoint counts remain exact, and the normalized pulls records retain exact dedicated identities, but `relation_evidence: "unavailable"` explicitly marks PR-set relations unavailable. Legacy evidence never claims an overlap, either endpoint's only-in set, or an identity symmetric difference; those relation lists and counts remain JSON `null`, never empty, inferred, or projected.

Legacy count-only reconciliation declares `policy.metric: "endpoint_cardinality_delta"` and records `observed_endpoint_cardinality_delta = abs(mixed_count-dedicated_count)`. It is accepted if and only if that observed cardinality delta is at most the declared `policy.max_total` bound; for example, endpoint counts 35,528 and 35,551 produce an observed delta of 23. The mathematical possible identity symmetric-difference range, from `abs(mixed_count-dedicated_count)` through `mixed_count+dedicated_count`, may remain in the receipt as informational evidence, but neither endpoint of that range is the legacy acceptance metric. In particular, a large possible upper bound does not turn a within-bound endpoint cardinality delta into `compatible_unproven` or rejection.

Validation binds the policy metric to its evidence mode. It rejects legacy count-only evidence that selects an identity-set metric, exact identity evidence that selects `endpoint_cardinality_delta`, non-null or otherwise forged legacy identity-relation arrays or counts, an observed delta that does not equal the endpoint-count difference, a verdict inconsistent with the observed delta and bound, arithmetic overflow, or a missing legacy evidence reason. `study-monitor` applies the same validation when consuming a declared forge receipt; it does not reinterpret count-only evidence as identity evidence. New exact captures and already-upgraded `legacy_checkpoint_projection` corpora remain strict: they reconcile the available identity sets and accept or reject by their exact symmetric difference, unchanged by the legacy-only cardinality policy.

The corpus remains explicitly `partial` until all six sources have reached terminal pages and the cross-endpoint policy accepts; inaccessible sources retain failure evidence and the command exits nonzero.

Every accepted page is atomically checkpointed to `--out` by default. Normalization and duplicate/cursor checks finish before the page becomes durable, and the temporary file is renamed over the prior checkpoint only after the full canonical corpus has been encoded and validated. A process interruption before that rename preserves the preceding checkpoint, so the default can redo at most one accepted page. `--checkpoint-pages N` bounds write amplification for very large corpora by writing every N accepted pages plus the terminal state; the corresponding recovery tradeoff is that an interruption may redo at most N accepted pages.

`--resume` validates the stored corpus and receipt schemas, repository, cutoff, API base, source order, revision, page continuity, page-chain and record checksums, counts, and cursor history before requesting source pages. It then starts from the last validated page's `next` URL, preserves the already normalized records, and rejects repeated cursors or duplicate identities across the resume boundary. Completed sources are not refetched. Transient REST failures retry up to `--retries`; `--token-env` names the environment variable holding an optional read-only token.
