...
| **Upgrade** | [Install-path chooser](../adoption/install-paths.md) | Reuse the installation channel you operate. `go install ...@latest` selects the newest tag, while source builds select the checked-out revision. [Stable releases](../stable-releases/README.md) are sparse, evidence-backed rollback anchors; rolling `vX.Y.Z` tags are the faster release channel. |
