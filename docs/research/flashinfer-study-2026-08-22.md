...
[`inventory/flashinfer-ai-flashinfer.json`](inventory/flashinfer-ai-flashinfer.json), generated by `fak study-inventory` from a detached checkout of
...
- **Roadmap/TODOs:** no dedicated roadmap file exists. The tree contains 408 `TODO`/`FIXME` matches across 170 runtime files; these and the complete tracker ledger are distributed planning evidence, not promises.
