...
7. Do not satisfy a class by naming it alone when the machine map says the class is `partial` or `external_required`. Add `source_evidence` rows with durable commands, exported artifacts, note anchors, or issue URLs for non-tree classes such as full open/closed issue and PR history, fak self-query witnesses, candidate matrices, and issue tracking. Evidence references must be traceable rather than free-form assertions; the compound forge class must name issue, pull-request, and discussion coverage. The check does not replay commands or re-fetch URLs, so a reference proves traceability rather than the truth of the target artifact.
