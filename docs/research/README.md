...
- [Micro-context fabrics for 100â€“10,000 parallel agents](micro-context-fabrics.md) â€” split one cached agent base into many bounded logical contexts; includes the runnable 10k synthetic spine and controlled-kernel/API-only research ladder.
- [Micro-context S1 real-endpoint witness](micro-context-s1-real-endpoint.md) â€” 100/100 contexts through four bounded workers, with TTFT/usage telemetry and a retained 16-worker overload finding.
- [Micro-context S2 shared-prefix A/B](micro-context-s2-prefix-ab.md) â€” no cache benefit observed on the first real endpoint; scoped concurrency improved aggregate work while worsening TTFT.
...
1. **Question or hypothesis** â€” the proposition being tested.
2. **Source and date** â€” paper, repository revision, dataset, model, interview, or local observation.
3. **Method and environment** â€” baseline, hardware, software version, workload, mode, and constraints.
4. **Result and uncertainty** â€” what was observed, its scope, and what remains unknown.
5. **Maturity and generation** â€” hypothesis, observed, validated option, or promoted; plus the applicable generation stream.
6. **Promotion witness** â€” the evidence and maintained destination required before the claim becomes implementation guidance.
