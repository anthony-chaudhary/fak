...
| **Capability gating** | None built into the engine; `--api-key` protects only `/v1`, and operators are told to add a reverse proxy. | A reviewable, editable capability floor (`fak policy --dump`/`--check`, `--policy floor.json`) enforced fail-closed, with a closed 17-reason refusal vocabulary. |
