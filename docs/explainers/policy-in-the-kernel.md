...
tool run?" lives *outside* the loop — a pre-tool hook in another process, a sidecar
policy service over a socket, an LLM that grades the request. All three share two
weaknesses: the model can argue its way past a recognizer, and when the outside
thing crashes or times out, the call usually runs anyway (fail-open). `fak` moves
that decision onto the *same* code path as the tool call — one Go address space, no
IPC, default-deny — so the check is not a thing the agent talks to, it is a thing the
agent's call *passes through*, the way a `read()` passes through the OS kernel before
it touches a disk. The refusal of an irreversible action does not depend on catching
the attack. It depends on the lever never having been wired up.

That is the whole flip, and it is worth slowing down on, because "policy in the
kernel" sounds like a slogan and is actually a specific, checkable claim about
*where the code runs*.
...
Every one of these is a *recognizer*. It works by trying to tell good from bad. And
the serious prompt-injection research has already reached the uncomfortable
conclusion that recognizing attacks is a losing game: a classifier asks "is this
text bad?", and an attacker with paraphrase, encoding, or a foreign language can
make bad text not look bad. Our own audit of `fak`'s built-in detector measured it
as **≈100% evadable** by a determined attacker — and we say so in the README. A
recognizer is a helpful bonus. It is not a floor.
...
1. **There is nothing to talk past.** The model never addresses the gate; its call
   ...
   conversational participant — the same reason a process cannot `printf` its way
...
3. **The decision is structural, not heuristic.** Whether an irreversible tool runs
   is decided by whether its name is on a reviewable list — not by whether a model
   or a regex *recognized* this particular request as dangerous. A list is something
   you can read, diff, and sign. A recall curve is not.
...
quoting it as "fak is thousands of times faster" would be comparing against a
baseline nobody actually runs, and would miss what matters.)
...
- **It does not, by itself, bound the *arguments* of an allow-listed tool.** If you
  ...
  *capabilities* (path/host/amount as first-class constraints, not regexes) are the
  real fix, and are roadmap, not shipped.
...
- **The deepest rung is proven, not yet wired live.** The same quarantine verdict can
...
- The deployable policy manifest and its exact honest-scope boundary: [`fak/POLICY.md`](../../fak/POLICY.md)
...
- The extension model (how a rung registers without a spine edit): [`fak/ARCHITECTURE.md`](../../fak/ARCHITECTURE.md)
- Live A/B on real models (injection kept out of context 5/5): [`fak/LIVE-RESULTS.md`](../../fak/LIVE-RESULTS.md)
- The full per-capability honesty ledger: [`fak/CLAIMS.md`](../../fak/CLAIMS.md)
