...
| `nativeserve` | off-by-default (wired) | `--native` | `Native` | `internal/gateway/messages.go:201 (stream), :205 (buffered)` | routes BOTH streamed and buffered /v1/messages through fak's owned loop — `agent.RunArmStream` rendered as Anthropic SSE for `stream: true`, `agent.RunArm` for the buffered turn. A streamed turn degrades to the BUFFERED native handler (never the proxy) on a non-flushable ResponseWriter, a planner that does not support streaming, or an armed stop-gate; off by default |
...
| `assumesessionturns` | wired | `--assume-session-turns` | `AssumeSessionTurns` | `internal/gateway/messages.go:headSessionPrior` | the head-anchored burst gate's presumed session length when NO bounded Budget.TurnsLeft horizon is wired (the `fak manage -- claude` case): headSessionPrior maps the trace's served-turn depth to CurrentTurn and this value to TotalTurns, so a WARM un-budgeted long session fires the #1407 head re-anchor shed early (agent.CacheBurstPaysBack, #1408) and refuses near the presumed end; DEFAULT-ON at gateway.DefaultAssumedSessionTurns, a genuine wired Budget.TurnsLeft always wins, pass 0 for the byte-for-byte conservative no-horizon behavior. Inert unless --compact-anchor-head engages the head re-anchor |
