...
| `compactanchorhead` | off-by-default (wired) | `--compact-anchor-head` | `CompactAnchorHead` | `internal/gateway/messages.go:compactAnthropicRawWithReason` | re-anchors compacthistory's protected prefix on the stable system/tools head (agent.CompactAnchorHead) instead of the first cache_control breakpoint, the #1407 anchor-starved fix; off by default because a fire bursts the recent breakpoint's cached suffix once, gated on agent.CacheBurstPaysBack (#1408) against the live session's Budget.TurnsLeft horizon — a session with no bounded turns budget (DecideSession unwired, or session.Unbounded) only ever relabels the bail reason and never actually sheds tokens |
