...
| `compacthistory` | wired | `--compact-history-budget` | `CompactHistoryBudget` | `internal/gateway/messages.go:365` | compacts old turns in the Anthropic outbound body once it sprawls past the budget, cache prefix byte-identical; DEFAULT-ON at ~48k (gateway.DefaultCompactHistoryBudget), pass 0 to disable |
...
| `compactsolvencyfloor` | off-by-default (wired) | `--compact-solvency-floor` | `CompactSolvencyFloorTokens` | `internal/gateway/messages.go:compactAnthropicRawWithReason` | the OCCUPANCY axis under the head-anchored burst gate: once the trace's OBSERVED peak resident window (harnessCoherence.heldResidentPeak — the same signal the volume-aware horizon reads) reaches this many tokens, a `burst_unprofitable` refusal is OVERRIDDEN and the shed fires anyway. agent.CacheBurstPaysBack prices a fire in cache dollars and has no term for running out of window (the gateway never sees a window SIZE), so measured over 3191 served turns its fire rate INVERTED against occupancy — 33% at 96-125k resident down to 0.0% above 170k, with 100% of traces that ever fired never firing again. Above this line the question stops being "does the burst repay?" and becomes "can we afford not to?". Can only ever turn a BAIL into a fire, never the reverse; forced fires are counted apart as `compaction_solvency_forced` so they are never booked as cache wins. OFF (0) by default and byte-for-byte inert when unset — the operator (or `fak dispatch`, which derives 85% of window−reserve = 142800) must supply it. Inert unless --compact-anchor-head engages the head re-anchor |
