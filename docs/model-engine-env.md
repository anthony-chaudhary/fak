...
|---|---|---|---|---|
| `FAK_GPU_BUDGET_MB` | int (MiB), or `auto` | unset / `0` / invalid = **unbounded** for budgeted weight-upload paths | The **Spills** lever. Cap device-local weight residency on a card whose VRAM is smaller than the weight set; weights past the cap spill *in upload order* (early/hot layers stay device-local, the cold tail spills by choice instead of losing the allocation race): Vulkan uses host-visible memory, CUDA uses managed memory. Set **`auto`** to derive the cap from the backend's own capacity query (≈90% of total device-local memory), so the same binary right-sizes itself on a smaller card without a hand-computed MiB number; if capacity is unknown it fails open to unbounded (byte-identical to today). | `internal/compute/vulkan.go:57`, `internal/compute/cuda.go:185` |
| `FAK_BACKEND` | `auto`, `cpu`, or a registered backend name | unset = platform automatic selection | Select the native compute backend used by `fak up`, in-kernel `fak serve`, `fak run`, and local-model `fak guard`. An explicit `--backend` wins, including `--backend auto`; otherwise this variable applies. Unset or `auto` selects registered, usable Vulkan on Linux/Windows, preserves automatic Metal on Darwin, and uses the CPU floor when that platform backend is unavailable. Set `cpu` to force the portable CPU floor. The native micro-scout remains explicitly CPU-only because its process-cached classifier has no residency close hook. A named unavailable backend fails loud. Vulkan becomes eligible only after a `-tags vulkan` build successfully registers it, which currently also requires `FAK_VULKAN_SPIRV` to name the compiled shader directory. | `cmd/fak/serve_model_load.go` (`resolveServeBackendSelection`) |
| `FAK_VULKAN_SPIRV` | filesystem path (compiled SPIR-V dir) | unset = **Vulkan backend not registered** | Point at the compiled shader dir to register and use the Linux/Windows Vulkan backend at runtime. `build_vulkan.ps1` sets it after a build. Without it, `init()` returns early and only the Reference floor is available. (Requires the `vulkan` build tag.) | `internal/compute/vulkan.go` (`init`) |
| `FAK_METAL` | flag (set/unset) | unset = automatic on darwin/arm64 when usable, otherwise off | Apple GPU. In `fak serve` it is the env-equivalent of `--metal`: with `--gguf` and no `--base-url` it runs the in-kernel chat through the metalgemm GPU — **prefill + GPU-resident Q8 decode** on a dense Qwen-class Q8 model (#67). In `cmd/fakchat` it routes the resident-Q4_K hybrid **prefill** q4_k GEMMs through the Metal dequant-GEMM (Qwen3.6-27B path; decode stays CPU there). The Metal backend is linked automatically on darwin/arm64 with cgo; it is a no-op on the pure-Go build (and `fak serve --metal` **fails loud** if requested without the build/device, rather than serving on CPU). | `cmd/fak/serve.go` (`--metal`/`resolveServeMetal`), `cmd/fakchat/main.go:205` |
...
LRU at a fixed budget and bounded by an exact offline oracle, active pins are
...
| Variable | Type / units | Default | When to use | Source |
|---|---|---|---|---|
| `FAK_INKERNEL_RADIX` | enum-ish: `off` disables, anything else/unset enables | enabled | Turn off planner-owned RadixAttention prefix reuse for A/B or diagnosis. Device-backend planner requests still bypass radix reuse today because the reusable prefix clone is a CPU-session cache. | `internal/agent/inkernel_planner.go:162` |
| `FAK_INKERNEL_RADIX_BUDGET` | int, edge tokens | unset / invalid / `0` = unbounded | Bound the planner radix tree's edge-token budget. When over budget, the tree uses `FAK_NATIVE_KV_VICTIM_RULE`; `cost-aware` keeps reused prefixes ahead of one-shot cold prompts. | `internal/agent/inkernel_planner.go:163` |
