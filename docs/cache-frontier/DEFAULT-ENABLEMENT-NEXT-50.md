...
| 9 | API/provider | Make provider-cache secret classification default-deny for active warming. | Secret/regulated prefixes can still be sent normally, but no pre-warm/pin is scheduled. **OPEN (#1527), classified `gen/now`:** the secret-class default-deny fence is not yet in the warm-admission path — `internal/compute/prewarm_admit.go`'s `PrewarmCandidate` (as of `bb73d065`) carries fences 1–3 (byte-known / pool-pressure / timeliness) but no secret-class field, so a secret/regulated prefix is not yet refused for pre-warm/pin. Horizon classified `gen/now` (immediate cache safety fence, focused-test witness, no new serving architecture; #1519–#1528 pool of [GENERATION-CACHE-CONTEXT-PROGRAM-MAP-2026-06-30](../notes/GENERATION-CACHE-CONTEXT-PROGRAM-MAP-2026-06-30.md)). Implementation stays a `compute`-lane leaf: add a `SecretClassified` fence returning `WarmSkip` (new `ReasonSecretClass`) plus a refuse-warm-but-still-send test — the ordinary send path is never gated. |
...
| 46 | Remove legacy | Add docs/claims lint for cache headlines that omit plane/provenance. | "99% cache" or "cache win" without provider/kernel/context labels fails review. **OPEN (#1564).** |
