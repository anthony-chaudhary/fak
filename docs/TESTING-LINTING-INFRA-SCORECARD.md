...
| `fak test --json -n race` / `fak test --json codelint cmd/fak/test.go` | emits `fak.test_repair_packet.v1` with the resolved command, normalized finding class, diagnostics, exit code, and next action; command-result packets classify Go test, go build, go vet, gofmt, codelint, setup, and spawn failures, while ruff and scorecard-specific parsers are still pending |
...
| Agentic usability | 85 | Agent-readiness is A, `fak test` is discoverable, and `fak test --json` now emits repair-packet envelopes with next actions for Go test plus build/vet/gofmt/codelint gates; raw ruff/Python scorecard output still lacks the full unified ranked parser set. |
