...
| `fak test --list` | tiers: `fast`, `full`, `race`, `<pkg>` |
...
| CI speed/caching | 76 | Go build/test cache and separate race cache are wired; there is no package-duration shard planner or hard slow-test budget trend yet. |
...
| 4 | Turn raw failures into an agent repair packet | Agents need the shortest path from red to next edit, not pages of logs. | `fak test --json` emits normalized findings for build, vet, gofmt, Go tests, ruff/codelint, and scorecard failures. | Fixture tests for each failure class and a docs example. |
