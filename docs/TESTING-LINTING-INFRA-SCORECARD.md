...
| `fak test --json -n race` / `fak test --json build` | emits `fak.test_repair_packet.v1` with the resolved command, normalized finding class, exit code, and next action; command-result packets classify Go test, go build, go vet, gofmt, setup, and spawn failures, while ruff/codelint/scorecard-specific parsers are still pending |
...
| Agentic usability | 85 | Agent-readiness is A, `fak test` is discoverable, and `fak test --json` now emits repair-packet envelopes with next actions for Go test plus build/vet/gofmt gates; raw Python/codelint/scorecard output still lacks the full unified ranked parser set. |
