...
| `fak test --json -n race` | emits `fak.test_repair_packet.v1` with the resolved command, normalized finding class, exit code, and next action; command-result packets classify Go test/build/setup/spawn failures, while build/vet/gofmt/ruff/scorecard-specific parsers are still pending |
...
| Agentic usability | 85 | Agent-readiness is A, `fak test` is discoverable, and `fak test --json` now emits a first repair-packet envelope with next actions; raw build/vet/gofmt/Python/scorecard output still lacks the full unified ranked parser set. |
...
| 4 | Turn raw failures into an agent repair packet | Agents need the shortest path from red to next edit, not pages of logs. | `fak test --json` emits normalized findings for build, vet, gofmt, Go tests, ruff/codelint, and scorecard failures. First slice shipped: resolved-command, usage, spawn, Go test/build/setup failure packets with output tails and next action. | Fixture tests for each failure class and a docs example. |
