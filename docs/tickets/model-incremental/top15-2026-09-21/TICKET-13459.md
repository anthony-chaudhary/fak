<!-- fak-public-issue: anthony-chaudhary/fak#13459 -->
<!-- fak-model-key: model-top15-20260921-shared-accumulate -->
# refactor(model): reuse ordered accumulation for shared expert merge

```routing
repo: fak
lane: model
paths: ["internal/model/v41_router.go","internal/model/v41_router_test.go"]
expected_steps: 3
```
...
## Current state
The already-shared routed-plus-shared wrapper still owns another elementwise addition loop; a second generic AddPair API would be redundant.
...
### File:Line seam
- internal/model/v41_router.go:137 v41SharedExpertAdd (at the pinned baseline).
- internal/model/v4_live_expert.go:174 existing shared-add consumer (at the pinned baseline).
Read-only reproduction: `git show 7e70e0e5aa7c4037d326ac57f9b15af9df76bf0b:internal/model/v41_router.go`.
Observed result: The already-shared routed-plus-shared wrapper still owns another elementwise addition loop; a second generic AddPair API would be redundant.

## Why this is next
Rank 14/15. A third accumulation consumer without inventing another primitive or moving routing policy.
...
Dependencies: [#13458](https://github.com/anthony-chaudhary/fak/issues/13458) must land first.
Reuse `internal/model/ffn/README.md` and the existing adapters; no new registry.

## Core through-line
After ticket rank 13 lands, retain all existing v41SharedExpertAdd admission, typed v4RouteError fields/reasons and allocation in this wrapper. Copy routed into the new output and use AddScaled(output, shared, 1). Keep input validation order and the first non-finite result error unchanged. After AddScaled, scan the output in increasing index order and retain the first v4RouteError{Field:"shared_add", Reason:"non-finite result at %d"} result failure.

Consumer: V4.1 forward and V4 live-expert callers through the existing wrapper. The witness must exercise this real adapter or existing fixture, beyond merely compiling a new helper.

## Gold-plating boundary
Do not create AddPair, import Config/router errors into ffn, change input mutation or claim a speedup. Preserve all existing finite checks and typed errors.
...
Witness: `go test ./internal/model -run '^(TestV41SharedExpertAddScaledParity)$' -count=1`

## Verifiable Witness
Run from the public checkout with `GOWORK=off` and its documented host toolchain:
```powershell
go test ./internal/model -run '^(TestV41SharedExpertAddScaledParity)$' -count=1
go test ./internal/model -count=1
```
NEW tests to author (not present or passing yet): TestV41SharedExpertAddScaledParity.
The owning-package suite supplies broader regression coverage.
Before accepting a filtered green result, run `go test ./internal/model -list '^(TestV41SharedExpertAddScaledParity)$'` and verify that every named test appears. Zero matches or skipped required cases fail acceptance. Record leaf feedback separately from the full owning-package result; both are required.
...
## Likely files
- internal/model/v41_router.go
- internal/model/v41_router_test.go
Absent test/leaf paths are intended additions; no unrelated file ownership is implied.

## Expected steps
3
1. Pin finite bit parity, signed zero, overflow and exact error fields.
2. Use AddScaled under the existing wrapper.
3. Verify one output allocation and unchanged inputs/error order.

## Value
- Centrality: Core
- P1 Context: advanced - pinned source seams and file ownership avoid broad repeated reconnaissance.
- P2 Net value: advanced - A third accumulation consumer without inventing another primitive or moving routing policy.
...
github_issue: anthony-chaudhary/fak#13459
