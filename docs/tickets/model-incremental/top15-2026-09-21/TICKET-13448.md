<!-- fak-public-issue: anthony-chaudhary/fak#13448 -->
<!-- fak-model-key: model-top15-20260921-qwen-shared -->
# refactor(model): reuse Gated in Qwen shared expert fallback

```routing
repo: fak
lane: model
paths: ["internal/model/moe.go","internal/model/qwen_shared_ffn_composition_test.go"]
expected_steps: 3
```
...
## Current state
Qwen's generic shared-expert path repeats gating while its separate fused route and post-down sigmoid must stay intact.
...
### File:Line seam
- internal/model/moe.go:629 qwen35SharedExpert (at the pinned baseline).
- internal/model/moe.go:639 fused branch; 655 generic tail; 661 output sigmoid (at the pinned baseline).
Read-only reproduction: `git show 7e70e0e5aa7c4037d326ac57f9b15af9df76bf0b:internal/model/moe.go`.
Observed result: Qwen's generic shared-expert path repeats gating while its separate fused route and post-down sigmoid must stay intact.

## Why this is next
Rank 3/15. Separates reusable arithmetic from the Qwen-specific output gate without touching device policy.
...
## Core through-line
Delegate only the generic activation/down tail to Gated. Keep the fused Q4_K early return and the post-down scalar sigmoid multiplication in their current order and owner.

Consumer: moeFFN.apply -> qwen35SharedExpert. The witness must exercise this real adapter or existing fixture, beyond merely compiling a new helper.

## Gold-plating boundary
No fused-route activation, backend switch, output-gate relocation or tensor naming changes.
...
Witness: `go test ./internal/model -run '^(TestQwen35SharedExpertFFNGatedParity|TestQwen35MoESharedExpertAdded)$' -count=1`

## Verifiable Witness
Run from the public checkout with `GOWORK=off` and its documented host toolchain:
```powershell
go test ./internal/model -run '^(TestQwen35SharedExpertFFNGatedParity|TestQwen35MoESharedExpertAdded)$' -count=1
go test ./internal/model -count=1
```
NEW tests to author (not present or passing yet): TestQwen35SharedExpertFFNGatedParity.
Existing regression checks: TestQwen35MoESharedExpertAdded.
Before accepting a filtered green result, run `go test ./internal/model -list '^(TestQwen35SharedExpertFFNGatedParity|TestQwen35MoESharedExpertAdded)$'` and verify that every named test appears. Zero matches or skipped required cases fail acceptance. Record leaf feedback separately from the full owning-package result; both are required.
...
- internal/model/qwen_shared_ffn_composition_test.go
Absent test/leaf paths are intended additions; no unrelated file ownership is implied.

## Expected steps
3
1. Add old-body parity for SiLU, both GELUs, and output gate values.
2. Replace only the generic tail.
3. Verify parity, width fallback and allocation behavior.

## Value
- Centrality: Core
- P1 Context: advanced - pinned source seams and file ownership avoid broad repeated reconnaissance.
- P2 Net value: advanced - Separates reusable arithmetic from the Qwen-specific output gate without touching device policy.
...
github_issue: anthony-chaudhary/fak#13448
