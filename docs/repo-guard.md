...
[`.claude/settings.json`](../.claude/settings.json) wires `repo_guard.py` as a
...
- The **DOS** checker is best-effort shell parsing: it splits on `; | && ||`,
  ...
  remains the FAK default-deny floor ([`POLICY.md`](../POLICY.md)).
...
- [`POLICY.md`](../POLICY.md) — the capability floor + the `arg_rules` schema.
- [`dos.toml`](../dos.toml) — `[reasons.OUT_OF_TREE_WRITE]` and its sibling reasons.
- [`DOGFOOD-CLAUDE.md`](../DOGFOOD-CLAUDE.md) — the policy's deny table in context.
- [`tools/proc_resource_guard.py`](../tools/proc_resource_guard.py) — the sibling
