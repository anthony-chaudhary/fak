...
| 4. Pin a stable version | a fleet should hold a version and not auto-upgrade | `FAK_APP_VERSION=0.47.0` |
...
1. `FAK_APP_VERSION=0.47.0` in the environment (highest precedence; the simplest **pin to v0.47.0**).
2. A release build's `-ldflags "-X …/internal/appversion.BuildVersion=0.47.0"`.
