...
| `make mac-perf` | runs on-device Metal tok/s microbenchmarks (`BenchmarkMetalQ4KGemv`, `BenchmarkMetalQ4KGemmSteady`) and validates 3-way Mac comparison packet | regular shift-left performance gate for Mac development (verifies Metal prefill/decode throughput on Apple Silicon) |
