...
- A box that can't run a task safely is **never launched** for it — CPU server fak-native
...
- The CPU server number is **llama.cpp** (the mmap baseline), labelled OBSERVED against a
...
**CPU server CPU baseline — COLLECTED (the headline comparison).** The first two launches
were squeezed out by a **peer's 446 GiB fak-native resident serve** (driving CPU server to
...
**CPU server NVMe load rate — 1.549 GiB/s.** A cold sequential read of the 434 GB GGUF from
...
The hourly overnight tick keeps the loop alive: it re-attempts CPU server only when
