...
pipe-to-shell, out-of-tree writes, and writes into credential/SSH paths. So a block is
almost always a **`DEFAULT_DENY`** — a tool the floor simply never enumerated (a new MCP
tool, a harness verb, an orchestration name), *not* a danger refusal.
