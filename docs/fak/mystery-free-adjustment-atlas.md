...
| Repository architecture | `fak architecture --leaf LEAF` reports tier, package dependencies, fan-out, violations, and redundant reachability. | The package edge or declared tier rule named by the report. A redundant edge is a transitive-reduction candidate, not proof that its API use is safe to delete. | The source import/API call that creates the package edge, or the architecture declaration. When source provenance is absent, search the named source package; [#8452](https://github.com/anthony-chaudhary/fak/issues/8452) tracks putting it in the report. | Before editing, predict which dependency row a proposed import would add; rerun the leaf report after the isolated change. | [`docs/architecture.md`](../architecture.md), [`internal/archreport`](../../internal/archreport) |
