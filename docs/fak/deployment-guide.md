...
docker build -t fak:0.32.0 .

# Stamp a specific version into the binary:
docker build --build-arg APP_VERSION=0.32.0 -t fak:0.32.0 .

# Without cloning — build straight from the Git remote:
docker build -t fak:0.32.0 https://github.com/anthony-chaudhary/fak.git
```
...
docker run --rm -p 8080:8080 fak:0.32.0 serve --addr 0.0.0.0:8080 \
...
docker run --rm -p 8080:8080 \
  ...
  fak:0.32.0 serve --addr 0.0.0.0:8080 \
...
Installer knobs (environment): `FAK_VERSION` pins a version (e.g. `0.32.0`;
default latest release), `FAK_INSTALL_DIR` sets the target (default
`/usr/local/bin` if writable, else `~/.local/bin`).

```bash
FAK_VERSION=0.32.0 FAK_INSTALL_DIR=/usr/local/bin \
