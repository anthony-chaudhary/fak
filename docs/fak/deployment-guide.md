...
```bash
docker pull ghcr.io/anthony-chaudhary/fak:0.41.0
...
docker build -t fak:0.41.0 .

# Stamp a specific version into the binary:
docker build --build-arg APP_VERSION=0.41.0 -t fak:0.41.0 .

# Without cloning — build straight from the Git remote:
docker build -t fak:0.41.0 https://github.com/anthony-chaudhary/fak.git
```
...
docker run --rm -p 8080:8080 fak:0.41.0 serve --addr 0.0.0.0:8080 \
...
docker run --rm -p 8080:8080 \
  ...
  fak:0.41.0 serve --addr 0.0.0.0:8080 \
...
docker pull ghcr.io/anthony-chaudhary/fak:0.41.0-cuda
...
docker run --rm --gpus all -p 8080:8080 \
  -v /models:/models:ro \
  ghcr.io/anthony-chaudhary/fak:0.41.0-cuda \
...
services:
  fak:
    image: ghcr.io/anthony-chaudhary/fak:0.41.0   # published image; swap for your own to build from source
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: fak
          image: ghcr.io/anthony-chaudhary/fak:0.41.0   # or REGISTRY/fak:0.41.0 if you build from source
          args:
            - serve
            - --addr=0.0.0.0:8080
            - --provider=openai
...
Installer knobs (environment): `FAK_VERSION` pins a version (e.g. `0.41.0`;
default latest release), `FAK_INSTALL_DIR` sets the target (default
`/usr/local/bin` if writable, else `~/.local/bin`).

```bash
FAK_VERSION=0.41.0 FAK_INSTALL_DIR=/usr/local/bin \
