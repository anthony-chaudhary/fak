...
The in-process producer is `microagent.WithLineage`. `microcontextdemo` inherits the enclosing `FAK_REGISTRATION_ID` / `FAK_ROOT_REGISTRATION_ID` edge and wraps every logical context before it enters the host queue. Each context gets a durable `in_process_microagent` registration and distinct opaque session identity despite sharing its host PID; completion, failure, and cancellation terminalize the row. A nested runtime calls `Lineage.Child(parentLogicalID)` so its immediate parent changes while the original root remains stable. If ancestry is partially configured or registration fails, execution fails closed before the context's first model step.
