...
| FakBenchmarkFleetLoop | every 15m | node-work queue | `fak bench-loop fleet --apply --json` | walk every benchmark node and enqueue one content-addressed request per current next test; reruns do not duplicate work | `fak bench-loop install --interval 15`; inspect with `fak bench-loop fleet status --json` |
