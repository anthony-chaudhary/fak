...
The physical 4-arm comparison remains **PENDING_HARDWARE**. On `node-macos-a` the blockers observed
against the expected arm targets are: AX Engine 7.2.0 is absent; MTPLX is 2.11.2 (not the expected
2.9.0) and reports the 27B model `can_run:false`; llama.cpp is 8200 (not the expected b3600) with no
`--spec-type draft-mtp`; and swap is near-exhausted (~18.9/20.5 GB), so the strict mtpbench runner
cannot clear its no-swap-growth gate. The software increment is verified by
`go test -v ./internal/macbench -run TestValidateMTPComparisonPacketNodeMacOSA` and a `make mac-perf`
exit 0, but that is fixture/envelope validation only, **not** a `[HW-WITNESSED]` performance claim.
