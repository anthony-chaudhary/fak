...
- `fak serve` with **no `--policy`** loads the built-in production capability floor (standard
  coding and agent tools plus read-only `read_`/`get_`/`search_`/`list_`/`lookup_`/`find_`
