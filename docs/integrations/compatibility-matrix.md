...
| [Gemini CLI (Google)](https://github.com/google-gemini/gemini-cli) | Gemini (native Generative Language API via google/genai SDK) | Yes | `fak manage -- gemini` injects `GOOGLE_GEMINI_BASE_URL` into the child and a launch-scoped `GEMINI_CLI_SYSTEM_SETTINGS_PATH` clear-session hook; persistent user settings are unchanged |
...
- **Gemini CLI (Google)** — `fak manage -- gemini` is the supported native-wire route. `GOOGLE_GEMINI_BASE_URL` repoints Gemini protocol, not an arbitrary OpenAI/Anthropic wire; direct manual export remains an SDK seam with sandbox-propagation caveats, while fak injects it at the parent launch boundary. The session adapter uses Gemini's documented system-settings override and `SessionStart(clear)` hook without editing persistent user settings.
