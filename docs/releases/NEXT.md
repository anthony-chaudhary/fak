# fak vNext (targeting v0.55.0): Work in Progress

This document tracks in-flight work on `main` targeting the upcoming `v0.55.0` release.
It is updated as commits land so that release notes are maintained proactively rather than scrambled at cut time.

- **Projected version:** `0.55.0` (`minor` bump)
- **Base release tag:** `v0.54.0`
- **Commits in flight:** 0
