...
## Deterministic native-medium construction receipt

`no-spend-native-medium/manifest.json` is generated by the documented `--dry-run` command. It contains baseline, pinned-original (`caveman`), and canonical `native_medium` cells with three trials over the same corpus. It deliberately makes no network request and spends no API credits. Its fixed semantic outputs validate receipt construction only; they do not measure profile effectiveness or token savings.
