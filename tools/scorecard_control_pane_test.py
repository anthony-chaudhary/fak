...
def test_scoreboard_debt_doc_covers_at_least_20_categories() -> None:
    assert len(scp.DEBT_CATEGORIES_METADATA) >= 20


def test_scoreboard_debt_doc_fresh_on_trunk() -> None:
    root = scp.repo_root()
    baseline = root / scp.BASELINE_REL
    ok, msg = scp.check_scoreboard_debt_doc(root, baseline)
    assert ok is True, msg


def test_scoreboard_debt_doc_stale_detection(tmp_path: Path) -> None:
    baseline_path = tmp_path / scp.BASELINE_REL
    baseline_path.parent.mkdir(parents=True, exist_ok=True)
    baseline_path.write_text(__import__("json").dumps({"metrics": {"code": 10}, "grade_weights": {"code": 2}}), encoding="utf-8")

    doc_path = tmp_path / scp.SCOREBOARD_DEBT_DOC_REL
    doc_path.parent.mkdir(parents=True, exist_ok=True)

    # Missing file -> stale
    ok, msg = scp.check_scoreboard_debt_doc(tmp_path, baseline_path)
    assert ok is False
    assert "not found" in msg

    # Mutated file -> stale
    doc_path.write_text("mutated body", encoding="utf-8")
    ok, msg = scp.check_scoreboard_debt_doc(tmp_path, baseline_path)
    assert ok is False
    assert "drifted" in msg

    # write_scoreboard_debt_doc restores freshness
    changed = scp.write_scoreboard_debt_doc(tmp_path, baseline_path)
