...
# --- GENERATED, CHECKSUM-PINNED THIRD-PARTY ARTIFACT EXEMPTION (#5405) --------
#
# The needle scan is a CLAIM gate: a needle in an added line means THIS repo is
# asserting something it must not assert in public. That reading is exactly right for
# hand-written content and exactly wrong for a GENERATED third-party data artifact,
# where the needle is upstream DATA the artifact records in order to BLOCK it.
# `internal/egresslist/lists/*.txt` is rendered verbatim from a community filter feed
# by `fak egresslist refresh`; a feed of ad/telemetry domains carries vendor brand
# names by construction, so the affiliation needle refuses the whole CLASS of artifact
# and no amount of feed-shopping fixes it.
#
# WHY FLEET_ALLOW_LEAK=1 IS THE WRONG ESCAPE, AND WHY THIS BLOCK IS HONOURED TWICE.
# The affiliation needle is in AUDIT_NEEDLES and therefore in EXPORT_AUDIT_NEEDLES,
# and it has NO entry in REPLACEMENTS -- nothing would rewrite it on export. Overriding
# the COMMIT gate once would leave permanent needles that fail the POST-SCRUB export
# audit on every future publish: a broken publish pipeline, not one-commit friction.
# So the exemption is applied on BOTH paths -- the staged/range commit gate AND the
# post-scrub export audit (plus the whole-tree backstop).
#
# WHAT KEEPS IT FROM WIDENING TO HAND-WRITTEN FILES. Four independent conditions must
# ALL hold, and every one of them fails CLOSED:
#   1. PATH   -- the file matches a declared class glob (one directory, one extension).
#   2. HEADER -- its first line is the generator's do-not-hand-edit banner.
#   3. PIN    -- its bytes hash to the sha256 the class manifest pins for its name,
#                under the same canonicalization internal/egresslist.Checksum uses.
#                A hand-smuggled line changes the hash and the exemption evaporates.
#   4. SHAPE  -- the individual LINE carrying the needle is a rendered upstream rule
#                (`||host^` / `@@||host^`). A needle in the artifact's prose header, or
#                on any line the generator does not emit, is still a hit.
# And only the AFFILIATION needle class is exemptible. Credential SHAPES
# (AUDIT_REGEXES), operator home paths, real IPs and private host aliases stay strict
# inside these artifacts: an upstream feed may legitimately carry an IPv4 literal, and
# that is still not something to normalise.
#
# NOTE ON THE EXPORT PASS. REPLACEMENTS runs over every text file, including these
# artifacts. If a future feed ever contains a rewritten token the artifact's exported
# bytes stop matching the pin and it loses the exemption -- the export audit then
# reports it. That is the fail-closed direction on purpose: a rewritten security
# artifact means the public copy would block different hosts than the private one, and
# that divergence deserves a human, not a silent pass.

# The AFFILIATION class: employer/brand tokens whose presence is a claim about who
# this repo is, not a secret. These are the ONLY needles a generated upstream feed can
# legitimately carry, so these are the only ones an artifact exemption may drop.
AFFILIATION_NEEDLES = {"samsung"}

# Declared generated-artifact classes. Deliberately enumerated (not a pattern anyone can
# opt into): adding a class is a reviewed edit to this file, which is the same bar as
# adding a needle. `globs` are repo-relative POSIX paths -- both the root-hoisted and the
# `fak/`-nested export layouts, spelled out rather than a leading `*` so a sibling
# directory cannot match. `manifest` resolves BESIDE the artifact, so the class works
# unchanged under either layout.
GENERATED_ARTIFACT_CLASSES = [
    {
        "id": "egresslist-bundled-filter-list",
        "globs": (
            "internal/egresslist/lists/*.txt",
            "fak/internal/egresslist/lists/*.txt",
        ),
        "manifest": "manifest.json",
        "header": "! fak bundled egress filter list - GENERATED by "
                  "`fak egresslist refresh`; do not hand-edit.",
        # The ONLY line shape internal/egresslist.RenderArtifact emits for upstream
        # data: a domain-anchor block rule, optionally allow-prefixed. The lookahead
        # requires a dot and the class is [a-z0-9.-], which is exactly the package's
        # own isPlausibleHost predicate -- so "a line the generator could have
        # produced" is decided by the generator's grammar, not by a guess.
        "rule_re": re.compile(r"^(?:@@)?\|\|(?=[a-z0-9.-]*\.)[a-z0-9.-]+\^$"),
    },
]


# The UTF-8 byte-order mark, as the decoded character. Spelled with chr() rather than a
# literal so the mark itself never sits in this source (an invisible character in a
# security denylist is exactly the kind of thing nobody can review).
_BOM = chr(0xFEFF)


def _canonical_checksum(text: str) -> str:
    """hex(sha256) of `text` with a UTF-8 BOM dropped and CRLF normalized to LF.

    Byte-identical to internal/egresslist.Checksum, which is what pins the artifact in
    lists/manifest.json. The normalization is what makes the pin portable across a
    Windows and a Linux checkout of the same artifact; re-deriving it here (rather than
    hashing raw bytes) is what lets the gate verify the SAME pin the Go side verifies.
    """
    t = text[1:] if text.startswith(_BOM) else text
    return hashlib.sha256(t.replace("\r\n", "\n").encode("utf-8")).hexdigest()


def _artifact_class(rel: str):
    """Return the declared class whose path glob matches `rel`, else None."""
    norm = rel.replace("\\", "/")
    for cls in GENERATED_ARTIFACT_CLASSES:
        for pat in cls["globs"]:
            if fnmatch.fnmatch(norm, pat):
                return cls
    return None


def generated_artifact_class(rel: str, read_rel):
    """Return the exemption class for `rel` iff it is a GENERATED, checksum-PINNED
    third-party artifact -- else None.

    `read_rel(path)` returns the decoded text of a repo-relative path, or None. Callers
    supply the reader for THEIR view of the tree: the git INDEX for the pre-commit gate
    (the bytes the commit would actually record, not whatever the worktree holds), a rev
    for the CI range gate, and the filesystem for the tree scan and the post-scrub
    export audit.

    Fails CLOSED at every step: an unreadable artifact, a missing/short header, an
    absent or unparseable manifest, a manifest with no entry for this name, and a
    checksum mismatch all return None, i.e. NO exemption and the strict gate applies.
    """
    cls = _artifact_class(rel)
    if cls is None:
        return None
    norm = rel.replace("\\", "/")
    text = read_rel(norm)
    if not text:
        return None
    first = text.split("\n", 1)[0].rstrip("\r")
    if first.lstrip(_BOM) != cls["header"]:
        return None
    base = norm.rsplit("/", 1)
    name = base[-1]
    ...
    if len(base) < 2:
        return None
    raw = read_rel(base[0] + "/" + cls["manifest"])
    if not raw:
        return None
    try:
        entries = (json.loads(raw) or {}).get("lists") or []
    except (ValueError, TypeError, AttributeError):
        return None
    pinned = ""
    for entry in entries:
        if isinstance(entry, dict) and entry.get("name") == name:
            pinned = str(entry.get("sha256") or "").strip().lower()
            break
    if not pinned or pinned != _canonical_checksum(text):
        return None
    return cls


def _is_upstream_data_hit(cls, needle: str, line: str) -> bool:
    """True iff ONE needle hit is upstream data inside an exempt artifact.

    Both halves are required. The needle must be in the AFFILIATION class -- the only
    class an upstream feed can legitimately carry -- and the LINE must be one the
    generator emits for an upstream rule. A brand name in the artifact's header prose,
    or on any line that is not a rendered rule, stays a hit.
    """
    if cls is None or needle.lower() not in AFFILIATION_NEEDLES:
        return False
    return bool(cls["rule_re"].match(line.strip()))


def _content_hit_is_upstream_data(cls, needle: str, text: str) -> bool:
    """Whole-FILE twin of `_is_upstream_data_hit`, for the non-diff scans.

    The tree scan and the post-scrub export audit match a needle against whole-file
    CONTENT, so the per-line shape test is applied to every line carrying it: the hit is
    upstream data only if EVERY occurrence sits on a rendered rule line. One occurrence
    anywhere else and the file is reported, which is what keeps a hand-written claim
    smuggled into a re-pinned artifact visible.
    """
    if cls is None or needle.lower() not in AFFILIATION_NEEDLES:
        return False
    low = needle.lower()
    ...
    for line in text.splitlines():
        if low in line.lower():
            saw = True
            if not cls["rule_re"].match(line.strip()):
                return False
    return saw


def _disk_reader(root: str):
    """A `read_rel` over a directory tree (the export dir, or a working tree)."""
    def read(rel: str):
...
def _git_reader(root: str, rev: str = ""):
    """A `read_rel` over git objects; `rev=""` reads the staged INDEX (`git show :p`).

    The commit gate scans a DIFF, not the worktree, so the checksum must be verified
    against the bytes the commit would actually record. Reading the index also means an
    artifact whose manifest is already committed (only the artifact restaged) still
    resolves its pin. Results are cached: the manifest is re-read once per file otherwise.
    """
    import subprocess
    ...
    def read(rel: str):
        if rel not in cache:
            ...
            cache[rel] = out.stdout if out.returncode == 0 else None
        return cache[rel]
...
def _scan_added_lines(diff_text: str, needles: list[str] | None = None, artifact_reader=None):
    ...
    add the pulled-private REAL needles. ``artifact_reader`` is an optional
    `read_rel` (see `generated_artifact_class`) that lets the scan verify a
    GENERATED, checksum-pinned third-party artifact and drop the AFFILIATION-class
    hits that are upstream data rather than a claim; omit it and the scan is strict
    for every file, exactly as before. Returns a list of
    ...
    artifact_cls = None  # current file is a checksum-pinned generated artifact (#5405)
    ...
    for line in diff_text.splitlines():
        if line.startswith("+++ b/"):
            ...
            # A GENERATED, checksum-pinned third-party artifact (#5405). Resolved once
            # per file, from the caller's view of the tree, and only for the AFFILIATION
            # needle class on a rendered-rule line -- the PATH scan just below stays
            # strict, because a needle in a FILE NAME is this repo naming something, not
            # upstream data.
            artifact_cls = (
                None if (self_ref or artifact_reader is None)
                else generated_artifact_class(current_file or "", artifact_reader)
            )
            if not self_ref:
            ...
            if not self_ref:
                for needle in needle_list:
                    if needle.lower() in payload_l:
                        if _is_upstream_data_hit(artifact_cls, needle, payload):
                            continue
                        preview = payload.strip()[:80]
                        hits.append((current_file, new_line_no, needle, preview))
                # SHAPES are never exempted: a credential or a real IP inside a
                # generated artifact is still a credential and a real IP.
                for rx, label in AUDIT_REGEXES:
...
def audit_staged(root: str) -> int:
    ...
    if result.returncode != 0:
        ...
        return 2
    # The artifact reader is the INDEX (`git show :path`): the checksum must be verified
    # against the bytes this commit would record, not against the worktree.
    return _report_hits(
        _scan_added_lines(result.stdout, _effective_audit_needles(root), _git_reader(root, "")),
        "staged content",
    )
...
def audit_range(root: str, rev_range: str) -> int:
    ...
    if result.returncode != 0:
        ...
        return 2
    # Verify a generated artifact's pin at the range's END rev -- the state the push
    # would leave on the trunk. A range with no `..` is a single rev; an empty tail
    # (`BASE..`) means HEAD, the same default git itself applies.
    end_rev = rev_range.split("..")[-1].strip() or "HEAD"
    rc = _report_hits(
        _scan_added_lines(
            result.stdout, _effective_audit_needles(root), _git_reader(root, end_rev)
        ),
        f"range {rev_range}",
    )
    if rc == 0:
...
def audit_tree(root: str, as_json: bool = False) -> int:
    ...
    for rel in result.stdout.splitlines():
        ...
        # Same #5405 exemption the commit gate and the export audit apply, read from
        # the working tree (this scan is about what SITS in the tree, not a diff).
        artifact_cls = generated_artifact_class(rel, _disk_reader(root))
        for needle in real_needles:
            if needle.lower() in content_l:
                if _content_hit_is_upstream_data(artifact_cls, needle, content):
                    continue
...
def _post_scrub_audit(export_dir: str) -> int:
    ...
    print("=" * 72)
    misses = 0
    export_reader = _disk_reader(export_dir)
    for dirpath, _d, filenames in os.walk(export_dir):
        for name in filenames:
            full = os.path.join(dirpath, name)
            ...
            content_l = content.lower()
            # The EXPORT half of the #5405 exemption -- the half that makes it real.
            # FLEET_ALLOW_LEAK=1 only silences the COMMIT gate, so an affiliation needle
            # with no REPLACEMENTS rewrite would fail HERE on every future publish. The
            # pin is re-verified against the EXPORTED bytes, so a scrub that rewrote the
            # artifact loses the exemption and gets reported.
            artifact_cls = generated_artifact_class(rel, export_reader)
            for needle in EXPORT_AUDIT_NEEDLES:
                if needle.lower() in content_l:
                    if _content_hit_is_upstream_data(artifact_cls, needle, content):
                        continue
