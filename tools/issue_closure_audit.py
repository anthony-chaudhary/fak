...
def classify_refs(subject: str, body: str) -> dict[int, str]:
    ...
    RESOLVING wins if EITHER the GitHub-closing form (close/fix/resolve #N, in
    subject or body) matches, OR #N appears in the subject line. Otherwise the
    reference is a MENTION (Relates-to, prose/bullet body refs) and never counts
    toward TRUE_RESOLVED.
    """
    subject = subject or ""
    body = body or ""
    out: dict[int, str] = {}
    ...
    resolve_refs = {int(m) for m in _RESOLVE_RE.findall(subject + "\n" + body)}

    for n in _REF_RE.findall(subject + "\n" + body):
        num = int(n)
        if num in resolve_refs or num in subject_refs:
            out[num] = RESOLVING
        else:
            out.setdefault(num, MENTION)
    # A resolving classification always wins over a mention for the same issue.
    for num in resolve_refs | subject_refs:
        out[num] = RESOLVING
    return out
