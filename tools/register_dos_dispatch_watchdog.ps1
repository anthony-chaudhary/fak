...
$act = New-ScheduledTaskAction -Execute 'powershell.exe' `
  -Argument ('-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "{0}" -Target {1} -Interval {2}' -f $Watchdog, $Target, $Interval)
...
Write-Output "installed $TaskName (every 5 min, current-user S4U windowless, Target=$Target Interval=$Interval)"
