...
def _work_rules(n: int, lane: str) -> list[tuple[str, str, str]]:
    ...
    return [
        ("lane-lease",
         ...
         "owns. Choose the mode explicitly: take --mode shared when your files are "
         "DISJOINT from other in-flight work in the lane (the common case - shared "
         "leases may overlap), and --mode exclusive only for a whole-lane or serial "
         "mutation where no sibling may run at all; `dos lease-lane acquire` defaults "
         "to exclusive, so an omitted --mode over-serializes file-disjoint siblings. "
         "A refusal here is a pause, not your stop condition: acquire the lane "
         ...
         f"dos lease-lane acquire --lane {lane} --mode shared --owner <you>"),
