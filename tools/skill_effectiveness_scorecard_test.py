...
def test_pathdisc_re_accepts_fak_commit_sweep_and_git_paths() -> None:
    # fak commit --path variants
    assert se.PATHDISC_RE.search("fak commit --path <p>")
    assert se.PATHDISC_RE.search("fak commit --path internal/pkg/file.go -m 'msg'")
    assert se.PATHDISC_RE.search("fak commit --preview -m 'msg' --path <paths>")
    # fak sweep variants
    assert se.PATHDISC_RE.search("fak sweep")
    assert se.PATHDISC_RE.search("fak sweep --apply --lane core -m 'msg'")
    # git commit ... -- <paths> variants
    assert se.PATHDISC_RE.search("git commit -s -m 'msg' -- <paths>")
    assert se.PATHDISC_RE.search("git commit -s -m 'msg' -- internal/foo.go")
    assert se.PATHDISC_RE.search("git commit -s -F msg -- README.md")
    assert se.PATHDISC_RE.search("commit-by-path")
    assert se.PATHDISC_RE.search("never git add -A")
    assert se.PATHDISC_RE.search("explicit path")
    # negative cases that lack discipline
    assert not se.PATHDISC_RE.search("git commit -m 'undisciplined'")
    assert not se.PATHDISC_RE.search("git add . && git commit -m 'undisciplined'")
    assert not se.PATHDISC_RE.search("fak commit -m 'missing path'")


def test_commits_re_identifies_real_commit_invocations() -> None:
    assert se.COMMITS_RE.search("git commit -s -m 'msg'")
    assert se.COMMITS_RE.search("fak commit --path <p>")
    assert se.COMMITS_RE.search("fak commit --preview -m 'msg' --path <p>")
    assert se.COMMITS_RE.search("fak sweep --apply --lane core")
    # non-committing mentions or preview-only
    assert not se.COMMITS_RE.search("fak commit --preview")
    assert not se.COMMITS_RE.search("fak sweep --json")
    assert not se.COMMITS_RE.search("does not authorize git commits")
