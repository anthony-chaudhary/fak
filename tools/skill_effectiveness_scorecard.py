...
trigger, a resolving reference, a witness step — never by spraying a keyword.
  ...
  TRUST      — the model/operator can trust what the skill ships
    commit_discipline    a skill that commits names the explicit-path discipline
                         (fak commit --path / fak sweep / commit-by-path / `-- <paths>` /
                         never `git add -A`) — the shared-trunk safety rule that stops a
                         skill sweeping a peer's staged files
...
# commit INVOCATION (`git commit -s/-m/-F …`, `fak commit --path …`, `fak sweep --apply …`),
# not a prose mention ("does not authorize git commits", "operator handles git commit")
# — every committing skill writes the command with a flag, so the `-` is what
# separates run from mention.
COMMITS_RE = re.compile(
    r"git commit\s+-|fak commit\s+--path|fak commit\b[^\n]*--path|fak sweep\s+--apply", re.I)
# The explicit-path discipline phrases this shared trunk demands:
# rewards fak commit --path / fak sweep AND git commit ... -- <paths>.
PATHDISC_RE = re.compile(
    r"explicit path|by path|commit-by-path|--\s*<|git add -A|pathspec|<paths>|<explicit"
    r"|fak commit\s+--path|fak commit\b[^\n]*--path|fak sweep"
    r"|git commit\b[^\n]*--\s+\S+", re.I)
...
def kpi_commit_discipline(facts: list[dict]) -> dict[str, Any]:
    ...
    failed = [f"{s['name']}: commits but never names the explicit-path discipline "
              "(fak commit --path / fak sweep / commit-by-path / `-- <paths>` / never `git add -A`) — it can sweep a "
