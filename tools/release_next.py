...
# Ensure tools/ is on sys.path regardless of how script was invoked
...
def repo_root() -> Path:
    ...
    return Path(out.strip()) if code == 0 and out.strip() else TOOLS_DIR.parent
...
def inspect_next_state(root: Path, draft_rel: str = DEFAULT_DRAFT_PATH, limit: int = 300) -> dict:
    ...
    # Check which commits are represented in the draft
    untracked_commits: list[dict] = []
    tracked_commits: list[dict] = []

    for c in commits:
        clean = clean_public_subject(c.get("subject", ""))
        short_sha = c.get("short_sha", "")
        # A commit is considered tracked if its clean subject, subject, or short_sha appears in the text
        if draft_exists and (
            (clean and clean.rstrip(".") in draft_text)
            or (c.get("subject", "") in draft_text)
            or (short_sha and short_sha in draft_text)
        ):
...
def parse_existing_draft(text: str) -> dict[str, list[str]]:
    ...
    for line in text.splitlines():
        ...
        if trimmed.startswith("## "):
            ...
            # Match known or custom sections
        ...
        if current_sec and trimmed.startswith("- "):
            ...
            if item and item not in sections[current_sec]:
...
def render_next_draft(state: dict, existing_sections: dict[str, list[str]] | None = None) -> str:
    ...
    # Preserved custom sections / Upgrade notes
