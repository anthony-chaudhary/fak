...
def render_doc(payload: dict[str, Any], stamp: str = "") -> str:
    ...
    lines += [
        ...
        "",
        f"**intent_literal_debt {c['intent_literal_debt']}** (unbounded; drive to 0 - the primary "
        f"metric) - {c['rows']} positioned metric(s) over {c['surfaces']} surface(s) - "
        f"{c['divergent']} with a divergent denominator - {c['soft_signals']} coverage advisory. "
        f"Legacy bounded score: {c['score']}/100, grade {c['grade']} (saturates once every "
        f"divergence is disclosed; not the driver).",
        "",
