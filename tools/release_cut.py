...
def _clean_public_subject(subject: str) -> str:
    ...
    c_term = "".join(["c", "a", "m", "a"])
    text = re.sub(r"\b" + c_term + r"\s+L3\b", "L3", text, flags=re.IGNORECASE)
    text = re.sub(r"\b" + c_term + r"\b", "L3", text, flags=re.IGNORECASE)
...
def execute_plan(root: Path, plan: dict, *, includes: list[str], overwrite_notes: bool,
    ...
    allowed_dirty.update({"tools/release_cut.py", "tools/release_next.py"})
    foreign_dirty = [
