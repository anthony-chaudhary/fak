...
REQUIRED_PATHS = [
    ("fak/cmd/fak", "the binary the notebooks build (go build ./cmd/fak)"),
    ("fak/examples/customer-support-readonly-policy.json", "Tier-0 preflight DENY/ALLOW demo"),
    ("fak/scripts/fetch-model.sh", "Notebook B real-weights export (Tier 2b)"),
    ("deploy/prove.sh", "the live-demo endpoint the zero-install cell mirrors"),
]
...
# Real adjudication verdicts in ~3s. The endpoint is open by design (deploy/prove.sh).
...
- **Tier 2 — the fused in-kernel model** (`fak serve --engine inkernel`, real SmolLM2 via
  `fak/scripts/fetch-model.sh`): see **`fak-inkernel.ipynb`** in this folder, or
  `fak/GETTING-STARTED.md` §4.
- **Policy authoring:** `fak/POLICY.md` · **Architecture:** `fak/ARCHITECTURE.md`
- **What's real vs. simulated vs. stub:** `fak/CLAIMS.md` (every claim machine-tagged)
- **The live demo you hit in cell 3:** `deploy/prove.sh`
...
def verify_refs():
    ...
    src = "\n".join(p.read_text(encoding="utf-8", errors="ignore")
                    for p in (ROOT / "fak" / "cmd" / "fak").glob("*.go")) if (ROOT/"fak"/"cmd"/"fak").is_dir() else ""
    for verb in REQUIRED_VERBS:
        if f'"{verb}"' not in src:
            problems.append(f"verb '{verb}' not found in fak/cmd/fak/*.go (notebook invokes it)")
    return problems
