...
if [ -n "$BIND_ALL" ]; then
  ...
  log "  PowerShell: . scripts\\connect-fak-node.ps1 -GatewayHost $TAILSCALE_IP -GatewayKey \"$GATEWAY_KEY\""
  log "  bash/zsh:   source scripts/connect-fak-node.sh $TAILSCALE_IP $GATEWAY_KEY"
  log "  (note: must be sourced / dot-sourced so env vars reach your shell)"
  log ""
