...
#
# Usage:
#   GitHub:  ./apply.sh github  <owner>/<repo>                 (needs `gh auth login`)
...
# the push rules. Edit github-ruleset.json's status-check contexts before applying.
...
case "$forge" in
  github)
    ...
    gh api -X POST "repos/${target}/rulesets" \
      --input "${here}/github-ruleset.json" \
      -H "Accept: application/vnd.github+json"
    echo "applied fak-trunk-laws ruleset to github.com/${target}" >&2
    ;;
  gitlab)
...
# the source of truth for the predicate set; this mirrors it declaratively.
#
