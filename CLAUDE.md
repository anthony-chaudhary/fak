...
- **Default is to ship green work and sync safely unprompted** — do more work by default:
  ...
  Dual-repo synchronization invariant & Companion Dev Harness: when working across both repos or touching shared interfaces (`pkg/*`), keep both `fak` and `fak-private` synchronized (fetch both remotes and `refs/fak/locks/*`, fast-forward both, sync `go.work` to prevent module skew, and audit with public leak scrub `tools/scrub_public_copy.py --audit-staged` or `fak-dev audit-leak --staged`). Local development in `fak` is first-class and companion-governed: pre-commit hooks in `fak` automatically run 5-gate boundary checks (`cmd/fak-boundary`) and secret leak audits when `fak-private` is present. In-tree CLI commands `fak-dev gate`, `fak-dev provenance`, `fak-dev audit-leak`, and `fak-dev ticket` bridge directly to companion tools without requiring directory switches. Cross-repo issue quoting rule: bare `#<num>` strictly denotes a public `fak` issue; quoting a `fak-private` issue in public `fak` MUST be explicitly qualified as `fak-private#<num>` (or `anthony-chaudhary/fak-private#<num>`) to ensure provenance is unambiguous. Full default + verify command in [`AGENTS.md`](AGENTS.md).
