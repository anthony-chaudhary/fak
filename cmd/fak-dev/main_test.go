...
func TestRunDispatchesDevelopmentDiagnosticsUsage(t *testing.T) {
	for _, verb := range []string{"amd-gpu-facts", "amd-setup", "amd-gotchas", "amd-sim", "amd-strix-package", "amd-strix-validate", "amd-strix-probe", "amd-gpudirect", "cuda-gpudirect", "commit-subject-coverage"} {
