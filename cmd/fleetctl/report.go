...
// JSON files per box from live state; this public tool reads, folds, renders, and
// scores them. The boundary is a DATA contract, not a code import: the private side
// never links this binary and this binary never knows the lab transport. A report
// carries only GENERIC operational state — a state word, a version, an age, a free
// note — never a host, a token, or a transcript.
//
// CONTRACT FOR THE PRODUCER (the private bridge):
