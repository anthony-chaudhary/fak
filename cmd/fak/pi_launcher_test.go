...
// regress. Every test that calls runPi must call this first.
...
// operator's harness defaultModel (and baseUrl to a dead test port) on every `go test
// ./cmd/fak/` run, so a bare `pi` launch regressed away from the live fak router.
func TestRunPiWritesOnlyUnderIsolatedHome(t *testing.T) {
	isolated := t.TempDir()
	t.Setenv("PI_CODING_AGENT_DIR", isolated)

	// Real-home sentinels: if runPi ignores the isolation these gain a file.
	...
	// The isolated home must have been written.
	...
	// The real home must be byte-identical (or still absent).
	if settingsErr == nil {
