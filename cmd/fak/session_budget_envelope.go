...
// session_spend.go), Pace, TimeBudget, and the throughput envelope (#2762: the
// expected pace-shaping rate plus the min sustained-rate floor DebitUsage drains at).
func applyGuardSessionBudgetEnvelope(tbl *session.Table, traceID string, env session.BudgetEnvelope, hasEnvelope bool, contextOverride *int, effectiveContext int, wallLimit time.Duration, now time.Time) {
	...
	if hasEnvelope {
		...
		if !env.Throughput.IsZero() {
			tbl.SetThroughputBudget(traceID, env.ThroughputBudget())
		}
	}
	if wallLimit > 0 {
