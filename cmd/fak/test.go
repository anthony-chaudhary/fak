...
//	fak test scorecard           run the scorecard ratchet gate
...
func runTest(stdout, stderr io.Writer, argv []string) int {
  ...
  fak test scorecard           scorecard ratchet gate
...
`)
	...
	if len(args) > 0 {
		...
		case "scorecard":
			return runTestScorecard(stdout, stderr, args[1:], *json, *dry || *dry2)
		case "affected":
			subArgs := args[1:]
