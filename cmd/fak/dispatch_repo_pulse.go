...
func dispatchRepoPulseOrientation(root string) (string, error) {
	...
	if r.Verdict != "PASS" {
		return "", fmt.Errorf("repo pulse verdict %s", r.Verdict)
	}
	return fmt.Sprintf("%s\nreceipt: inline_tokens=%d folded_tokens=%d saved_tokens=%d parent_tool_turns=%d->%d tool_turns_skipped=%d journal_rows=%d", r.Collapsed, r.InlineTokens, r.FoldedTokens, r.SavedTokens, r.ParentToolTurnsInline, r.ParentToolTurnsCollapsed, r.ToolTurnsSkipped, r.JournalRows), nil
}
