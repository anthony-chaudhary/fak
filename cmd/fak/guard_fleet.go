...
// installGuardFleetProvider wires the live fleet pull source onto srv for the lifetime of ctx.
// Spine startup is best-effort: when config or multicast is unavailable, the returned provider
// remains the disk-only fold, so guard startup never depends on LAN discovery.
func installGuardFleetProvider(srv *gateway.Server, ctx context.Context, logf fleetspine.Logf) {
	srv.SetSessionFleetProvider(newGuardFleetProviderMaybeSpine(ctx, logf))
}
