...
func TestArchitectureTextRendersRedundantDependencies(t *testing.T) {
	root := t.TempDir()
	mustWriteArchitectureFile(t, root, "internal/architest/architest_test.go", `package architest
var tier=map[string]int{"a":3,"b":2,"d":1}
...
`)
	...
	code := runArchitecture(&stdout, &stderr, []string{"--workspace", root})
	if code != 0 {
	...
	stdout.Reset()
	stderr.Reset()
	if code := runArchitecture(&stdout, &stderr, []string{"--workspace", root, "--leaf", "b", "--json"}); code != 0 {
		t.Fatalf("scoped JSON code=%d stderr=%s", code, stderr.String())
	}
	var report archreport.Report
...
func TestArchitectureLateralEdgeConnectivityPartitionPolicy(t *testing.T) {
	diff := archreport.ReportDiff{LateralEdgeConnectivityChanges: []archreport.LateralEdgeConnectivityChange{{
		...
		BeforeSourceSide: []string{"a"}, BeforeSinkSide: []string{"b", "c", "d"},
