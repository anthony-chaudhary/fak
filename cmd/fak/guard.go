...
func cmdGuard(argv []string) {
	...
	positiveResidualSubstitution := fs.Bool("positive-residual-substitution", false, "replace compacted negated/stale history with its positive residual; original bytes remain available through ctxrestore (default off)")
	...
	*tokPath = pathutil.ExpandTilde(*tokPath)
	...
	srv, err := gateway.New(gateway.Config{
		...
		PositiveResidualSubstitution: *positiveResidualSubstitution,
