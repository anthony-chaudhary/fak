...
func TestWorktreeWorkerLandPrepareAccept(t *testing.T) {
	...
	if head := strings.TrimSpace(worktreeWorkerTestGit(t, repo, "rev-parse", "refs/heads/main")); head != receipt.CandidateSHA {
...
func TestWorktreeWorkerLandPrepareInheritsGOFLAGS(t *testing.T) {
	repo, wt, base, paths := newPreparedCLIWorkerFixture(t, false)
	probePath := filepath.Join(wt, "pkg", "goflags_probe.go")
	probe := "//go:build inherited_prepare_probe\n\npackage pkg\n\nvar _ = inheritedPrepareMissing\n"
	if err := os.WriteFile(probePath, []byte(probe), 0o644); err != nil {
		t.Fatal(err)
	}
	worktreeWorkerTestGit(t, wt, "add", "pkg/goflags_probe.go")
	worktreeWorkerTestGit(t, wt, "commit", "-qm", "test(calc): add inherited GOFLAGS probe (fak calc)")
	paths = append(paths, "pkg/goflags_probe.go")
	t.Setenv("GOFLAGS", "-tags=inherited_prepare_probe")

	var stdout, stderr bytes.Buffer
	res, code := runWorktreeWorkerLand(&stdout, &stderr, preparedCLIArgs("prepare", repo, wt, base, paths))
	out := requireSinglePreparedLandJSON(t, stdout.Bytes())
	if code == 0 || res.OK || out.OK {
		t.Fatalf("prepare ignored inherited GOFLAGS: result=%+v json=%+v stderr=%s", res, out, stderr.String())
	}
	if !strings.Contains(res.Detail, "inheritedPrepareMissing") {
		t.Fatalf("prepare failure did not come from GOFLAGS-selected file: %+v", res)
	}
}

func TestWorktreeWorkerLandPrepareFailureJSON(t *testing.T) {
	t.Run("missing_worktree", func(t *testing.T) {
		repo, _, base, paths := newPreparedCLIWorkerFixture(t, false)
		args := preparedCLIArgs("prepare", repo, filepath.Join(t.TempDir(), "missing"), base, paths)
		args = append(args, "--verify", "off")
		var stdout, stderr bytes.Buffer
		res, code := runWorktreeWorkerLand(&stdout, &stderr, args)
		out := requireSinglePreparedLandJSON(t, stdout.Bytes())
		if code == 0 || res.OK || out.OK {
			t.Fatalf("missing worktree prepare succeeded: result=%+v json=%+v stderr=%s", res, out, stderr.String())
		}
	})

	t.Run("outside_git_without_root", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		res, code := runWorktreeWorkerLand(&stdout, &stderr, []string{"prepare", "--worktree", filepath.Join(outside, "missing"), "--verify", "off"})
		out := requireSinglePreparedLandJSON(t, stdout.Bytes())
		if code == 0 || res.OK || out.OK {
			t.Fatalf("outside-git prepare succeeded: result=%+v json=%+v stderr=%s", res, out, stderr.String())
		}
	})
}

func requireSinglePreparedLandJSON(t *testing.T, raw []byte) worktreeWorkerPreparedLandOut {
	t.Helper()
	trimmed := bytes.TrimSpace(raw)
	if len(trimmed) == 0 || bytes.Contains(trimmed, []byte("\n")) {
		t.Fatalf("prepared mode must emit exactly one compact JSON object, got %q", raw)
	}
	dec := json.NewDecoder(bytes.NewReader(raw))
	var out worktreeWorkerPreparedLandOut
	if err := dec.Decode(&out); err != nil {
		t.Fatalf("decode prepared JSON: %v; raw=%q", err, raw)
	}
	var extra any
	if err := dec.Decode(&extra); err != io.EOF {
		t.Fatalf("prepared mode emitted more than one JSON value: err=%v extra=%v raw=%q", err, extra, raw)
	}
	return out
}

func TestWorktreeWorkerLandAcceptRejectsChangedExpectation(t *testing.T) {
	tests := []string{"paths", "verification", "tags", "goflags", "fix-policy", "stale-parent"}
	for _, name := range tests {
		t.Run(name, func(t *testing.T) {
			t.Setenv("GOFLAGS", "-tags=prepared-one")
			repo, wt, base, paths := newPreparedCLIWorkerFixture(t, name == "fix-policy")
			args := preparedCLIArgs("prepare", repo, wt, base, paths)
			args = append(args, "--symptom-tags", "metal")
			if name == "fix-policy" {
				args = append(args, "--unsafe-skip-symptom-witness")
			}
