...
func runSupportMaturityScorecard(stdout, stderr io.Writer, argv []string) int {
	...
	return emitScorecard(stdout, stderr, "fak support-maturity-scorecard", supportmaturityscore.DebtKey, payload,
		*comparePath, *asJSON, *asMarkdown, standardScorecardDoc(
			"fak support-maturity scorecard",
			"fak's deterministic support-maturity scorecard folds the generated model x backend coverage matrix into support_maturity_debt, coverage percentage, and an A-F grade.",
			"Support-maturity scorecard",
			"Auto-generated by `fak support-maturity-scorecard --markdown`. Do not hand-edit; re-run the tool.",
			"A cell is fully mature only when the generated coverage matrix marks it SUPPORTED; proof-path-only, fenced, and undefined cells remain honest but incomplete support.",
			supportmaturityscore.DebtKey, fmt.Sprintf(" - %v/%v cells supported", payload.Corpus["supported"], payload.Corpus["cells"])))
}
