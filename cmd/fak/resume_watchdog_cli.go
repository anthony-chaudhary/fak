...
func runResumeWatchdog(stdout, stderr io.Writer, argv []string) int {
	...
	planFlag := fs.String("plan", "", "explicit plan file for a TARGETED run: consume exactly this file, skip the registry refresh, and fail closed if it is missing/unreadable/empty — the shared resume_plan.json can be regenerated by a concurrent fleet refresh between an operator's write and this tick's read (#2367)")
	...
	if targetedPlan != "" {
		...
		if len(plan) == 0 {
			note("PLAN RACE GUARD: targeted plan %s is missing, unreadable, or empty — refusing to act (#2367)", targetedPlan)
			return 1
		}
	...
	for _, p := range plan {
		...
		// latch lies once the session dies AGAIN — the sweep re-plans it every tick and
		...
		if resume.CountAttempts(hist) > 0 &&
			...
			if revived, released := resume.ReviveOutcome(outcome, ev); released {
				outcome = revived
				note("  REVIVE %s — resume took once, but no live process holds it and the transcript has been idle %s: stale once-latch released, session re-eligible (#2368)",
...
// collector `fak ps` uses) — one snapshot per tick, taken lazily. Injectable seam for
...
// rwProcScan lazily snapshots the process table once per tick — only a row that would
