...
func runLoopScore(stdout, stderr io.Writer, argv []string) int {
	...
	if code := emitJSONOrRender(stdout, stderr, "fak loop-score", *asJSON, payload, func(w io.Writer) {
		if *asMarkdown {
			fmt.Fprint(w, loopscore.Markdown(payload))
		} else {
			fmt.Fprintln(w, loopscore.Render(payload))
		}
	}); code != 0 {
		return code
	}
