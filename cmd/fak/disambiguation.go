...
func runDisambiguation(stdout, stderr io.Writer, args []string) int {
	...
	default:
		fmt.Fprintf(stderr, "fak disambiguation: unknown command %q (want schema, query, search, reverse, cli-source, docs, explain, ownership, freshness, provenance, policy-source-self-test, fleet-source-self-test, runtime-source-self-test, reason-source-self-test, cache-source-self-test, session-source-self-test, stale-symbols-self-test, or coverage-self-test)\n", args[0])
		return 2
	}
}
