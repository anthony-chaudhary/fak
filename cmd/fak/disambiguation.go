...
func runDisambiguation(stdout, stderr io.Writer, args []string) int {
	...
	default:
		fmt.Fprintf(stderr, "fak disambiguation: unknown command %q (want diff, metrics, schema, query, search, reverse, cli-source, docs, explain, ownership, freshness, provenance, ownership-source-self-test, go-source-self-test, lifecycle-source-self-test, claims-source-self-test, policy-source-self-test, fleet-source-self-test, runtime-source-self-test, reason-source-self-test, cache-source-self-test, session-source-self-test, stale-symbols-self-test, or coverage-self-test)\n", args[0])
		return 2
	}
}
