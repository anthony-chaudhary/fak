...
var servewiringData = []wiringRow{
	...
	{"elideresult", "--elide-result-bytes", "ElideResultBytes", verdictWired, "internal/gateway/messages.go:maybeElideAnthropicRaw", "shrinks an old oversized tool_result body to a bounded head+tail in the un-cached middle, cache head byte-identical; DEFAULT-ON at gateway.DefaultElideResultBytes (16KB), pass 0 to disable"},
