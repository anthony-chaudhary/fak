...
var servewiringData = []wiringRow{
	...
	{"routealiases", "--route-aliases", "RouteAliases", verdictOffByDefault, "internal/gateway/gateway.go:resolveRoute", "rewrites the requested model name through a runtime alias chain to a terminal model id BEFORE the roster binds it (#11091); hot-editable via /v1/fak/route/aliases; off when no registry file is named"},
