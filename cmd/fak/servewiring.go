...
var servewiringData = []wiringRow{
	...
	{"allowlan", "--allow-lan", "AllowLAN", verdictOffByDefault, "internal/gateway/gateway.go:234", "allows unauthenticated requests from private/local networks (RFC 1918, link-local, loopback); off by default"},
	...
	{"sessiontable", "(host table, default-on)", "Table", verdictWired, "internal/gateway/gateway.go:398", "binds the shared serve session table to admission, management reads and transitions, revision feeds, and native termination signals"},
	...
	{"controlingress", "FAK_CONTROL_INGRESS_JOURNAL + --require-key-env/--key-principal", "ControlIngress", verdictOffByDefault, "cmd/fak/serve.go:openServeControlIngress", "explicitly enabled only when FAK_CONTROL_INGRESS_JOURNAL names the dedicated journal and a credential door exists; unset stays nil/503. Before HTTP bind, Unix may O_EXCL-create then file+directory sync it; Windows refuses first creation and requires a pre-provisioned regular file because portable directory-entry durability is unavailable. Authenticated cancel asynchronously takes the existing ControlSession terminating transition; other actions return unavailable until proven. Authentication does not prove human origin, C0 isolation, descendant quiescence, or UI latency"},
	{"speculativemode", "--speculative", "SpeculativeMode", verdictWired, "internal/gateway/chat_completions.go:237 (speculativeModeSelected)", "the STRUCTURAL speculative-decode selector (#1891): the DEFAULT front door sets the fast route by construction instead of leaving it env-only. serve.go defaults to \"mtp\" and guard.go carries the same flag, so speculativeModeSelected consults cfg.SpeculativeMode first — shouldEnableMetalMTP admits the resident Metal/Vulkan MTP draft-verify-rollback path and shouldEnableNGramSpeculative admits the prompt n-gram path where each mode's admissibility envelope holds (Metal+hybrid for MTP). FAK_SPECULATIVE / SPECULATIVE remain the operator override and win when set; empty leaves the route unselected; non-Metal/non-hybrid deployments are untouched. Covered by internal/gateway/speculative_mode_structural_test.go"},
	{"streamsoftprogress", "--stream-soft-progress-timeout", "StreamSoftProgressTimeout", verdictOffByDefault, "internal/agent/stream_stall.go:457 (streamSoftProgressWindow)", "the streaming SOFT no-progress DIAGNOSTIC deadline (#10638): a STREAMING turn that has not advanced for this long (same progress definition as --stream-progress-timeout, keepalives ignored) gets a content-free elapsed-since-progress / retry-attempt receipt (soft-stalls.jsonl under --stream-incident-dir) and KEEPS RUNNING, while --stream-progress-timeout remains the hard client-survivable ceiling that actually ends it. Carried verbatim onto every proxy planner and resolved by agent.(*HTTPPlanner).streamSoftProgressWindow: 0 (the serve default) derives hard/3 so the diagnostic always has lead time, a NEGATIVE value disables it, and an out-of-band or non-earlier positive value falls back to the derived default. OFF by default (derived, not a fixed window). Streaming proxy path only; inert on the buffered turn and the offline mock planner"},
}
