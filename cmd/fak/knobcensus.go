...
// with its disposition, owner epic, and file:line provenance. --json for tooling;
...
func indexKnobs(stdout, stderr io.Writer, root string, asJSON bool) int {
	...
	fmt.Fprintf(tw, "SURFACE\tNAME\tVERDICT\tDISPOSITION\tOWNER\tPROVENANCE\n")
	for _, k := range census.Knobs {
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%s:%d\n",
			k.Surface, k.Name, k.Verdict, k.Disposition, k.OwnerEpic, k.File, k.Line)
	}
