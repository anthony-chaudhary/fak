...
const guardStopHookContinueReason = "fak guard: your previous turn proposed only tool call(s) the capability floor refused, so it ended without action (reported upstream as end_turn). Do NOT re-propose a refused call unchanged — pick an ALLOWED alternative and continue the task. Most refusals are MODEL-FIXABLE by RESHAPING, not a dead end: a SELF_MODIFY deny means the floor saw a guarded path (e.g. VERSION, .dos/, internal/…) named in your command beside a write — even READING that path next to an unrelated write (`… > /tmp/out`) trips it — so re-issue the command WITHOUT naming the guarded path alongside the write (split the read from the write, or drop the mention). If there is genuinely no allowed way to make progress, say so explicitly and then stop."
