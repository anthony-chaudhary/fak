...
var serveHelpCategories = []serveHelpCategory{
	{name: "start", summary: "Listener, deployment manifest, and provider connection.", flags: strings.Fields("addr stdio config print-effective-config print-features provider base-url model opencode opencode-config write-opencode-config pi pi-config write-pi-config pi-config-path claude claude-config write-claude-config codex codex-config write-codex-config codex-config-path api-key-env require-key-env allow-lan stream-progress-timeout expose key-principal unsafe-allow-unauthenticated-bind")},
	{name: "model", summary: "Local engines, weights, reference interop, and hardware.", flags: strings.Fields("engine backend gguf tokenizer qwen38-runtime llama-server llama-startup-timeout cuda-graph cpu-offload-experts n-cpu-moe gpu-layers native-gpu-layers gpudirect-overflow metal memory-governor expert-parallel tensor-parallel replica-base-url vulkan-q4k-profile vulkan-stage-q4k mock")},
	{name: "cache", summary: "Shared engine-cache attachment and cache correctness.", flags: strings.Fields("engine-cache-engine engine-cache-base-url engine-cache-admin-key-env engine-cache-idle-timeout engine-cache-require-exact-span remote-kv-mode remote-kv-backend remote-kv-url remote-kv-token remote-kv-timeout")},
	{name: "context", summary: "Context budgets, compaction, elision, and reuse.", flags: strings.Fields("ctx-view-budget compact-history-budget positive-residual-substitution compact-anchor-head assume-session-turns elide-result-bytes elide-stale-reads vcache-anchor defer-cold-tools defer-tools tool-ceiling native-context-tokens context-budget-tokens ctx max-total-tokens max-batch-prefill-tokens reset-on-budget")},
	{name: "policy", summary: "Policy, routing, plans, and invalidation.", flags: strings.Fields("policy profile policy-canary-turns policy-check plan-json vdso invalidation route-manifest route-accounts")},
