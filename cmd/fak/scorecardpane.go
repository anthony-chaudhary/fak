...
func cmdScorecardPane(argv []string) {
	if len(argv) == 0 {
		fmt.Fprintln(os.Stderr, "usage: fak scorecard control-pane [--json|--check|--pin] | portfolio-discovery [--workspace PATH] | qa-dogfood [--json|--existing-json PATH] | debt-page [--check-doc|--write-doc]")
		os.Exit(2)
	}
	switch argv[0] {
	case "control-pane":
