...
// (#13116) â€” the same predicate the host sizing path (resolveHostServeLoadArm ->
...
func loadServeInKernelModel(modelPath string, backend compute.Backend, cpuOffloadExperts bool, contextBudgetTokens int, expertShard *ggufload.ExpertShard, expertRanks int, fit *serveFitBudget) (inKernelModel *fakmodel.Model, inKernelQ4K bool, loadProfile *gateway.ModelLoadProfile, phase gateway.StartupPhase) {
	...
	// the resident store â€” the residency that fits GLM-5.2 across the fleet (#971). It rides ONLY
	...
	case backend == nil && cpuOffloadExperts && cpuOffloadArm:
		...
		// -tags vulkan (CGO_ENABLED=0), so no device backend registers and backend is nil â€” but
	...
	case backend != nil && cpuOffloadExperts && cpuOffloadArm:
		...
		// dequant-fused k_q4k_gemm tile (#485) â€” skipping the lean path's Q4_K->f32->Q8 round-trip
		...
		// expert pool must be charged that band and not the whole routed set â€” otherwise the
		...
		// experts (~424 GiB for GLM-5.2 Q4_K) are pinned in HOST RAM â€” and a device backend does not
		...
		// fak#13177: on a split-aperture box (64 GiB VRAM carve-out + 62.43 GiB system window) the
		// device-scoped dense transit is destined for the VRAM window and fits it, but judging it
		// against the system window alone refuses it. Thread the device aperture so the split-aware
		// form judges the transit against the window it actually occupies, falling back to the
		// single-window #13171 bound when no distinct device window is known.
		devTotal, devFree, devKnown := compute.DeviceMemoryInfo(backend)
		must(refuseDeviceStagingAgainstReportedAperture(memPlan, hostFit, devTotal, devFree, devKnown))
	...
	case residentQ4K:
		if backend != nil {
			...
			// cpuOffloadExperts arm) â€” all weights are device-resident, so the fit uses the
	...
	case (os.Getenv("FAK_Q4K") != "" && os.Getenv("FAK_Q4K") != "0") && artifactQuant.Q4KResident:
		...
		// copy keyed on the GGUF quant type â€” the SAME resident-K-quant lever the device
		...
		// (dequantâ‰ˆ0) or the slow f32 round-trip. Thread a real profiler here too (parity with
...
// gap â€” the lower-level /v1/fak/syscall route then NL-tokenizes a call's arguments and
...
func resolveServeTokenizer(tokPath, ggufPath string, sinks ...func(gateway.StartupMessage)) (*tokenizer.Tokenizer, bool) {
	...
	if ggufPath != "" {
		...
		// tokenizer (e.g. an SPM-only checkpoint), we keep the MockPlanner fallback â€” pass
