...
func executeDesktopConsoleSoak(
	...
	if rep.OK && rep.RunsCompleted == runs {
		...
		rep.Reason = fmt.Sprintf("tracked console host counts did not return to baseline: %s", renderDesktopConsoleSoakIncreases(increases))
	} else {
...
func writeDesktopConsoleSoakResult(stdout, stderr io.Writer, rep desktopConsoleSoakReport, asJSON bool) int {
	if asJSON {
		...
		fmt.Fprintf(stdout, "  before=%s after=%s\n", renderDesktopConsoleSoakCounts(rep.Before), renderDesktopConsoleSoakCounts(rep.After))
	}
	if !rep.OK {
		return 1
	}
...
func renderDesktopConsoleSoakCounts(counts desktopConsoleSoakProcessCounts) string {
...
func renderDesktopConsoleSoakIncreases(increases map[string]int) string {
	return renderDesktopConsoleSoakCounts(desktopConsoleSoakProcessCounts{Families: increases})
}
