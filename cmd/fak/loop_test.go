...
func TestLoopUsage(t *testing.T) {
	...
	for _, sub := range []string{
		"fak loop - durable long-running loop ledger",
		"fak loop append",
		"fak loop run",
		"fak loop status",
		"fak loop health",
		...
		if !strings.Contains(got, sub) {
			t.Errorf("loopUsage output missing %q", sub)
		}
	}

	var stdout, stderr bytes.Buffer
	if code := runLoop(&stdout, &stderr, nil); code != 2 {
		t.Errorf("runLoop(nil) exit code = %d, want 2", code)
	}
	if !strings.Contains(stderr.String(), "fak loop - durable long-running loop ledger") {
		t.Errorf("runLoop(nil) stderr missing loopUsage output: %s", stderr.String())
	}
	if stdout.Len() != 0 {
		t.Errorf("runLoop(nil) stdout non-empty: %s", stdout.String())
	}

	for _, helpArg := range []string{"-h", "--help", "help"} {
		stdout.Reset()
		stderr.Reset()
		if code := runLoop(&stdout, &stderr, []string{helpArg}); code != 0 {
			t.Errorf("runLoop(%q) exit code = %d, want 0", helpArg, code)
		}
		if !strings.Contains(stdout.String(), "fak loop - durable long-running loop ledger") {
			t.Errorf("runLoop(%q) stdout missing loopUsage output: %s", helpArg, stdout.String())
		}
		if stderr.Len() != 0 {
			t.Errorf("runLoop(%q) stderr non-empty: %s", helpArg, stderr.String())
		}
	}

	stdout.Reset()
	stderr.Reset()
	if code := runLoop(&stdout, &stderr, []string{"unknown_subcmd"}); code != 2 {
		t.Errorf("runLoop(unknown_subcmd) exit code = %d, want 2", code)
	}
	if !strings.Contains(stderr.String(), `unknown subcommand "unknown_subcmd"`) {
		t.Errorf("runLoop(unknown_subcmd) stderr missing unknown subcommand message: %s", stderr.String())
	}
	if !strings.Contains(stderr.String(), "fak loop - durable long-running loop ledger") {
		t.Errorf("runLoop(unknown_subcmd) stderr missing loopUsage output: %s", stderr.String())
	}
}
