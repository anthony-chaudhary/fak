...
func validateServeMaxTotalTokens(sf *serveFlags, admissionTokenBudget int) error {
	maxTokens := sf.effectiveMaxTotalTokens()
	if maxTokens > 0 && admissionTokenBudget > 0 && maxTokens > admissionTokenBudget {
		return fmt.Errorf("max_total_tokens (%d) exceeds admission token budget (%d); decrease --max-batch-prefill-tokens or --max-total-tokens", maxTokens, admissionTokenBudget)
	}
	return nil
}
...
func cmdServe(argv []string) {
	...
	*sf.sessionStatePath = durability.Path
	...
	if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" {
		if *sf.ggufPath == "" && strings.TrimSpace(*sf.baseURL) == "" && len(sf.replicaBaseURLs.Values()) == 0 {
			if *sf.opencode || (sf.pi != nil && *sf.pi) || (sf.claude != nil && *sf.claude) || (sf.codex != nil && *sf.codex) || *sf.metal {
				*sf.ggufPath = "default"
				*sf.metal = true
	...
	*sf.keepAwake = keepAwakeMode
	...
	if err := validateServeNativeContextTokens(*sf.nativeContextTokens); err != nil {
		fmt.Fprintf(os.Stderr, "fak serve: %v\n", err)
		os.Exit(2)
	}
	if sf.isExplicitFlag("native-admission-token-budget") {
		if err := validateServeMaxTotalTokens(sf, policy.TokenBudget); err != nil {
			fmt.Fprintf(os.Stderr, "fak serve: %v\n", err)
			os.Exit(2)
		}
	}
	if *sf.printEffectiveConfig {
		// Effective-config inspection stays model-I/O-free. Validate an explicit
		// scheduler envelope, or the unresolved fallback when no local model was
		// named; a local auto-sized model is checked after header resolution.
		if !sf.isExplicitFlag("native-admission-token-budget") && strings.TrimSpace(*sf.ggufPath) == "" {
			if err := validateServeMaxTotalTokens(sf, policy.TokenBudget); err != nil {
				fmt.Fprintf(os.Stderr, "fak serve: %v\n", err)
				os.Exit(2)
			}
	...
	rt := &serveRuntime{t0: t0, toolPlugins: toolPlugins, toolPreferences: toolPreferences,
		explicitFlags: explicit, nativeAdmissionExplicit: explicit["native-admission-token-budget"], startupPhases: []gateway.StartupPhase{
...
func (rt *serveRuntime) buildGateway(sf *serveFlags) {
	...
	srv, err := gateway.New(gateway.Config{
		...
		InKernelPlanner:              nativePlannerConfig,
