...
func TestRunDojoLiveFoldsScoredRows(t *testing.T) {
	...
	if got.Report.Measured != 2 || got.Report.Unmeasured != 9 {
		t.Fatalf("live report measured/unmeasured = %d/%d, want 2/9 (scored live row + dispatch-yield measured; provider-turns, provider-cache, cache-read-share, provider-cost, provider-tokens, provider-completion, and provider-toolcall each honestly unmeasured on the empty session corpus, plus context-restore honestly unmeasured while the context-span ledger records drops but no restores, plus provider-firsttry honestly unmeasured while the attempt ledger records dispatches but no per-worker gate attempts)", got.Report.Measured, got.Report.Unmeasured)
	}
