...
func execCodexLaunchChild(stdout, stderr io.Writer, argv, env []string) int {
	...
	defer stop()
	return execCodexLaunchChildContext(ctx, stdout, stderr, argv, env)
}

func execCodexLaunchChildContext(ctx context.Context, stdout, stderr io.Writer, argv, env []string) int {
	if len(argv) == 0 {
