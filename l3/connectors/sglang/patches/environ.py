...
class Envs:
    ...
    SGLANG_CAMA_REMOTE_PORT = EnvInt(18001)
...
def _maybe_start_rank_prewarm():
    """Start CAMA prewarm in this rank process if the parent signaled it.

    The parent process (preflight) sets _SGLANG_CAMA_PREWARM_SIGNAL as a
    JSON env var.  Since SGLang uses mp.set_start_method("spawn"), each
    child inherits the env var but gets a fresh interpreter — so we start
    an independent prewarm daemon here.  environ.py is imported very early
    (before Scheduler.__init__ / model loading), giving the prewarm the
    maximum window to complete in the background.

    Non-fatal: any failure falls back to blocking connect in CamaStorage.
    """
    signal = os.environ.pop("_SGLANG_CAMA_PREWARM_SIGNAL", None)
    if signal is None:
        return

    try:
        import json as _json
        cfg = _json.loads(signal)
        ...
        start_cama_prewarm(
            ...
            send_buf_size=cfg.get("send_buf_size", 0),
            nic_striping=cfg.get("nic_striping", True),
            model_page_bytes=cfg.get("model_page_bytes", 0),
        )
    except Exception:
        pass  # Non-fatal — CamaStorage will create a fresh connection
