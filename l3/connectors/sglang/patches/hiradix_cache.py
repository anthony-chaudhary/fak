...
class HiRadixCache(RadixCache):
    ...
    def reset(self):
        TreeNode.counter = 0
        # Release lock refs for any in-flight write/load acks before clearing
    ...
    def write_backup(self, node: TreeNode, write_back=False):
        ...
        if host_indices is not None:
            ...
            if len(node.host_value) == 0:
                logger.warning("write_backup: host_indices empty after write, node_id=%d", node.id)
                return 0
    ...
    def writing_check(self, write_back=False):
        if write_back:
            # blocking till all write back complete — but don't spin forever
            # if acks will never arrive (e.g. writes failed at host alloc).
            _max_spins = 1000
            _spins = 0
            while len(self.ongoing_write_through) > 0:
                if not self.cache_controller.ack_write_queue:
                    ...
                    if _spins >= _max_spins:
                        orphaned = len(self.ongoing_write_through)
                        logger.warning(
                            "writing_check(write_back): no acks after %d spins, "
                            "releasing %d orphaned entries",
                            _spins, orphaned,
                        )
                        for node in self.ongoing_write_through.values():
                            self.dec_lock_ref(node)
        ...
        while finish_count > 0:
            if not self.cache_controller.ack_write_queue:
                # Queue was cleared by a concurrent reset (e.g. reconnection).
                # Orphaned ongoing_write_through entries will be cleaned up on
                # the next reset or flush cycle.
                logger.warning(
                    "ack_write_queue drained mid-pop (likely reconnection reset), "
                    "skipping %d remaining acks",
                    finish_count,
                )
                break
        ...
        if self.cache_controller.write_policy == "write_back":
            ...
            for node in write_back_nodes:
                if node.backuped:
                    ...
                    # write_backup failed (host alloc drop) — evict without backup
    ...
    def _evict_backuped(self, node: TreeNode):
        ...
        if num_evicted <= 0:
            logger.error("_evict_backuped: evict_device returned %d for node %d", num_evicted, node.id)
            return 0
    ...
    def evict_host(self, num_tokens: int):
        ...
        while num_evicted < num_tokens and len(eviction_heap):
            ...
            if v is not x:
                logger.warning("evict_host: tree inconsistency for key %s, skipping", key)
                continue
    ...
    def load_back(
        ...
        while node.evicted:
            if not node.backuped:
                logger.error("load_back: evicted node %d has no backup, aborting load", node.id)
                return None
    ...
    def check_hicache_events(self):
        # Application-level cache operations must never crash the scheduler.
        # Prefetch misses, host alloc drops, and queue races are NORMAL under
        # memory pressure — absorb and log so the scheduler keeps running.
        try:
            self.writing_check()
        except Exception as e:
            logger.warning("writing_check failed (non-fatal): %s", e)
        try:
            self.loading_check()
        except Exception as e:
            logger.warning("loading_check failed (non-fatal): %s", e)
        if self.enable_storage:
            try:
                self.drain_storage_control_queues()
            except Exception as e:
                logger.warning("drain_storage_control_queues failed (non-fatal): %s", e)
        if self.enable_storage_metrics:
            try:
                ...
                logger.debug("storage metrics collection failed: %s", e)
