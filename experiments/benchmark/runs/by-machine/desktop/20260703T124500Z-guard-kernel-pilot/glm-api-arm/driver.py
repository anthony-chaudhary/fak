...
# Optional bearer for the guard-OFF baseline talking straight to a provider API.
# In guard-ON mode the child hits the local guard gateway (no auth) and the guard
# holds the real key upstream, so this stays unset there.
DIRECT_KEY = os.environ.get("FAK_PILOT_KEY", "")
# API arms cost tokens: allow a smaller sample than the free in-kernel arm.
N = int(os.environ.get("FAK_PILOT_N", "20"))
WARMUP = int(os.environ.get("FAK_PILOT_WARMUP", "2"))
