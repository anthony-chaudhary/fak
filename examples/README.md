...
| **Managed hosts** | [`corporate-tls/`](corporate-tls/README.md) | the corporate-laptop postures fak used to degrade on silently: TLS interception (a proxy re-signs every connection with a private root) and a request-signed cloud upstream (Bedrock/Vertex, where the child ignores the gateway repoint), each named at launch with a token, a read-only check, and a recovery plan — and raising nothing at all on a host that has neither. Seven witnesses, no proxy, no certificate store, no admin rights, no `-k`, no key/model/GPU/network |
...
| `remote-gpu-dev-policy.json` | Remote-GPU dev repo (ssh to GPU boxes, python/pip toolchains, TensorRT engine builds, org Atlassian MCP) — the shipped floor plus shared-trunk git discipline | full shipped-floor allow surface + `mcp__atlassian__` (deletes denied); remote `ssh host 'sudo …'` is allowed while LOCAL sudo stays denied (the structural sudo rule); no force-push, no bulk `git add`, no env-file staging | `go run ./cmd/fak preflight --policy examples/remote-gpu-dev-policy.json --tool Bash --args '{"command":"git push --force origin main"}'` |
