...
|---|---|---|
| **Onboarding** | [`deny-in-60s/`](deny-in-60s/README.md), [`trace-authoring/`](trace-authoring/README.md) | the 60-second first touch — one command, no key/model/GPU: an irreversible call is refused under a default-deny floor, then the same call clears under a permissive one; and author your own `fak run --trace` fixture from scratch (the trace schema + an ALLOW/DENY trace and a quarantine trace), no model or network |
