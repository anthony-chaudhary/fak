...
<!-- badges: durable facts about the project, not live CI/CD status. A workflow-status badge
     turns red on any transient run failure on main — normal for an agentic-velocity repo, and it
     makes the front door look broken when it isn't. These four are green by construction (a static
     license/version fact or a link to a durable resource), so the front door stays honest. Keep the
     Apache-2.0 license badge first. Do NOT add actions/workflows/*/badge.svg status badges here. -->
