...
#   QWEN_REPO       HF repo for the pinned GGUF        (default unsloth/Qwen3.6-27B-GGUF)
#   QWEN_FILE_GLOB  exact GGUF filename                 (default Qwen3.6-27B-Q4_K_M.gguf)
#   MODEL_ID        model id the gateway advertises    (default qwen3.6-27b)
...
# Pin the same exact Qwen3.6 artifact as tools/qwen36_a100_fak_serve.sh. The unit launcher
# verifies repository, filename, byte length, and SHA-256 before it can emit readiness. CUDA
# GDN/SSM parity remains tracked by #4714 and needs an independently read-back hardware run.
