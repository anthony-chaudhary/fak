...
| `COMPACTION_THRASH` | the served session's context window refilled to the compaction limit on 3 consecutive turns (#2424): history compaction FIRED every one of those turns and bought no lasting headroom, so the session is spending a compaction per reply and making no progress against its window. Detection and the /metrics count are always on; the session STOP that carries this reason at the served boundary is armed only by FAK_COMPACTION_THRASH_STOP. | restart the work in a FRESH window seeded from durable state (commit, plan, ledger) rather than letting the same transcript re-fill — tightening --compact-history-budget only compacts more often and changes nothing, because the binding constraint is the traffic, not the budget. Shrink the per-turn floor instead: window large reads, summarize bulky tool results, and drop tools the run does not use. Watch fak_gateway_compaction_thrash_sessions_total before arming the stop. Floor: go test ./internal/gateway -run TestCompactionThrashVerdict. |
...
| `INVALID_OPTIONS` | fak git-daily was invoked without a complete repository identity, so the unattended tick cannot safely choose its Git common directory or ledger. | Run fak git-daily from the target clone, or set RepoRoot from `git rev-parse --show-toplevel` and GitCommonDir from `git rev-parse --git-common-dir`, then retry. |
| `LEDGER_WRITE_FAILED` | Daily cleanup and maintenance completed, but fak could not append the bounded JSONL witness in the Git common directory. | Make the Git common directory and ledger writable, then rerun `fak git-daily --force` so completion is independently recorded. |
| `LOCK_CLEANUP_FAILED` | The daily tick found a lock that its evidence-gated cleanup could not safely reclaim, so maintenance did not proceed as if the clone were clean. | Inspect the lock sweep in `fak git-daily --json`; resolve the named live, ambiguous, or failed lock condition, then rerun the tick. |
| `MAINTENANCE_INCIDENT` | Daily object maintenance reached a guarded safety gate or failed command and recorded an incident instead of continuing unattended. | Inspect the maintenance refusal in `fak git-daily --json`, correct the reported repository condition, and rerun only after the safety gate is satisfied. |
| `MICROCONTEXT_LEDGER_REFUSED` | A micro-context witness, quality ledger, cache A/B, or health scorecard failed schema or accounting reconciliation, so fak refused to grade or publish it. | Run the matching `microcontextdemo -verify-* <artifact>` command printed by the producing stage; regenerate the artifact from the source run rather than editing totals by hand. |
...
| `DEPTH_NOT_CARRIED` | a `met` closure claims an objective's goal was reached while its declared plan is not carried — a declared phase has no witness in the latest W3 commit-progress row, or no plan was declared at all. The depth mirror of `FOCUS_WIP_SATURATED`: that bounds fan-out, this refuses a line that stopped short | carry the remaining phases (bind each landing commit with a `Trajctl-Phase: <id>` trailer, then `fak trajctl score`), read the frontier with `fak trajctl depth --id <id>`, or close `--status abandoned` (never refused) if the line is dead. One-shot escape: `--force`. Floor: `go test ./internal/depthadmit` |
...
| `FLEETBUS_APPLY_REFUSED` | An instance KNEW the op and refused to apply it -- no live session matched, none were tagged into the stated lane/wave/label, the op needs a per-session payload a bare fan cannot name (redirect/budget/priority), or the local route refused it outright (a steer at a serve that owns no session loop carries STEER_NO_OWNED_LOOP through in the detail). An instance that matched nothing refuses rather than acking a hollow 'applied', because a fleet-wide 'everybody applied, 0 affected' is the accepted-but-never-applied phantom wearing a success token. | Read the ack's detail -- it names WHICH nothing this was and each is a different fix: start the serve with --native for steer, drop the session selector if the sessions are untagged, or use the single-session verb for a payload-carrying op. Floor: go test ./cmd/fak -run FleetBusApplier. |
| `FLEETBUS_EXPIRED` | A directive reached an instance after its TTL had run out, so the instance ACKED it expired instead of applying it. A control directive is a statement about NOW ('restart, the account was just added'); replaying an hour-old one against an instance that was down for that hour is a fresh action wearing a stale intent. | Re-issue the directive if it is still what you want (`fak fleet control send` re-publishes with a new id), or raise --ttl for an op whose intent genuinely outlives the window. --ttl 0 means never expires -- state it deliberately. Floor: go test ./internal/fleetbus -run Expire. |
| `FLEETBUS_MALFORMED` | A fleet-bus envelope could not be trusted as WRITTEN -- a missing or wrong schema tag, an id outside the [A-Za-z0-9._-] token grammar (ids are path segments, so '.' / '..' / a separator would escape the bus directory), an empty selector (an unstated set never means 'every instance'), a selector that states both --all and a filter, or an ack claiming a non-applied status with no reason token. | Fix the envelope at its producer, not at the reader: state at least one selector axis or an explicit --all (never both), keep every id a bare token, and give every non-applied ack a closed reason. Floor: go test ./internal/fleetbus -run 'Validate|Selector'. |
| `FLEETBUS_NO_TARGET` | A send resolved to ZERO instances -- the roster held no fresh instance the selector addressed -- so it was refused at the edge rather than published. Publishing to nobody would return a report that is trivially 'complete' (0 of 0 applied) and read as success; the denominator is the whole point of the roster. | Run `fak fleet control instances` to see who is actually announced and fresh, widen or correct the selector, or arm the instances first (`fak serve --fleet-bus`). An instance silent for longer than the TTL is stale and is not a target. Floor: go test ./cmd/fak -run FleetControl. |
| `FLEETBUS_UNKNOWN_OP` | An instance received a directive whose op is outside ITS control vocabulary and acked it refused. The bus is deliberately op-agnostic (it carries an opaque token), so a mixed-version fleet is a real state: the token names the instance that cannot speak the word, rather than dropping the directive and leaving it forever OUTSTANDING. | Send an op the instance advertises (`fak fleet control instances` prints each instance's ops), or upgrade the instances that refused. A refused ack is a completed round trip -- the directive is accounted for, not lost. Floor: go test ./cmd/fak -run FleetBusApplier. |
...
| `OVERLAY_WOULD_GATE` | A change would wire the read-mostly steer/prplan overlay into a blocking path: an overlay code path reaching a git mutation (commit/push/reset/revert/merge/rebase/force-push), an overlay call that can fail a commit, or a --check exit code consumed by a commit, hook, or promotion path. The overlay's contract is observability WITHOUT a merge gate; a gating overlay is refused as a class, not patched as an instance. | Keep the overlay advisory: derive git state only through read-only verbs (log/rev-parse/show/...), let --check report RESIDUAL solely from an operator or CI reporting invocation, and keep commit/hook/promotion paths free of overlay references (fold/render helpers stay fair game -- rendering is the overlay's job). If a gate is genuinely wanted, build it in the kernel's witness path (dos commit-audit / shipgate), never in the overlay. Floor: `go test ./internal/architest -run Overlay`. |
...
| `PIN_EVICT_REFUSED` | A requested memory-pin eviction would discard retained context that still has live dependent references. | Keep the pin; close or rebind the dependent references, then retry eviction. Do not force-remove retained memory. |
| `RELAY_IDLE_PARKED` | The goal relay is healthy but correctly idle, with no actionable objective or observed state change. | Leave it parked; supply a concrete objective or wait for the watched state to change, then resume the relay. |
| `RELAY_NO_PROGRESS` | The goal relay repeated work without producing a new independently witnessed effect. | Inspect the latest witness and blocker, narrow or repair the next action, then resume; do not count another narration-only turn as progress. |
| `RELAY_ORPHANED_FOLLOWON` | A relay follow-on names no live parent objective or durable ownership link. | Rebind the follow-on to a live parent goal/issue or file it as independently owned work before dispatching it. |
| `SESSION_CEILING_SATURATED` | The session/account ceiling has no free admitted slot. | Do not bypass the ceiling; wait for or reclaim a witnessed stale slot, then rerun the authoritative capacity preflight. |
| `SKILL_DESC_BUDGET_EXCEEDED` | The skill description exceeds the enforced discovery-context budget. | Shorten the description to the trigger and scope essentials, preserving detailed procedure in `SKILL.md`, then rerun the skill budget check. |
| `SKILL_DESC_BUDGET_STALE` | The recorded skill-description budget witness no longer matches the current skill inventory. | Regenerate the budget witness from the current inventory, inspect the delta, and commit the refreshed evidence with the skill change. |
| `TICK_BUSY` | Another live dispatch tick currently owns the serialized tick lease. | Do not start a competing tick; wait for the live holder or reclaim it only after the lease evidence proves it stale, then retry. |
| `TICK_LOCK_ERROR` | The dispatch tick could not read or update its serialization lock safely. | Inspect and repair the lock/lease store, preserving a live owner; rerun preflight before retrying the tick. |
